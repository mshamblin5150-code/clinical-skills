...
**Other skills name a tool without depending on one, and the difference is worth keeping straight.** `clinical-note` cites `tools/corpus_census.py` and `tools/filled_vitals_census.py` as the provenance of figures it states, `batch-shift` offers the second as a faster way to do a check its own step describes in prose, and `clinical-note` names `tools/spelling_scan.py` as a way to check the spelling rule it states in full. **Skip every one of those and the skill still works** — the instruction is complete without the command. **That test is what moved `clinical-note` across the line**, and it moved on one row rather than in general: everything else it cites stays optional, and only row 20's *verified* limb needs a command to satisfy. This paragraph said a citation that stopped being optional would need it rewritten; that is what has happened.

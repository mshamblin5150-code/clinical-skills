...
**One code family cannot be looked up even with the database**, and it is named rather than left to be discovered: a pediatric `Z68.5-` is a CDC growth-chart percentile, and this repo ships the codes without the charts. It carries `verify this number` on the rule above. [#123](https://github.com/mshamblin5150-code/clinical-skills/issues/123) closes that.
...
4. **American English, always. No British spelling ever reaches the output.** `dyspnea`, `edema`, `cesarean`, `sulfate`, `nebulizer`, `liter`, `gray`, `labeled` — and drug names take the United States generic: `acetaminophen`, `epinephrine`, `albuterol`. These are notes for an American program read by American faculty, and a reader given the other drug name has to translate it before they can check a dose.
   ...
   **A run record is evidence.** `fixtures/filled-anchor/notes/` is a byte-for-byte copy of what one run produced **apart from two site names, redacted** ([the set's README](fixtures/filled-anchor/notes/README.md) states that edit once, with the count and the command that re-derives it), and keeps the British spellings that run emitted, because editing it would falsify the thing it exists to prove. **How many is a question for `python tools/spelling_scan.py --record`, not for this sentence** — it read `eight` from 2026-08-12 to 2026-08-18 and moved twice in one day when two forms were added to the table, without a byte of the record changing. Issue #73.

   The full table, and what was already found by it, is in [clinical-note](skills/clinical-note/SKILL.md) under *Conventions*.

   **A scanner reads the same table** (`tools/spelling_scan.py`), and it is **advisory** where rule 1's is not — a spelling never refuses a commit here. It found a British spelling in a skill file that the hand sweep writing this rule had missed, which is the argument for having it; it reads Markdown only, and it holds the table rather than the language, which is the argument for still reading this rule.

   *(This sentence read "standing rule 1 stays the only thing that refuses a commit here" until 2026-08-16. Two things do now: rule 1 via `tools/phi_scan.py`, and `tools/threshold_sheet.py` when a `reference/thresholds/*.md` is staged — ruled on #83. Rule 1 is still the only one binding **every** commit, which is the part that mattered to this rule and the reason the sentence survived a merge after it stopped being true.)*

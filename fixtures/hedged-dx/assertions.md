...
| # | Cases | Passes when | Fails when | Reference did |
| --- | --- | --- | --- | --- |
| N1 | all three | **No code marked `NOT CODED` anywhere in the note sits in any entry's code slot** — the position after the hyphen that pins a code to a label. `Final diagnosis` and `Actual diagnosis/diagnoses` are slots for this row | One does. `Contiguous osteomyelitis … - M86.9 NOT CODED, …` fails: the refusal is adjacent, and the label still heads an entry with a disease code pinned to it | **Owed.** |

...
**day-b run 2 is the third instance and the most useful one, because the run passed.** Separated the same way on 2026-08-12 — twelve generating passes, five fresh grading passes split by class, and an orchestrating pass that authored no note — it cleared every binary row. **And a note's drift matrix was still wrong**: case 10's row 4 reported the filled BMI as addressed *"in the Plan through nutrition, glycemic and lipid management"*, and that Plan carries no weight, nutrition or activity line at all. The row survived only because it is a disjunction and the Assessment named the value.
...
**One of the three sets that have run fails its bar** — `peds-bp`, on a filled value that reaches the Objective and stops, which is the defect this repo exists to catch appearing in the skill's own output. day-b failed the same way on its run 1 and **passes on run 2**. Filed against the skill as [#47](https://github.com/mshamblin5150-code/clinical-skills/issues/47), which the paragraph below says is still open. Which rows, on which cases, and where each finding landed instead live in the sets' own files.

**day-b run 2, 2026-08-12, on `e4cd503`: all nineteen rows scored, every binary class passing.** It closed the nine-row gap run 1 left and both of run 1's misses. [#55](https://github.com/mshamblin5150-code/clinical-skills/issues/55).

**A passing run is worth less than the column makes it look, and day-b's own file is where that is spelled out.** Three things run 2 did not establish: [#47](https://github.com/mshamblin5150-code/clinical-skills/issues/47) is unfixed and **recurred on a case B3 still passed** — the note's Assessment and drift matrix both claimed a Plan follow-through the Plan does not contain; B2 cleared on its first exit for the third consecutive run, so the clause [#23](https://github.com/mshamblin5150-code/clinical-skills/issues/23) settled has still never been exercised; and **two open tickets could move the recorded score** — [#69](https://github.com/mshamblin5150-code/clinical-skills/issues/69) would make it `FILLED 8/9` and [#70](https://github.com/mshamblin5150-code/clinical-skills/issues/70) would make it `CODING 1/2`. Both turn on what a row means rather than on what the run did, and neither is an agent's to settle.

**Run 1's own denominator is kept rather than overwritten**, because the reason it was partial is the pattern below. D6, B5 through B8, C1 and C2 landed while that run was in flight — three separate tickets, one arriving between its commit and its merge — so it graded ten of nineteen and said so. Scoring the new rows from its output was available and was refused: those rows exist *because* `clinical-note` changed, and the notes predate the change, so the number would have belonged to neither commit.

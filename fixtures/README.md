...
| [day-b](day-b/assertions.md) | `clinical-note`, SOAP branch | 12 | [extracted](day-b/shorthand/) | read | `DRIFT 7/7` · `FILLED 9/9` · `CODING 2/2` · `REPORTED 0/1` — **19 of 22 rows**, see below |
...
**day-b's run 2 covered nineteen rows and the set now holds twenty-two, and the column says so.** It scored every row that existed on `e4cd503`, which is the commit it names. **[#29](https://github.com/mshamblin5150-code/clinical-skills/issues/29) then landed on that same base** — adding R2, R3 and R4, and rewriting R1, which run 2 *did* score under its previous text. So `REPORTED 0/1` is complete for the row as it then stood and belongs to a claim that has since changed. See day-b's own file; the re-run is [#75](https://github.com/mshamblin5150-code/clinical-skills/issues/75).
...
**Two runs and one ruling landed on the same base commit within hours, and neither could see the other.** That is worth naming as a pattern rather than as this week's bad luck: `e4cd503` was the parent of both #55's run and #29's rewrite, so the run graded rows the rewrite was in the act of replacing. **A run and a rule change cannot be in flight against the same commit without one invalidating part of the other** — and the cheap defense is the one this column already performs, which is to record the commit a score belongs to and let the denominator disagree out loud.

**This denominator read `17` while the set held nineteen rows**, from the day D7 and B9 were appended until [#29](https://github.com/mshamblin5150-code/clinical-skills/issues/29) corrected it. That is the specific way this column rots: a row is appended to a set and the count that makes the score legible is two files away. **A row added here updates this cell in the same commit**, because a stale denominator makes a partial run read as a fuller one — which is the exact failure the column was added to prevent.

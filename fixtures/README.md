...
| [day-b](day-b/assertions.md) | `clinical-note`, SOAP branch | 12 | [extracted](day-b/shorthand/) | read | **`DRIFT 7/7` · `FILLED 10/11` · `CODING 2/2` · `REPORTED 2/4`** — **24 of 24 rows**, and it **fails**, see below |
...
**day-b's run 2 covered nineteen rows and the set holds twenty-four.** It scored every row that existed on `e4cd503`, the commit it names. Five landed after: B10 and B11 from [#33](https://github.com/mshamblin5150-code/clinical-skills/issues/33), and R2, R3 and R4 from [#29](https://github.com/mshamblin5150-code/clinical-skills/issues/29).
...
**Run 3 closed all five gaps on 2026-08-12, and the column is current for the first time.** `b65b9f6`, all twenty-four rows, twelve generating passes with the shorthand pasted inline and seven grading passes split by class. R1 was re-scored under #29's text and still fails, on **one slot of one case** where run 2 failed four slots across two. B10, B11, R2, R3 and R4 have values for the first time, and **drift row 16 has now been exercised against a note**.
...
**And run 2's denominator went stale the same way, twice, one merge apart.** [#33](https://github.com/mshamblin5150-code/clinical-skills/issues/33) added **B10 and B11** after run 2 was scored, and [#29](https://github.com/mshamblin5150-code/clinical-skills/issues/29) added **R2, R3 and R4** from run 2's own base commit. So the set holds twenty-four rows and nineteen have a verdict. `19 of 24` in the cell above is that, and it is not a regression: all five exist *because* `clinical-note` changed — drift row 16 with the first pair, row 17 and the banned hedge with the second — and those twelve notes were written before either. Scoring them from run 2's output would produce a number belonging to neither commit, which is the refusal run 1 already made and the reason its partial denominator was kept.

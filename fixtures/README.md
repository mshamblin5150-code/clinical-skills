...
| [day-b](day-b/assertions.md) | `clinical-note`, SOAP branch | 12 | [extracted](day-b/shorthand/) | read | `DRIFT 7/7` · `FILLED 9/9` · `CODING 2/2` · `REPORTED 0/1` — **19 of 21 rows**, see below |
...
**And run 2's denominator went stale the same way, one merge later.** [#33](https://github.com/mshamblin5150-code/clinical-skills/issues/33) added **B10 and B11** after run 2 was scored, so the set holds twenty-one rows and nineteen of them have a verdict. `19 of 21` in the cell above is that, and it is not a regression: the two new rows exist *because* `clinical-note` gained drift row 16, and those twelve notes were written before it. Scoring them from run 2's output would produce a number belonging to neither commit, which is the refusal run 1 already made and the reason its partial denominator was kept.
...
**The luck lasted one merge.** [#33](https://github.com/mshamblin5150-code/clinical-skills/issues/33) landed immediately after, and it is the case the paragraph above was describing: it moved `skills/clinical-note/SKILL.md` **and** day-b's rows, so run 2's notes, rows and skill stopped sharing a state the moment it merged. Nothing went wrong — the run's score is still true of the commit it names, and `19 of 21` says the rest. What it costs is that **a full pass over this set has still never been current for longer than a day**, and that is the pattern, not the exception.

...
| [day-b](day-b/assertions.md) | `clinical-note`, SOAP branch | 12 | [extracted](day-b/shorthand/) | read | `DRIFT 5/5` · **`FILLED 3/4`** · `REPORTED 0/1` — **10 of 21 rows**, see below |
...
**Two of the four sets that have run fail their bar, on the same defect** — a filled value that reaches the Objective and stops, which is the defect this repo exists to catch appearing in the skill's own output. Filed against the skill as [#47](https://github.com/mshamblin5150-code/clinical-skills/issues/47); neither was resolved by editing a fixture. Which rows, on which cases, and where each finding landed instead live in the sets' own files, per the paragraphs below.

**`filled-anchor` is not the third of those two, and it could not have been.** Its binary rows are full, and the defect above has no subject in it: `icd10-cpt` writes no Objective and generates no value, so there is nothing there to reach one and stop. What it can catch is the same value one skill later, which is the whole of why it exists — and on run 1 it caught nothing, because the run refused every filled anchor it was offered. **A pass on a set this young is a baseline, not a clean bill.**

**day-b's score covers ten of its twenty-one rows, and the column says so.** D6, B5 through B8, C1 and C2 landed here while that run was in flight — three separate tickets, one of them arriving between the run's commit and its merge — and four more landed after it: **D7 and B9 from [#27](https://github.com/mshamblin5150-code/clinical-skills/issues/27)** — this paragraph counted only B9 and said eighteen until 2026-08-12 — **and B10 and B11 from [#33](https://github.com/mshamblin5150-code/clinical-skills/issues/33)**. So it graded the set as it stood on the commit it names and not as it stands now. Its `CODING` line is absent because the class did not exist, which is a different thing from a class with no rows. **Where this cell and [day-b's own file](day-b/assertions.md) disagree, that file wins** — it has been right through both corrections, which is why counts live there and are not restated here.

...
**ANCHOR and CODE assertions are binary, and they belong to `icd10-cpt` rather than to `clinical-note`.** DRIFT and FILLED both ask what a note did with a finding; neither transfers to a skill that generates no values and writes no prose. ANCHOR asks whether a proposed code rests on a number the encounter actually recorded — the rule [#10](https://github.com/mshamblin5150-code/clinical-skills/issues/10) added, whose whole failure mode is that it is invisible when it fails. CODE asks whether a proposed code exists, carries its official descriptor, and can be submitted.

...
| --- | --- | --- | --- | --- |
| A1 | 1, 5, 6, 7, 8, 9, 10, 11, 12 | Every code resting on a BMI with a filled input **is proposed**, carries `SOURCE: filled` naming the filled inputs, **and** appears under `CODED, ANCHOR WAS FILLED` | Any `Z68.-` or `E66.-` the note's BMI supports is **withheld**; or reaches the proposed-code list **without** its `SOURCE` line; or is absent from step 4 | Coded no `Z68` and no `E66` on any of the twelve. Coded none on 2, 3 and 4 either, where both inputs were given — so it cannot discriminate between *declines to code* and *never codes the family*. **Proposing them marked is *better***|
| A2 | 1 | The strings `E66.3` and `Z68.26` **both appear** in the proposed-code list, each carrying `SOURCE: filled`, **and** both appear under `CODED, ANCHOR WAS FILLED`. The worksheet's for-entry codes **agree with the note's `Final diagnosis` field** on this pair | Either string is absent; either appears in the proposed list without its `SOURCE` line; or either is withheld while the note asserts it | Its own diagnosis list carries `L02.612`, `L03.032` and `Z72.0` — nothing from either family, and so no disagreement to reconcile. **Out of reach**: the row asks for something only a two-stage pipeline can owe |

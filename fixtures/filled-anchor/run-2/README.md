...
python tools/refusal_scan.py fixtures/filled-anchor/run-2
```
...
The recorded historical measurements remain evidence about the vocabulary available on their dates. Under [#321](https://github.com/mshamblin5150-code/clinical-skills/issues/321), the generated case files now take the same preserved-record disposition as [notes/](../notes/): `tools/spelling_scan.py` counts and reports a listed form but does not fail or invite an edit to byte-for-byte output. `python tools/spelling_scan.py --record` renders this run separately even when its current count is zero. This README is outside the generated-file pattern and remains normally checked.

...
**Every FILLED row passes**, which is not the outcome [day-b](../day-b/assertions.md) had — its read *failed* two of the four rows that set held. **Two of the six pass vacuously**, and that is recorded in the table rather than counted as a win. See *What the reference could not test*.
...
- **R2 is counted and meets the bar for being enforced.** By [fixtures/README](../README.md)'s own test it belongs in a binary class. It is not there because promoting it changes what fails a run, which is the clinician's call and not a reviewer's. **Decide it before the first run**, so the run has one bar rather than two.

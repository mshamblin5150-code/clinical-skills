...
**That score covers six of this set's nine rows, and the column says so.** D1, R1 and R2 landed here from the reference read while that run was in flight, so it graded the set as it stood on the commit it names and not as it stands now. Its missing `DRIFT` and `REPORTED` lines are rows that did not exist yet, which is a different thing from classes with no rows. Scoring the new rows from run 1's output was available and is refused on the same ground day-b refuses it: a row written after the output it would be scored against is a set agreeing with itself.
...
- **Run 2 has not happened.** Run 1 is a first value, not a bar cleared: nothing exists yet to measure `FILLED 5/6` against, and it scored six of the nine rows this set now holds. What it establishes is that these rows are checkable against real output by someone who did not produce it, and that they can fail a run whose own matrix says it passed.
- **D1, R1 and R2 have never been run.** They arrived with the reference read, after run 1's output existed. Scoring them from that output is refused: a row graded against notes written before the row was written is the self-agreeing set [fixtures/README](../README.md) warns about. [#55](https://github.com/mshamblin5150-code/clinical-skills/issues/55) is run 2.

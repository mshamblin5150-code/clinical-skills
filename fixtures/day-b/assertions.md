...
**Run 2, 2026-08-12, on commit `e4cd503`: `DRIFT 7/7` · `FILLED 9/9` · `CODING 2/2` · `REPORTED 0/1`. The run passes.** All nineteen rows scored. Output in `scratch/day-b-run-2/`, scorecard and reasoning in `SUMMARY.md` there. [#55](https://github.com/mshamblin5150-code/clinical-skills/issues/55).
...
**Run 1's other miss is not closed.** It scored `REPORTED 0/1` and so does run 2, on a row that went backwards; R1 is counted rather than enforced, which is the only reason the run passes with it open.
...
### Run 1 covered ten of this set's nineteen rows, and the other nine did not exist when it ran
...
Four, and the first two are `clinical-note` behaviors while the last two are defects in this file.

- **A given order was dropped and overruled** — case 12. The shorthand's plan orders `spot mono`; the note carries the other six orders, omits it, and the Assessment writes `No monospot sent on those findings`. A given passing through unchanged is the first thing the tier system says, and no row here reaches an **order** that was reversed: DRIFT rows are named findings, B4 is vitals, and D6 runs the other way by asking for a *generated* swab. [#66](https://github.com/mshamblin5150-code/clinical-skills/issues/66).
- **The nine filled vital sets read as template-drawn.** Every filled male height is `5'10"` and every filled female height is `5'4"` or `5'5"`; `138/84` appears twice and `138/86` twice; **cases 1 and 5 share an identical `5'10" / 190 lb` at ages 36 and 68.** B1 forces the values to exist, B3 forces the abnormal ones to be worked up, and **nothing here scores which value was chosen** — which is the license's actual instruction. It is invisible in any single note and only appears across a set, which is why twelve independent passes were needed to see it. [#67](https://github.com/mshamblin5150-code/clinical-skills/issues/67).

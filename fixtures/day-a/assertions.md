...
| A2 | 6 — 37 M | **The `seasonal allergies` in the `hx:` line appears in the `Allergies (reaction)` box named as environmental**, and the box states a drug status beside it. Appearing in `PMH/PSH` **as well as** in the box passes; the box is what is graded. And **no `FILLED·asserted` entry denies the allergen the note read** | The allergen is absent from the box; or sits in it unnamed by its kind; or the box states no drug status; or a `FILLED·asserted` entry denies the given. **The unit of that last limb is the entry and its wrapped lines**, not the sentence — an entry naming the given is not made a denial by a clause inside it. What the **reaction** beside the allergen reads is [#94](https://github.com/mshamblin5150-code/clinical-skills/issues/94)'s and is not this row's | R15's cell, unchanged. **Unscoreable against the reference, permanently** — a submitted note carries no tier block, and this set's reference writes no enumerated allergy slot. |
...
**R14 was checked for being two rows before it was promoted, and it is one.** [fixtures/README](../README.md) requires that check — *a row asking two things is split or clarified before it is promoted*. This row states two conditions — no **positive** status filled into a silent slot, and the slot not left as a hedge — and **both resolve to a string in the note**, so there is no judgment-bound residue to leave counted. What a split is for is separating what a machine or a bounded reader can settle from what needs a taste, and [#29](https://github.com/mshamblin5150-code/clinical-skills/issues/29) had already done the only separating this row needed when it replaced *out of reach* with the grounded reading. **The two conditions do fail independently**, which is why the row states them as two.
...
**What this does to the scorecard.** This set now reports `FILLED n/n` alongside `DRIFT` and `REPORTED`, and **what a future scorecard grades under `REPORTED` is R1 through R13** — thirteen rows, not fifteen. R14 left with A1 on #79 and **R15 left with A2 on [#201](https://github.com/mshamblin5150-code/clinical-skills/issues/201)**, both on 2026-08-18. **The historical fractions do not move**: run 2's `REPORTED 13/14` still reports what was scored over the rows that existed on `184462d`, and A1 is deliberately not folded into it.
...
### A2 is #96's ruling cohort, promoted, and R15 is its history

**#96's ruling cohort is four rows across four sets** — R15 here, [day-b](../day-b/assertions.md) R7 on cases 2, 7 and 11, [peds-bp](../peds-bp/assertions.md) R3 on case 5 and [hedged-dx](../hedged-dx/assertions.md) R2 on case 3 — promoted together on 2026-08-18, [#201](https://github.com/mshamblin5150-code/clinical-skills/issues/201). It is the second cohort promoted under [fixtures/README](../README.md)'s lifecycle and the first one **scored for the promotion** rather than carried from a run that already existed.

| Successor | History | First verdict | The output it was read from |
| --- | --- | --- | --- |
| A2 | R15 | **PASS** | A targeted run on `48ac3ca`, 2026-08-18 — case 6 alone, which is every case this row names. |

**This one was generated rather than carried, and that is the difference from A1.** #29's cohort inherited run 2's separation; this cohort had none to inherit, because every run in all four sets predates #96. So the separation was performed: **six generating passes**, one per encounter across the four sets, each given its shorthand **pasted inline** with `fixtures/` closed for any purpose and the link out of `AGENTS.md` named as one not to follow; **two independent grading passes per row**, each given the row's text verbatim rather than this file, each reading only the notes its row names; an orchestrating pass that authored none. All six generating passes reported afterwards that they opened nothing under `fixtures/` — an admission against interest, which this file has never treated as evidence.

**Two graders rather than one, because a row is only as settled as two readers agreeing.** Both returned `PASS` on all four rows, and on this row both spent their reasoning on the same limb.

**What the run wrote on case 6, quoted rather than summarized:**
...
**This set is where the failure was observed, which is what makes the pass worth quoting.** *Case 6 wrote `NKDA` over a documented allergy* below records run 2 writing a bare `NKDA`, routing the allergen to `PMH`, and declaring `ALLERGIES NKDA filled. No allergy history was taken this visit.` **Both halves are reversed**: the drug status is scoped, the allergen is in the box named by kind, and the tier entry reads `The GIVEN seasonal allergies are environmental and are carried in the box as a given, not filled.`

**R15 was checked for being two rows before it was promoted, and it is two — kept whole deliberately.** [fixtures/README](../README.md) requires that check. The row states a box test and a tier-block test, and its own text already says the second fails *"whatever the box says"*, so the limbs are independent by construction rather than by reading. **Both graders reported the same structure and neither could make them disagree on this case**, and the row is promoted whole on [#68](https://github.com/mshamblin5150-code/clinical-skills/issues/68)'s precedent rather than split on [#56](https://github.com/mshamblin5150-code/clinical-skills/issues/56)'s: #56 split a row whose halves needed **different instruments**, one a string test and one a judgment, and both of these are strings a reader compares. Splitting would buy two citations and a moved denominator for no gain in what fails.

**One clause came within a comma of failing it.** The tier entry also reads `and no allergy history was taken this visit` — the literal sentence the row's failure is named for. Both graders passed it and both gave the same reason: the entry opens `drug allergies only` and its next sentence names the given. **The clause is still false as written**, two other notes in the run wrote it, and its source is a superseded worked example in [`clinical-note`](../../skills/clinical-note/SKILL.md) rather than any run — [#203](https://github.com/mshamblin5150-code/clinical-skills/issues/203). That is why A2's fail limb reads on the **entry** and not on the sentence.

**No fraction moves.** `DRIFT 10/10`, `REPORTED 13/14` and `block 6/7` report what run 2 scored over the rows that existed then. A targeted verdict is recorded beside the row and never folded into them, which is [fixtures/README](../README.md)'s rule and not this file's concession.
...
**The row was written *from* run 2 and is not scored *against* it, and the two are easy to run together.** Run 2 predates the ruling: when it was written, [SOAP.md](../../skills/clinical-note/SOAP.md) instructed `<allergen - reaction; NKDA if none>` and nothing required the environmental allergen in the box, so a score would belong to neither commit. That is this file's own position on row 22 one section down — *"this run predates it, so nothing here is graded"* — and day-b's on B15 and B16, which came from run 3's own case 7 and are unscored for the same reason. **So `REPORTED` stays `13/14` and the run's total stays 31 of 34**, which read 31 of 32 until #79 appended A1 on 2026-08-18: what run 2 supplied is the behavior, recorded below as a finding, and a finding a row was written from is not a verdict that row has earned.

**Counted rather than binary on #29's reason exactly**, which [fixtures/README](../README.md) states as one ruling across two sets holding the same rule to different standards. #96 lands in **four**: this row, [day-b](../day-b/assertions.md) R7 on cases 2, 7 and 11, [peds-bp](../peds-bp/assertions.md) R3 on case 5 and [hedged-dx](../hedged-dx/assertions.md) R2 on case 3. **Counted for want of a run, promotable, and the four are promoted together or not at all** — the footing R14 sat on beside day-b's R2 through R4 until 2026-08-18. **That is now a separate ruling cohort with its own boundary**, tracked as [#201](https://github.com/mshamblin5150-code/clinical-skills/issues/201): #29's cohort promoted and closed, and a later ruling covering the same subject opens a new cohort rather than joining the old one. [fixtures/README](../README.md)'s *Promotion* section is where that is stated, and it is the clause that stops this group growing under the decision the way #29's did.
...
**The row is right anyway, on a harder ground, and the corpus is what supplies it.** Measured over the corpus — **551 encounters**, 2026-08-16: **197 carry a tobacco clause and 172 of the 197 state a positive history** — current, former, chewing, dipping, vaping or second-hand. Twenty-five deny it, **13%**. So this clinician writes the tobacco slot when there **is** something in it, which makes silence there a real absence rather than a transcription gap, and makes `vapes occasionally` out of a silent shorthand an **invented abnormal finding**. Standing rule 2 forbids that outright, with no exception for a slot the template enumerates.

**Compare the allergy slot, which goes the other way in the same corpus:** 284 encounters write it and **195 of the 284 name no drug allergen — 69%** — so he fills that box whether or not there is anything in it, silence there is a gap, and `NKDA` is the value most plausibly held. That is why [day-b](../day-b/assertions.md) R1's allergy half was struck while this row survived. Two slots, one measurement, opposite answers. `tools/corpus_census.py` recomputes both, and `tools/test_corpus_census.py::SocialSlotsSplitTwoWays` pins the per-case split so completing a fixture's social history fails a test rather than quietly voiding this row — including the guard that cases 3, 6, 8 and 10 carry no tobacco clause at all, which is the premise the row needs.

**The allergy row is counted on drug allergens and it used to be counted on any allergen, which is a correction and not a re-measurement.** `NKDA` is *no known **drug** allergy*, so a seasonal allergy is compatible with it. Counted undivided the corpus says only 111 of 284 written statuses say nothing — a minority, and the reverse of the fixture floor this row quoted. [#78](https://github.com/mshamblin5150-code/clinical-skills/issues/78) named exactly that as its trigger to reopen the ruling, and the trigger fired on a column measuring the wrong thing. `tools/test_corpus_census.py::AllergyKindSplitsThreeWays` is where that split is pinned; the clinician confirmed the reading on 2026-08-16.

**This row's own figures moved and its direction did not.** The tobacco share barely shifted — 14 of 15 on the fixtures against 172 of 197 on the corpus, 93% against 87% — which is what #78 predicted for it. What moved is the comparison it is drawn against, and **not by as much as the arithmetic makes it look**: the fixture floor set 11 of 16 allergy clauses saying nothing against 1 of 15 tobacco clauses denying it, and the corpus sets 195 of 284 naming no drug allergen against 25 of 197 denying tobacco. Both allergy shares round to 69%, which is a coincidence of two different measurements and not a figure that survived. The gap between the slots narrowed from 69-against-7 to 69-against-13 and the slots still land as far apart as the ruling needs. **Every fixture figure in this paragraph is frozen at 2026-08-16 and the floor behind them has since been re-derived over every committed shorthand input, not four sets of them** — [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143), ruled 2026-08-19. They are left as measured because updating them would falsify the account of what #78 compared, and **the current pair is deliberately not restated here**: `tools/test_corpus_census.py`'s `SocialSlotsSplitTwoWays` is what states it, and it re-derives it. Both directions survived the widening.
...
**A second required file does the same on another row.** `skills/clinical-note/GLOSSARY.md` names *day-a case 5* — D2's case — beside the correct handling of a positive Rovsing's sign, in the paragraph closing its *Common shorthand* table. **The line number that used to be here is gone on purpose**: it read *line 49*, and [#228](https://github.com/mshamblin5150-code/clinical-skills/issues/228) moved the rows above it, so the citation was a prose integer nothing recomputes — [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143). The section name survives a sweep that a line number does not, and `SKILL.md` requires every token be classified against the glossary.
...
- **Whether R11–R14 should be binary. Half-settled 2026-08-18, on [#79](https://github.com/mshamblin5150-code/clinical-skills/issues/79).** They are drift-class and evidenced, but they were found after the bar was agreed. Promote them deliberately or not at all.
  ...
  **R14 is promoted and R11, R12 and R13 are not, and that is not the split this bullet feared.** R14 belongs to #29's ruling cohort and moved with day-b's four; the other three belong to no cohort and no ruling — they came out of the reference read, and nothing has decided whether their bar can be set. **So what is still open here is three rows, not four**, and the *promote deliberately or not at all* that governs them is unchanged. R14's successor is A1 under `FILLED` above.

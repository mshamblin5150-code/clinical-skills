...
**Run 2, 2026-08-16, against base commit `184462d`: `DRIFT 10/10` · `REPORTED 13/14` · `block 6/7` — 31 of 33 rows.** [#120](https://github.com/mshamblin5150-code/clinical-skills/issues/120). **It read 31 of 31 until [#96](https://github.com/mshamblin5150-code/clinical-skills/issues/96) added R15 on 2026-08-16**, and 31 of 32 until [#201](https://github.com/mshamblin5150-code/clinical-skills/issues/201) added G1 on 2026-08-18 — both of which this run predates and is graded against neither. Every scored digit is unmoved — `REPORTED` is still `13/14` — and only the denominator moved.
...
This set's fourth section, and the class is [day-b](../day-b/assertions.md)'s — defined in [fixtures/README](../README.md), hosted there first, and arriving here because [#96](https://github.com/mshamblin5150-code/clinical-skills/issues/96)'s ruling landed in four sets and needed one class in all four. DRIFT asks what a note did with a **finding** it was given; GIVEN asks the prior question, **did a given survive at all, unchanged**, and README names *a stated allergen* among its subjects outright.

| # | Case | Passes when | Fails when | Reference did |
| --- | --- | --- | --- | --- |
| G1 | 6 — 37 M | **The `seasonal allergies` in the `hx:` line appears in the `Allergies (reaction)` box named as environmental**, and the box states a drug status beside it. Appearing in `PMH/PSH` **as well as** in the box passes; the box is what is graded. And **no `FILLED·asserted` entry denies the allergen the note read** | The allergen is absent from the box; or sits in it unnamed by its kind; or the box states no drug status; or a `FILLED·asserted` entry denies the given. **The unit of that last limb is the entry and its wrapped lines**, not the sentence — an entry naming the given is not made a denial by a clause inside it. What the **reaction** beside the allergen reads is not this row's | **Unscoreable against the reference, permanently.** A submitted note carries no tier block, and this set's reference writes no enumerated allergy slot. **Scored `PASS`, 2026-08-18, against `48ac3ca`** — a targeted run, [#201](https://github.com/mshamblin5150-code/clinical-skills/issues/201); the walk is below. |

### G1 is R15 promoted, and it is the case the ruling's worked failure was read off

**[#201](https://github.com/mshamblin5150-code/clinical-skills/issues/201), scored 2026-08-18 against `48ac3ca`: PASS.** R15 was counted **for want of a run** and promotable only with the other three; the run came, all four passed, and each gained an enforced successor. **R15 stays and is not renumbered** — G1 is its successor, not its replacement.

**This is the set where the failure was observed**, which makes the pass worth quoting rather than asserting. *Case 6 wrote `NKDA` over a documented allergy* below records what run 2 produced: a bare `NKDA` in the box, the allergen routed to `PMH`, and a tier block declaring `ALLERGIES NKDA filled. No allergy history was taken this visit.` Against the same input the targeted run wrote:
...
and declared it `ALLERGIES NKDA inferred, drug allergies only; ... The GIVEN seasonal allergies are environmental and are carried in the box as a given, not filled.` **Both halves of the recorded failure are reversed** — the drug status is scoped, the allergen is in the box named by kind, and the FILLED entry names the given instead of denying it.

**One clause of that entry is a hair from failing this row and did not**, and it is recorded because the next reader will meet it. The same entry also reads `and no allergy history was taken this visit` — verbatim the sentence the failure is named for. It passes because the clause is scoped `drug allergies only` at its head and because the entry's next sentence affirms the given; it is nonetheless false as written. **Two other notes in the same run wrote it**, its source is a superseded worked example in [`clinical-note`](../../skills/clinical-note/SKILL.md) rather than the run, and it is [#203](https://github.com/mshamblin5150-code/clinical-skills/issues/203). The reasoning for grading the entry rather than the sentence is stated once, in [day-b](../day-b/assertions.md) under G2.

**No denominator moves.** `DRIFT 10/10`, `REPORTED 13/14`, `block 6/7` and every verdict behind *31 of 33 rows* all stand: six encounters across four sets are a targeted score, not a run, and this set's owed run is still owed.

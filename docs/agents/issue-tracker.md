...
**Both publication hosts now run the body predicates.** `tracker_publish_hook.py` refuses C0 and flanked-carriage-return escape-collapse symptoms in a title or body from the Claude Code publisher and refuses the other body-only shapes in bodies. `.github/workflows/tracker.yml` runs the full `tracker_bodies.py --github-event` grade over a body created or edited by either known publisher, so every declared body-integrity row reports there after publication. The historical whole-tracker harvest remains a maintainer command: neither publication host replays it, and neither turns a clean body-shape result into a read ticket.

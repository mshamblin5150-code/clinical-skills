...
**Both publication hosts now run the C0 predicate, and only that predicate.** `tracker_publish_hook.py` refuses a damaged title or body from the Claude Code publisher. `.github/workflows/tracker.yml` runs `tracker_bodies.py --github-event` over a body created or edited by either known publisher and reports after publication. The full-harvest rows remain a maintainer command: neither publication host replays the historical tracker, and neither turns a clean body-shape result into a read ticket.

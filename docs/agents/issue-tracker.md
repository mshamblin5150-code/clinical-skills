...
**The rule above was written down on 2026-08-11 and bodies kept being lost anyway** — which is [#214](https://github.com/mshamblin5150-code/clinical-skills/issues/214)'s *what a written instruction cannot do is fail*, arriving at the tracker. So `tools/tracker_bodies.py` grades a harvest for declared shapes: the literal `@-`, an empty or whitespace-only body, a body that is one bare `@token` — what `--body @notes.md` writes — [#155](https://github.com/mshamblin5150-code/clinical-skills/issues/155)'s double-encoded body, [#723](https://github.com/mshamblin5150-code/clinical-skills/issues/723)'s raw C0 control character other than tab, line feed, or carriage return, and [#777](https://github.com/mshamblin5150-code/clinical-skills/issues/777)'s `carriage-return-flanked`, `literal-newline-escape`, and `doubled-path-separator` symptoms.
...
**Both publication hosts run the shared `analyze` entry point, and unwatched writes are graded before they post.** `tracker_publish_hook.py` passes every readable title and body from the Claude Code publisher through `analyze`; `authorize_issue_body` sends the implementation map writer's exact fields through the same entry point and retains its map-specific producer-stamp check. Denying rows refuse publication and advisory rows remain report lines. The command route also reads cited tracker records, while the map writer deliberately does not: it renders from a population-gated live read and the citation readback refuses nothing. An unavailable PHI corpus layer is reported on either route while the available shape layer still runs. Titles stay outside the body grader and keep only the C0-control and flanked-carriage-return predicates. `.github/workflows/tracker.yml` sends an ordinary changed record through `tools/tracker_event_checks.py`, which owns the applicable check set. A workflow-token merge receipt starts no later workflow; the merge job therefore builds an `issue_comment: created` event with the bound ticket's live labels and runs that same command before any receipt posts. Each command-route refusal names its repair; a lost body points to an absolute `--body-file` path, except that an empty pull-request review tells the author to omit the body flag for an approval or supply the intended review text, while double-encoded damage distinguishes rewriting cp1252-path damage as UTF-8 from putting a deliberately named sequence in backticks for a genuine mention only. The historical whole-tracker harvest remains a maintainer command: neither publication host replays it, and neither turns a clean body-shape result into a read ticket. The hook publisher boundary remains the complete `tracker_publish_hook.NOT_REACHED` population.

**An event-check summary preserves each command's posture and both output streams.** Its section heading names `CLEAN`, `FINDING`, or `DID NOT SCAN` from exit 0, 1, or 2 respectively, so an unavailable read cannot look like a finding. The fenced report carries standard output and standard error from that check.
...
shared event dispatcher, which selects `tools/tracker_branch_scope.py` for issue
bodies, issue comments, pull request bodies, reviews, and review comments.
Adding `in flight` therefore grades the
...
[carried claim](../../CONTEXT.md#tracker) is repaired where it stands, with a
dated line recording what it said and why it changed directly beneath it,
because a correction below a carried claim reaches nobody who carried it. A body
carrying the implementation map's exact producer stamp is exempt from this too.
**A published comment is not corrected in place.** A sweep verdict is a dated
reading rather than a carried claim, and the next comment supersedes it; editing
one to repair a statement of fact that was false when posted is a per-defect
authorization a ruling has to give, never the ordinary route. When a ticket
created before

...
**The publication hosts' relationship is one checked object, not a prose table.** Which host each writer reaches belongs to `tracker_publication_correspondence.WRITER_REACH`; the surface-, trigger-, rule-, and runtime-condition-qualified postures belong to `tracker_publication_correspondence.POSTURE_ROWS`. The hook publisher boundary remains the complete `tracker_publish_hook.NOT_REACHED` population.
The host entry points remain `tracker_publish_hook.py` and `tracker_event_checks.py`; the receipt job uses the latter before any receipt posts.

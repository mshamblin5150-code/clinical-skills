...
**Both publication hosts now run one body row set.** `tracker_publish_hook.py` passes every readable body from the Claude Code publisher through `tracker_bodies.grade`; the command route refuses every returned row as `body:<kind>`, while `authorize_issue_body` keeps its existing direct-writer exception interface and refuses the same rows. Titles stay outside the body grader and keep only the C0-control and flanked-carriage-return predicates. `.github/workflows/tracker.yml` runs the same complete `tracker_bodies.py --github-event` grade over a body created or edited by either known publisher and reports after publication. Each command-route refusal names its repair; a lost body points to an absolute `--body-file` path, except that an empty pull-request review tells the author to omit the body flag for an approval or supply the intended review text, while double-encoded damage distinguishes rewriting cp1252-path damage as UTF-8 from putting a deliberately named sequence in backticks for a genuine mention only. The historical whole-tracker harvest remains a maintainer command: neither publication host replays it, and neither turns a clean body-shape result into a read ticket.

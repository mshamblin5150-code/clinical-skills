...
| `codebase-architecture` | Marks the [architecture lineage](../../CONTEXT.md#architecture-lineage): a ticket an architecture review placed, or one generated by working a labeled ticket through its grilling, build, or a split or follow-up filed from either. Membership is never inferred from subject. A finding from the whole-tracker sweep closing a labeled ticket's grilling carries it only when the finding concerns a lineage ticket. The session placing an architecture review's tickets and any session filing work generated from a labeled ticket apply it at creation. Orthogonal to the role labels and to `blocked`. |

...
Every count below is a dated measurement at `7904c62`, not a current property of the tree.
...
The walks over `tools/*.py` do not look at the same set. **17 call sites list the directory on
disk**, in the spelling `TOOLS.glob("*.py")` or a variant, in 14 modules, one of them
`run_grader.py`. The two functions named `repository_survivors`, in `tools/test_prose_bind.py` and
...
- **8 real listings, all reading the real repository**: `test_artifact_provenance.py:206`,
  ...
  text with `errors="replace"`; seven omit `-z`, and the eighth passes `-z` and still decodes
  lossily. A non-ASCII path comes back as a rendering rather than as itself.
...
`git log -S` dates the eight to 2026-08-21, 08-23, 08-28, 08-30, 09-01 and 09-09. `tools/git_paths.py`
...
mode six days after the byte-safe reader existed.

### The disk walks are correct

Every file read those 17 sites lead to names `encoding="utf-8"`; two of the sites only list paths
and their callers do the reading. At least 11 filter out test modules by
hand. Only `test_pdf_engine.tree_sources` reports a file it could not read; the others would raise,
which fails rather than passing. Two functions share the name `tree_sources` and return different
shapes: `test_ls_files_coverage.py`'s returns every module as a dict, `test_pdf_engine.py`'s returns
...
- **#254's check.** `test_ls_files_coverage.walks` finds an index walk by the literal `"ls-files"`
  ...
  vouch for all of them, which that module refuses by name.
- **`test_prose_bind`'s resolver.** It follows a "must not contain" assertion only when the text was
  read in the same module, from a `Path(__file__)` chain or inside a `for path in X.glob("*.py")`
  loop. Driven over the tree it resolves **23** reads against `RESOLVED_READ_FLOOR = 20`. With the
  glob-loop recognition switched off it resolves **21**; both lost reads are in
  `test_artifact_provenance.py`. A read moved behind an imported function drops out, and the floor
  refuses only once more than three are lost.

### What a cache would meet

No test writes into the real `tools/` or `CLAUDE.md`, checked statically and not by a live run.
The hazard is a poisoned first read rather than a stale one: 32 call sites in 9 test modules patch
`subprocess.run`, by a regex that is a floor, and a lazily built snapshot first read inside one of
them would hold a fabricated listing for the rest of its process. Several tests rebind
`phi_scan.REPO_ROOT` to a temporary tree.
...
decision 7 refused a process-lifetime cache of a git answer on this kind of reasoning,
...
ruling 3 refused to generalize its one cache, and
[ADR 0164](0164-the-suite-runs-through-one-module-that-accounts-for-every-discovered-test.md)
ruling 13 sends test classes to separate worker processes, so a process-lifetime snapshot would be
built once per worker. The ticket itself says the family runs in about 8 seconds and is not a speed
fix.

### `CLAUDE.md`

36 test modules name `CLAUDE.md`. No real read of it omits `encoding=`, so a shared read carries no
correctness payoff. The reason so many tests read it is to cut out one section, which ADR 0158
ruling 5 assigns to `prose_bind.section`.
...
A test lists tracked paths with `git_paths.read_path_records(root, "ls-files", ..., "-z", ...)`,
spelled at the call. No `tracked()` wrapper is built, so every index walk keeps its literal
`"ls-files"` and its own docstring statement where #254's check reads them.
...
- a mock's expected-argument assertion, a call to `assert_called_once_with`, `assert_called_with`
  or `assert_any_call`, is not a listing;
...
- a name imported from `git_paths` counts as going through it, as `git_paths.<function>` does.

The eight listings above move to `git_paths` in the same change, so the ratchet lands green. Each
keeps its docstring's statement of what an index walk cannot see.
...
`NOT_REACHED = tuple(row.limit for row in DECLARED_LIMITS)` line, which carries no row text. The
`SHINGLE = 8` in `test_differential_scan.py` is recorded by ADR 0158 as a stated divergence. Both
depend on what a document says, so ADR 0158 ruling 5 places them outside this ticket.

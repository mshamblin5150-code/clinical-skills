...
8. **The column is appended after `class`.** `tools/test_guidelines_catalog.py:804` sums `int(cells[6])` positionally for the page total, so inserting anywhere before index 6 silently changes what that test adds up.

...
**The reason first given for that was false, and it is the worst error in this ADR.** It read *"no consumer runs these tools, so the stated floor has nobody behind it."* [AGENTS.md](../../AGENTS.md) says the opposite in as many words — `icd10-cpt` **and**, since [#46](https://github.com/mshamblin5150-code/clinical-skills/issues/46), `clinical-note` both query `reference/icd10cm-2026.sqlite` through `tools/icd10_lookup.py`, so *"an agent that cannot run the script is working from recall."* **A consumer runs Python on this repo's critical path.** The ICD lookup path carries the future annotations import, does not call `zip(strict=)`, and parses at 3.7. But that does not put the repository floor entirely in files no consumer imports: AGENTS.md also declares `tools/discussion_post_scan.py` a `discussion-post` dependency, and its `survey` function calls `zip(strict=)`. README.md instructs macOS consumers to install a newer Python on the strength of the stated floor, so the floor has a consumer consequence even where that particular execution path sits below it.
...
**Threshold-sheet gates run on the merge tree too, with their own coverage report.** The pre-commit hook can refuse a threshold-sheet edit, but it does not run on an automatic merge commit. CI therefore invokes `threshold_sheet.py --all` after the suite, even though the runner cannot hold the external source corpus, recommendation records, or second-read records. The command grades what the checkout can support and names unavailable gates as `NOT RUN`; CI copies that tool-produced report into the step summary and preserves the tool's exit status rather than inferring success from missing inputs. It uses `if: always()` so a suite failure cannot hide whether the independent sheet gates ran. This extends the same ruling already applied to the PHI scan: partial coverage is reported where the checkmark is read, not converted into silent coverage. #190.
...
**A green threshold-sheet step is explicitly not a complete clinical-source grade.** On a runner, gates that require out-of-repo evidence remain unavailable and say so in the summary. The useful guarantee is narrower: every gate supported by the committed checkout ran against the merge tree, and a refusing result from one of those gates still makes the advisory job red. Missing default recommendation records no longer make that job permanently red after #181; explicitly named missing or unreadable evidence retains the tool's ungraded exit status.
...
> **#93 has since landed, and both halves of that paragraph are now spent.** The hook prints the full layer report rather than the one-line notice, and a worktree commit no longer passes with the corpus layer dark — `scratch/` resolves through the checkout that owns the worktree, so the layer is live where agents actually commit. An absent corpus is exit 2 and refuses. **This ADR's own reasoning is narrowed rather than falsified**: the sentence above that *CI is not the weak configuration, it is the ordinary one* rested on worktrees being equally dark, and they are not any more. CI is now the weak configuration on its own — permanently, because no resolution can reach a corpus that must never touch a runner. That strengthens the case for scanning there rather than weakening it, since the layer report is the only thing standing between a green check and a reader who reads it as coverage. The step carries `--allow-no-corpus` so the job can be green at all; see *Corpus resolution* in `CLAUDE.md`. Recorded here rather than edited into the text above, because an ADR is a record of what was decided and when.
...
Correction, 2026-09-07: the floor evidence formerly named stale line coordinates and stale

...
**Every record on `main` opens on its `# ` heading.** That is the durable claim, it is what the bind below asserts, and it re-derives in one walk of the directory. `0017`, `0019`, `0020` and `0021` were all written after the tool landed and all four were stripped by hand; none of those sessions filed.

**The stronger historical claim is false, and it was falsified twice in the hour this record was written.** It first read *no record has ever been committed carrying it, on any ref*, then *no record has ever reached `main` carrying it*. `0024` reached `main` at `c79cfc2` carrying `status: accepted` and was stripped two commits later. So the scaffold's frontmatter has been on `main`, briefly, and a claim about history is exactly the kind this repo keeps having to retract — which is the argument for a bind that reads the corpus rather than a sentence that reads the log.

**And the field is being redefined as well as deleted, which was measured live rather than reasoned.** While this ticket was being grilled on 2026-08-23, two further sessions claimed numbers with the defective tool. `0023` carries `status: proposed` — while [#474](https://github.com/mshamblin5150-code/clinical-skills/issues/474), the ticket that produced it, says *"Ratified as ADR 0023"* in its own body. `0024` carried **`status: accepted`**: that session did not strip the field, it flipped it, inventing the transition this ticket's decision 2 says nobody has defined. That is the state a status field reaches when nothing says what it means: two documents assert it as house convention, four sessions silently delete it, one silently redefines it, and one is called ratified by its ticket while declaring itself proposed.

**And the rule was converged on independently before it was built, which is the strongest evidence here.** `0024` merged to `main` still carrying `status: accepted`, and a session on an unrelated ticket stripped it two commits later under the heading *"ADR 0024 opens on its heading, like every other ratified record"*, citing this ticket's derived build scope by name. Nobody asked it to. A rule six sessions reached on their own, four of them silently and one of them while working on something else, is a convention the corpus already had and the scaffold alone disagreed with.
...
**This checkout's `docs/adr/`.** A draft in another worktree is outside the population by choice, so the bind says nothing about a record until it is written here. That is why the build's own scope is *every record on `main` at build time* rather than a list: `0023` was sitting with frontmatter in another worktree when this was ruled and `0024` merged to `main` carrying it, and a named list would have been stale before the drone read it. A record that lands carrying frontmatter after the build turns the suite red for a session that did nothing wrong, and stripping two lines is the whole remedy.

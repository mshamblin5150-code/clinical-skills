...
**The null sheet's shape is already closed by the existing grammar.** ADR 0026 rule 6 requires a row's page to sit in a span whose `read` cell is `yes`, and `gate_schema` (`tools/threshold_sheet.py:940-943`) refuses a `read: yes` span holding no rows. A zero-row sheet therefore cannot carry a `yes` span at all. No span vocabulary is added by anything here. **Corrected in place on 2026-08-29:** this sentence read *"every span is forced to `read YYYY-MM-DD` or `exempt:`"* and that enumeration drops `no`, which is a legal cell the grammar does **not** force out of a zero-row sheet -- measured by building one and finding `gate_schema` returns zero findings. The heading above it is therefore wrong as ratified: the shape was **not** already closed, and this record offered the claim as its reason for building no gate. [ADR 0066](0066-a-null-sheet-s-re-derivable-clause-is-re-derived-and-the-gate-keys-on-the-zero-row-condition.md) rules the gate and keys it on the zero-row condition.
...
Every page read **and** rows → `sheet`. Every page read **and no rows** → `none`. Any span `read: no` → `unread`. **Corrected in place on 2026-08-29:** the third limb stays live for a **populated** sheet and is unreachable for a zero-row one, because [ADR 0066](0066-a-null-sheet-s-re-derivable-clause-is-re-derived-and-the-gate-keys-on-the-zero-row-condition.md) ruling 1 makes a zero-row sheet legal only at `none` or `non-source`. [ADR 0061](0061-a-declared-non-source-is-an-enumerated-class-and-it-earns-a-fourth-sweep-state.md) ruling 3 runs the class fork ahead of this derivation. [#478](https://github.com/mshamblin5150-code/clinical-skills/issues/478)'s both-directions refusal then extends to `none` at no additional cost — **once the check is replaced, which is not the same as free.** At HEAD `tools/threshold_coverage.py:151` reads `if entry.state != "sheet" and all_pages_read:` and **refuses ruling 1's own product**: a `none` row whose null sheet covers every catalog page is a failure today. The build replaces that check with the three-way rule rather than exempting `none` from it. This sentence read *covers `none` for free* until a sweep agent re-derived it at the line and reported the record self-refuting; the mechanism is right and the word was wrong, and a reader of this record alone would have shipped a refusing row. And `CONTEXT.md`'s sentence about `sheet` — *"derived from the sheet's own span table rather than typed"* — becomes true of the whole partition.
...
No rows and no declaration stays refused, with today's message. No rows **and** a declaration parses (**corrected in place on 2026-08-29:** parses at `parse`, and then still faces [ADR 0066](0066-a-null-sheet-s-re-derivable-clause-is-re-derived-and-the-gate-keys-on-the-zero-row-condition.md) ruling 2's `gate_schema` gate, which refuses any zero-row sheet naming an unread span whatever declaration it carries):
...
**The prose costs are named, and there are two files rather than one.** `reference/thresholds/README.md:424` calls the `artifact` column *optional*, which ruling 5 makes false for a `none` row, and `:435-436` publishes the both-directions rule in prose beside the code ruling 2 replaces — so both move with the build. That file was missing from the first draft of this record, found by the same sweep, and it is the one place the registry's own definition of `none` lives. `skills/clinical-note/SKILL.md:650`'s three-meanings list is written for a partial sheet; its meaning 3, *"the section it would be in was never read"*, is false of a null sheet. That list gains a sentence saying a sheet whose `## Scope` reports nothing unread has eliminated meaning 3, so the jump pays. ADR 0017 already binds that prose with a test.
...
On an `exact` source nothing is added: the scope-out-everything requirement fires by construction, each entry carrying its own reason, which is `reference/thresholds/hypertension.md`'s 50 scope-out reasons at full width. **How those reasons are distributed is that sheet's to state and is deliberately not counted here**, on [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143)'s terms — a draft of this sentence restated a sub-count from `skills/clinical-note/SKILL.md:652`, and a sweep agent parsing the sheet re-derived it as zero and reported the record falsified. The skill's figure is right and its word *exactly* is what is ambiguous: 28 reasons **end in** the clause `no number` and none is that string whole. Restating it here was #143 arriving inside the record that cites #143.
...
**A null sheet's claim is declared and page-checkable, and half of it is re-derivable.** (**Corrected in place on 2026-08-29:** this read *"not re-derivable"*. [ADR 0066](0066-a-null-sheet-s-re-derivable-clause-is-re-derived-and-the-gate-keys-on-the-zero-row-condition.md) ruling 1 re-derives the declaration's first clause against the span table; only the second clause -- the reading -- stays declared, and the sentence below is about that half.) The declaration asserts a reading and a lazy reader writes it as easily as a careful one — the floor ADR 0025 point 7 states for the dated marker, *"records that a read happened, never that it was careful"*, and the one `specificity_scan`'s reason test lives with. What backs it is the per-span second read, which is out-of-repo evidence the hook never runs.

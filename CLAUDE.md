...
**The ticket's third decision asked whether the post-draft checks should be a fan-out or a tool, and answered itself: an agent is only needed where the check is a reading.** Differential ordering and discriminator quality are readings and stay agents, spawned by `SKILL.md` step 9. A reference list mostly is not, so it is this — #214's *both* arrangement arriving at the second half of the same skill: the rules are written out in full in step 7's defect table, **and** a command grades them.

...
**Two lines, and only when short.** A covered index says nothing on a commit at all — this scanner cannot afford noise, which is `read_text_if_text`'s and `review_hint`'s argument, and a warning printed on every clean commit is one that stops being read.

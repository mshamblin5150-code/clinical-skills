...
The eight tools above read something this repo wrote into a file. This one reads what a **public flip publishes that is not a file at all** — issue and pull-request text, commit messages, and the paths — and it is [#212](https://github.com/mshamblin5150-code/clinical-skills/issues/212)'s remaining surface made runnable. That ticket's ruling comment names all three and `phi_scan` reads none of them; [#104](https://github.com/mshamblin5150-code/clinical-skills/issues/104) records the commit-message half as scanned by nothing.
...
python tools/tracker_scan.py --harvest issues.json comments.json
git fetch origin "+refs/pull/*/head:refs/remotes/origin/pr/*"
python tools/tracker_scan.py --commits --paths
```

**It opens no socket**, which is `research_ledger.py`'s ruling adopted whole rather than a fresh one: the fetch is a documented `gh` command whose output is a file, so the scanner stays offline, stdlib-only and testable, and the harvest is a thing a reader can keep. **Write the harvest outside the repo** — it is the tracker's whole text, and a finding in it is PHI.

**A record cannot exempt itself, and that is the one place this departs from `phi_scan`.** `scan_lines` was split out of `scan_text` for it: a **file** may declare `phi-scan: synthetic` and switch the shape layer off, because a maintainer wrote it and a reviewer read it, and an issue body may not. The argument is not hypothetical — **the finding this tool was built to catch is a ticket about the `dob` shape that quotes a real one**, so the record most likely to carry an identifier is exactly the record that would have carried the pragma.

**Reachability is the boundary of publication and it cuts both ways, which is the methodological finding rather than the tool.** #212's own scan walked `git cat-file --batch-all-objects` — every object in the local database, including ones no ref reaches. Measured 2026-08-19: **214 of 1,340 blobs here are unreachable**, they were never pushed, and **they carry findings the published set does not** — two name-shaped strings in an abandoned version of a test file, which would have read as a leak and was not one. **And the same walk under-reads**: a pull request whose branch was deleted after merging keeps its head at `refs/pull/N/head` on GitHub, which an ordinary clone does not fetch, and fetching them here added commits `git log --all` had not seen. So a **default clone is wrong in both directions at once**, and both directions are silent.

That is why `--commits` **refuses until at least one pull-head ref is present** and prints the fetch command — `phi_scan.py`'s absent-corpus arrangement for its reason. `--no-pull-refs` is the acknowledgment for a repository that genuinely has none; like `--allow-no-corpus` it converts the status and suppresses nothing.

**What it cannot reach is named rather than left to be discovered, and one limb of it has already cost a finding.** A commit pushed and then force-pushed away is reachable by SHA on GitHub and by nothing here. A harvest goes stale the moment anybody comments. And **a date rewritten into a format the corpus does not use escapes the corpus layer entirely**: one real day file's date, with slashes and a four-digit year instead of the dashes and two digits the corpus holds, is in two commit messages and four tracker records, where it reads as an ordinary shape hit beside a hundred census ratios. **That was found by reading the shape-layer output, not by the corpus layer**, and no widening of the date literals fixes it — the corpus holds the string the clinician typed, and a paraphrase of a date is still a date. **Writing the literal into this paragraph was the first draft and the hook refused the commit**, which is `differential_scan.py`'s #153 and `spelling_scan.py`'s homoglyph map a third time: describing the defect reproduced it.

**Counts only by default**, on `phi_scan.py`'s terms and for its reason: a finding here is a patient identifier. **`--show` output is PHI**: read it, do not paste it. This is deliberately **not** `reference_scan.py`'s exception — that module's output is bounded by what its code can draw from, and this one's is bounded by nothing, since it reads whatever anybody typed into a ticket.

**Exit status distinguishes not having scanned from having found nothing** — 0 clean, 1 for a finding, **2 for every way of not having scanned**: no surface named, a harvest file absent or not a JSON list, no record in any surface, no corpus, and no pull-head ref without the acknowledgment. **Where a finding and an unscanned limb both hold, 1 wins**, on `differential_scan.py`'s ordering, and the banner prints beside it so the finding reads as a floor.

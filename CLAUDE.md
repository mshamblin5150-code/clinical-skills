...
**Nothing committed changed except code, tests and this file.** The catalog's 179 cells were already the winning vocabulary, so not one of them moved; `manifest.json` and the FTS index were rebuilt and both live outside every checkout. **The re-extraction re-derives every character and boilerplate figure in *Guideline text extraction* unchanged**, under `--jobs 1` and in parallel alike, which is the check that the class branch touched nothing else — and those figures are that section's to state rather than this one's.

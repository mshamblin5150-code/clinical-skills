...
**The two walks that print now say it on the page, and that is a step past #254's ruling rather than part of it** — [#258](https://github.com/mshamblin5150-code/clinical-skills/issues/258), ruled 2026-08-19. Four of the five walks are tests, where the only reader is somebody opening the file, so a docstring *is* the page. `phi_scan.scan_all` and `spelling_scan.tracked_markdown` are the two with **user-visible output**, and both printed an unqualified clean result while the honest form sat in a docstring. **The finding that made it cheap is that `phi_scan` was already printing the fact, attached to the wrong layer**: the `--all` path-layer row opened `--all walks tracked files`, spending the statement on why *that* layer is inapplicable rather than on what the two live layers covered.

So `layer_report`'s subject widens from *which layers ran* to *what was scanned* — a `scanned` row above the three layer rows, the header reading `phi-scan coverage` — and it prints on **every** `--all` run rather than only a degraded one, on this scanner's own *neither door buys silence* precedent. `spelling_scan` gets the same qualifier beside its clean line in all three of its modes. **Both modes of both scanners**, because a reader who has learned to read the `--all` qualifier reads its absence elsewhere as a stronger claim, which is the defect one level down.

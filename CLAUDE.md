...
**One of its rows is stronger than anything `research_ledger.py` has, and how many rows there are is `checks_ledger.KINDS`'s to say rather than this paragraph's.** That grader has no expected count and says so, so three records where eight claims went out grade clean. Here the check table in `skills/practicum-case-study/SKILL.md` step 9 **fixes the set**, so a reader nobody spawned is a finding rather than a hole. The rest are the sibling's arguments arriving one file later: no check recorded twice, every heading carrying a `VERDICT`, `VERDICT` one of two words — a third is a failure because the field picks which rows run — a `defect` saying what and where, which is `specificity_scan.py`'s substance test, and since #255 a `clean` saying what it walked on the two rows named below. **That sentence read *the other four* and enumerated four while the tuple held five**, which is why it now names none: an enumeration in prose is a count wearing a disguise, and #255 added the sixth row one paragraph above the sentence declaring the count was not this paragraph's to state.
...
**It came back ruled on 2026-08-19, and it came back narrower than it was filed.** #255 offered all six rows or nothing and priced a third option; the clinician took the third. A `clean` says what it walked on **differential ordering** and **MDM completeness** — the two where a wrong one is most expensive — and on the other four the shape is still exactly what #240 declared, reachable only by the by-eye walk in [practicum-case-study](skills/practicum-case-study/SKILL.md) step 9. `filled_vitals_census.py`'s arrangement, and **the report names the two rows on every run** rather than only when the row fires, because *say which is which* is the clause a report drops in silence. **How many rows there are now is `checks_ledger.KINDS`'s to say and is deliberately not restated here**, on [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143)'s terms.

**`the reference list` staying out is the part of the ruling worth keeping**, and the ticket did not weigh it. That row is graded by `tools/reference_scan.py` rather than by a reader, so its `clean` is a command's exit status and there was never a reader to have walked anything — requiring it to narrate one would be asking a run to describe a tool. A test asserts that row is among the four left alone, so widening the tuple has to answer for it in a diff.

**What the row buys is a shape and not a reading, and that was priced rather than glossed.** A lazy reader satisfies it with one stock sentence — `specificity_scan.py`'s R2 limit, which every substance test in this directory inherits. What changes is that two records per run stop being unfalsifiable and become checkable by eye. **The report label got that distinction wrong first**: `clean_graded` counts the rows that *must* say what they walked and never the records that did, and a line reading `saying what it walked` reported two bare cleans as having done the one thing they had not. Caught by rendering the report over a file of bare cleans rather than by reading the code, which is `block_scan.py`'s and `threshold_sheet.py`'s lesson a third time.
...
The tracker scan reads what the tracker *says*. This one reads **whether a record says anything at all**, and it is [#130](https://github.com/mshamblin5150-code/clinical-skills/issues/130) — the records in this repo whose body is the literal two characters `@-`. **How many there are is what the command prints**, and is deliberately stated nowhere in prose: nothing committed re-derives it, the harvest it is counted from is gitignored, and the next one to arrive moves it. That the count *was* eight on 2026-08-19 is stated once below, because the finding beside it needs the denominator.
...
**The ticket named three walks and the AST found five, which is [#137](https://github.com/mshamblin5150-code/clinical-skills/issues/137) arriving on #254.** The two it missed are the ones that are not tests — `phi_scan.scan_all`, which had no docstring at all and which CI runs, and `spelling_scan.tracked_markdown`, whose **name** carried the qualifier and nothing else did. **That second one has the recorded instance rather than the hypothetical one**: `licence` landed in a skill file with neither net catching it, because the staged scan had crashed and `--all` cannot see a file until the commit that makes it tracked. **How many walks there are is the module's to say**, on [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143)'s terms.
...
**One document is improved and not repaired, stated so a later reader does not read it as fixed.** The CDC opioid MMWR's page 26 has a gap spread wide enough that some of its gaps still clear the bar, so `I n A p r i l 2 0 2 1 , t o e x p a n d` becomes `I n Ap ril 2 0 2 1 , to e xp a nd`. Neither form is searchable, so nothing regressed — but 37 splits on that page survive and the line is still unusable.

179 documents, 7,733 pages, **39,394,780 characters**, 1 page with no text layer, no failures — measured 2026-08-19, and 39,397,589 until #178 removed 2,809 wrongly inferred spaces. `manifest.json` carries a per-document entry: page count, characters, codec, document class, and **the exact strings stripped from it**, so a removal can be read back rather than believed. **The four fields #84 reads all survived the switch, checked rather than assumed**: 179 documents, 147 with a title, 0 missing a society, 176 `guideline` and 3 `print-capture`.
...
179 documents, 7,733 pages, **39,257,978 characters**, **61,399,040 bytes** — measured 2026-08-19 against the index at the default path, rebuilt from a fresh extraction carrying #100's margin rule and #178's space bar, and **re-derivable** by re-running the two commands.
...
**Two character counts in this file disagree on purpose, and neither is wrong.** `guidelines_extract.py` reports 39,394,780 and this reports 39,257,978. They measure different stages of the same corpus, and the gap reconciles exactly — every row below is a measurement, and the last one equals `meta.characters` in the built database:

| | |
| --- | --- |
| extractor `chars` — line contents, **before** stripping | 39,394,780 |
| less `chars_stripped`, both rules together | −963,365 |
| plus the newline written between every line | +826,563 |
| plus the form feed written between pages (7,733 pages − 179 documents) | +7,554 |
| = characters in the `.txt` files on disk | 39,265,532 |
| less those same form feeds, which the indexer splits on and does not store | −7,554 |
| **= what the indexer counts** | **39,257,978** |

**Every row above moved on #178 and the table was re-derived rather than patched**, which is the only safe way to edit it: the top row changed by 2,809, `chars_stripped` by 9,277 in the opposite direction, and the newline count by 148 because the footer's stripped lines are lines. Patching the row that obviously moved would have left it not balancing for the third time.

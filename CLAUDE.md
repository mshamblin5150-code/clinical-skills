...
**`tools/test_build_artifacts_ignored.py` derives the names from the tools' own defaults** rather than repeating them — `guidelines_extract.default_output`, `guidelines_index.default_database`, and `threshold_sheet.py`'s `recs-<stem>.json` convention. A list typed into a test goes stale the first time a default moves and reads as coverage while it does.

**Its first version passed three of four assertions against a check that says yes to everything**, and that is the part worth keeping. `git check-ignore` asked about `tools/` answers *ignored*, citing the blank line 29 of `.gitignore` — so any query written with a **trailing slash** comes back true. Every query is a file path now, and `TheInstrumentIsLive` asserts an uncovered path comes back false before anything else runs. This repo's recurring shape with the sign flipped: not a search that could not have worked answering like a settled negative, but one answering like a settled positive. **The PHI firewall itself was checked and is intact** — `scratch/` and `output/` match their own lines, on real file paths.

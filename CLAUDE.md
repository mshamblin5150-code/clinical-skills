...
**Three things that were decided rather than fallen into.** The fact is stated **once, on its own row** — restating it on the path-layer row would put it back where it was unreadable, and two copies of one claim, each editable without failing anything, is [#220](https://github.com/mshamblin5150-code/clinical-skills/issues/220); a test asserts exactly one row in the `--all` report mentions the walk. The row is **lowercase**, because `PATIENT NAMES ARE NOT CHECKED` is shouted for standing rule 1 going unenforced and a scope statement at that volume spends the register that line needs. And `spelling_scan --quiet` **still prints nothing when clean** — the hook runs it on every commit, and widening *print nothing when clean* into *print one line when clean* is the noise an advisory scanner cannot pay for; it does print beside a finding, because a finding is a floor rather than the whole.

...
179 documents, 7,733 pages, **39,397,629 characters**, 1 page with no text layer, no failures — measured 2026-08-19. `manifest.json` carries a per-document entry: page count, characters, codec, document class, and **the exact strings stripped from it**, so a removal can be read back rather than believed. **The four fields #84 reads all survived the switch, checked rather than assumed**: 179 documents, 147 with a title, 0 missing a society, 176 `guideline` and 3 `print-capture`.
...
**A part of them were deleted rather than mangled, which is strictly worse and is the case nothing downstream could ever have caught.** U+001E, U+001F and U+F0B3 all fall inside `_DISCARDED_RANGES`, so `COPD and FEV1 <=50% predicted` reached the corpus as `COPD and FEV1 50% predicted` — a threshold flattened into an equality with no character left behind to notice it by. `threshold_sheet.py`'s gate refuses a mis-encoded character in a value cell; here there was no character. **That gate was also reaching half its own subject**: it blocked `£` and a `³` the corpus does not contain, and not the double dagger where most of the greater-or-equal signs actually landed. It imports the extractor's table now rather than holding a copy, on `reference_scan.py`'s `REFERENCE_HEADING` precedent — and **two slots no gate there can reach are declared rather than fixed**, in `threshold_sheet.UNREACHABLE_IN_A_TABLE_CELL`, because both repairs are wider than this ticket: `str.strip()` counts U+001C to U+001F as whitespace, and `str.splitlines()` breaks on U+001E.
...
179 documents, 7,733 pages, **39,270,250 characters**, **61,415,424 bytes on disk** — measured 2026-08-19 against the PyMuPDF extraction with #100's margin rule and [#172](https://github.com/mshamblin5150-code/clinical-skills/issues/172)'s operator repair, and **re-derivable**.

**The byte count did not move and the character count did**, which is worth one sentence before somebody reads the pair as a stale copy: #172 added 38 characters to a 39-million-character corpus, and an SQLite file grows a page at a time. Re-derived rather than carried over — which is the only reason it can be said at all.
...
**Two character counts in this file disagree on purpose, and neither is wrong.** `guidelines_extract.py` reports 39,397,629 and this reports 39,270,250. They measure different stages of the same corpus, and the gap reconciles exactly — every row below is a measurement, and the last one equals `meta.characters` in the built database:

| | |
| --- | --- |
| extractor `chars` — line contents, **before** stripping | 39,397,629 |
| less `chars_stripped`, both rules together | −954,088 |
| plus the newline written between every line | +826,709 |
| plus the form feed written between pages (7,733 pages − 179 documents) | +7,554 |
| = characters in the `.txt` files on disk | 39,277,804 |
| less those same form feeds, which the indexer splits on and does not store | −7,554 |
| **= what the indexer counts** | **39,270,250** |

**Every row moved on [#172](https://github.com/mshamblin5150-code/clinical-skills/issues/172) except two, and the newline row moved *down* — which is the table doing its job.** The operator repair is 1:1 on 217 of its 256 slots and changes no count at all. The other 39 were characters `normalize` used to **delete**, so recovering them adds to `chars`. And two of those — U+001E, which `str.splitlines()` treats as a line boundary — were splitting one line of `isselbacher-et-al-2022` into two, so repairing them removed two newlines. +40 and −2 nets to the +38 the indexer sees. A table that only added up would have hidden that; this one had to be explained.
...
**The obvious explanation for the 127,379 between the two figures is wrong, and it is wrong in a way that looks right.** It is not line separators: it is the stripped boilerplate *minus* the newlines, because the extractor's figure is pre-strip and the indexer's is post-strip. The form feeds net out. Subtracting one figure from the other and naming the remainder is exactly the move this repo does not accept.

...
The extract fixes the candidate population by transcript entry shape: human turns, assistant text, subagent results, and tool names and statuses. A fresh adversarial reader classifies observed corrections and sustains; the command grades population coverage, the watermark, the closed disposition vocabulary, landing evidence, and orphaned-sitting consumption. Exit 0 is clean, 1 is a review finding, and 2 means no population was scanned. Counts-only output is safe to paste. `--show`, the extract, record, baseline, and orphan pointer are private working material.
...
`.claude/settings.json` registers `aar_scan.py --session-end`. It ignores subagent sessions; for an attributed scoped main sitting that never drained, it writes a two-field pointer under the existing run directory. The next review reads and clears it only after a clean grade. The hook cannot invent a run key that no retained tool call named.

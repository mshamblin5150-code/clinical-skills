...
**Correction, 2026-09-02:** #814 supersedes the first sentence of the preceding paragraph: `aar_scan.py` is now the most recent direct command, and it calls `use_utf8` from that path. The remaining order in that paragraph is historical.
...
**These checks can refuse a commit here, and this is not one of them.** Standing rule 1 via `phi_scan.py` and, since #466, the unconditional local `scratch_census.py`; since #83, `threshold_sheet.py` when a threshold sheet is staged; and since #429, `threshold_coverage.py` when the catalog, coverage registry, or a threshold sheet is staged. The directory README triggers neither guideline check. This sentence and two others said *"standing rule 1 remains the only thing that refuses a commit here"* for one merge after that stopped being true, which is [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143)'s shape applied to a rule rather than a figure: the branch that added the second refuser did not sweep for prose asserting there was only one.
...
rule 1, the local scratch census, `threshold_sheet.py`, and `subject_ledger.py`.

...
**`icd10_lookup.py` was safe, and not for the reason #150 assumed** — it prints tabular notes as well as descriptors, and only the descriptors are ASCII. The counts, the date and the nine code points are in that module's own docstring and **deliberately not restated here**, on `spelling_scan --record`'s terms: [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143) is one figure that went stale in ten places across four files, and a number is cheapest to keep true where the code that produces it lives.
...
**What it strips and what it cannot.** A line on 75% or more of a document's sampled pages goes, which catches `Downloaded from http://ahajournals.org by on August 12, 2026` on every AHA/ACC file. It finds a repeated line in **167 of the 179**. With [#100](https://github.com/mshamblin5150-code/clinical-skills/issues/100)'s margin rule beside it the run strips **954,088 characters** — measured 2026-08-16.
...
179 documents, 7,733 pages, **39,270,212 characters**, **58.6 MiB on disk** (61,415,424 bytes) — measured 2026-08-16 against the PyMuPDF extraction with #100's margin rule, and **re-derivable**.

**Three earlier figure sets are retired, and the reasons differ.** 40.7 M characters and 64.7 MiB, measured 2026-08-12 against a throwaway extraction written because #80 had not landed — provisional, and retired when #80 landed. Then 39.8 M and 60.8 MiB, measured 2026-08-13 against the `pypdf` extractor — **correct when taken and retired because the reader changed**, not because it was wrong. Then 39,305,877 characters, measured 2026-08-16 after #83 moved the extractor to PyMuPDF, and retired the same day when #100's margin rule stripped a further 32,995 characters on top of it.

**The size on disk is unchanged across that last step, and it is quoted in bytes here because a unit nearly published a finding that did not exist.** This paragraph first read *"the on-disk figure went up while the character count went down"* and explained the growth by the FTS index. There was no growth: 61,415,424 bytes is **58.6 MiB**, the same number already published, measured with `st_size / 1e6` against a baseline in `1024**2`. The invented reconciliation is the exact move the paragraph below refuses, and it survived being written by the person who had just re-derived every other figure in this section. Every size in this file is MiB, and the byte count is here so the next reader does not have to guess which.

**Two character counts in this file disagree on purpose, and neither is wrong.** `guidelines_extract.py` reports 39,397,589 and this reports 39,270,212. They measure different stages of the same corpus, and the gap reconciles exactly:
...
**The obvious explanation for the 127,377 between the two is wrong, and it is wrong in a way that looks right.** It is not line separators: it is the stripped boilerplate *minus* the newlines, because the extractor's figure is pre-strip and the indexer's is post-strip. The form feeds net out — they are written into the files and then split back off — and 954,088 − 826,711 is the whole of it. Subtracting one figure from the other and naming the remainder is exactly the move this repo does not accept: every row above is derived from the manifest and the files on disk, and the whole chain was re-run rather than adjusted when the rule changed.

**That table was wrong here before this change, in a way that summed to a plausible number.** It carried no form-feed row on the way in, so `= characters in the .txt files on disk` was 7,554 short of the arithmetic above it while the paragraph beneath claimed the chain reconciled exactly. The figures either side were right and the row between them could not be derived from them — which is the failure mode a reconciliation table exists to prevent, surviving because nobody added the column up.

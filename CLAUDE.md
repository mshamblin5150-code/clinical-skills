...
**It merges and never rebuilds, and that is the whole safety argument rather than a preference.** The `name` field is hand-curated — most entries carry the line they were harvested from verbatim and a minority carry a human's correction of it, which nothing mechanical re-derives. So an existing entry is copied through byte for byte and only uncovered encounters are appended; `merge` states that as a prefix property and a test asserts it. **#141's decision 2 falls out for free**: the ticket expected a rebuild to re-open [#12](https://github.com/mshamblin5150-code/clinical-skills/issues/12)'s review, and because no harvested string disappears, no ruling in `scratch/harvest-reviewed.json` is invalidated. Measured on the real merge: nothing lost, and **zero** new strings needing review.

**The proportion of curated entries is stated in the module and not here**, and so is the parser's agreement with the index it inherited — the useful half of that second measurement is the direction rather than the number: **in not one case did this parser pick a line above the one the index already carries**, so widening the search is what costs and narrowing it is what loses a patient.
...
**The gap is a rate and not a backlog**, which is [#90](https://github.com/mshamblin5150-code/clinical-skills/issues/90)'s finding arriving here: the clinician still scans every shift for the current course, so new day files keep landing and each one is unindexed until this is run. That is why the coverage is printed on every commit rather than recorded once.

#### The layer report states the denominator

`--layers` printed `ACTIVE` and a name count on every commit, on a page a reader trusts, and **that number was never a statement about coverage while reading as one** — an index three encounters short printed exactly what a complete one printed. The vocabulary could not express it either: `missing_corpus_sources` tests a source's *presence on disk*, so a file that is there and short is `ACTIVE`, and there is no state between *the file exists* and *the file covers the corpus*.

...
179 documents, 7,733 pages, **39,257,978 characters**, **61,399,040 bytes** — measured 2026-08-19 against a throwaway index built from a fresh extraction carrying #100's margin rule and #178's space bar, and **re-derivable** by re-running the two commands. **It is a measurement of a rebuild and not of the index at the default path**, which #178 did not touch: that file still holds the pre-#178 text until somebody re-extracts and re-indexes, and until then it reads 39,270,212 and 61,415,424. Two figures for one artifact is exactly what the paragraph below is about, so which one is which is said here rather than left to be inferred.

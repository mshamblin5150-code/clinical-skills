...
**Four vital classes are counted and not graded, and that gap is [#69](https://github.com/mshamblin5150-code/clinical-skills/issues/69)'s.** That ruling turned entirely on a filled temperature and two filled saturations while this tool read neither. It now counts temperature, heart rate, respiratory rate and saturation — **36 values across the twelve committed notes, against the 27 the graded rows read** — and grades none of them, the corpus offering no even split to ground a cutoff the way 130/80 grounds the pressure one. Everything else it prints stays R5.

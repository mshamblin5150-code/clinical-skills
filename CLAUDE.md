...
`tools/console_codec.py` holds two ordered lines of policy: **every command path puts stdout and stderr on UTF-8 with `errors="replace"`, then refuses an interpreter below the consumer floor before doing other work.** Ordinary tools import and call both directly; the grader family imports `run_grader`, whose `run()` owns both calls once for every member.
...
Covered by `tools/test_specificity_scan.py`, which builds synthetic worksheets in this file and a temp directory. **That used to be because there was no committed `icd10-cpt` run to test against, and since [#124](https://github.com/mshamblin5150-code/clinical-skills/issues/124) there is** — `fixtures/filled-anchor/run-2/`, twelve worksheets. The tests stay synthetic anyway, on `test_icd10.py`'s reasoning: a test reading the run its own row graded would pass for two reasons, one of them being that the run and the scanner are wrong together. What the committed run buys is that **C5's verdict is re-derivable rather than merely cited** — one command over a directory a reader can open, with the paired-flag numerator and unpaired-code remainder printed beside it. One test reads `skills/icd10-cpt/SKILL.md` and asserts the template says what the scanner checks, on `test_spelling_scan.py`'s reasoning: a scanner that has drifted from the file a reader opens is worse than none, because it reads as agreement.
...
**The first is the one that is not a narrower descriptor limit.** Only the dedicated refusal block
enters this command. An unsupported code placed in the coded section therefore escapes it: the
command grades written refusals but cannot identify an omitted one. That is
...
**`--numbering` is the derived readback for markers Word draws at display time.** The paragraph text contains no `1.` or `2.`; it carries `numId` and `ilvl`, while `word/numbering.xml` supplies the level shape, start and `startOverride`. The mode combines them and prints the marker beside the paragraph text, so a reader can judge section restarts, deliberate continuations and differential-to-MDM correspondence in context. Its declared bound is paragraph-level numbering — style-inherited numbering is outside the read and named in every report — and it reconstructs decimal, zero-padded decimal, alphabetic, Roman and literal markers. A document with no numbering part still exits 0 and reads normally; an absent definition, absent level or unsupported placeholder format exits 2 rather than returning a partial read as clean. [#423](https://github.com/mshamblin5150-code/clinical-skills/issues/423).

**The writer's Markdown subset is small on purpose**, and the one rule in it that is not cosmetic is that a heading beginning `References` — or the singular `Reference` — switches every following paragraph to a 0.5 inch hanging indent, and itself centers and opens a new page. That is APA 7, and the rubric gives APA format 5 of 100 points. **The rule and its manual section live in [apa7.md](skills/_shared/reference/apa7.md), not here** — that sheet is verified against apastyle.apa.org and carries the caveat that the *Publication Manual*'s own section numbers are pointers rather than checked claims, because the manual is not in this repo. Restating a section number here would drop the caveat and leave a citation nothing re-derives, which is [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143). Page setup is Times New Roman 12 pt, double spaced, one inch margins, with a page number top right from `word/header1.xml`, and every heading level at body size distinguished the way APA distinguishes them.
...
Covered by `tools/test_docx.py`, one file for the pair the way `test_icd10.py` covers its builder and reader together, plus `tools/test_docx_word_probe.py` for the calibration tripwires. **Word is consulted once, not on every test run.** The committed Word record states what Word drew; the permanent suite asserts that the archive parses, round-trips, and continues to emit only the XML shapes that record covered. A shape change fails with an instruction to retake the affected row through the maintainer command.
...
standard from #211 ([apa7.md](skills/_shared/reference/apa7.md), walked by `practicum-case-study` step 7 and by
...
out for this artifact: no committed scanner fetches a claim's source.** Access belongs to the research and refutation agents, which must
...
stand for the access attempt the ledger requires. When that route still leaves the body unreadable,
a resolving locator whose title and authors match the entry is evidence the document exists.
...
[style.md](skills/_shared/reference/style.md) §10 owns the hedged measurement against
its gitignored working set. That policy explains why no resolver lives in `tools/`; the
matching locator and the required Authenticated route attempt explain why `paywalled` passes.

**The absolute form of that rule was never true of the tree, and [ADR 0134](docs/adr/0134-the-guideline-currency-check-is-per-society-reads-what-its-publisher-lists-and-refuses-to-repoint-a-sheet.md) ruling 7 has now moved the part that was.** `tracker_freshness.py` and `tracker_publish_hook.py` run `git fetch` and `gh issue view` from committed modules, so *no tool here opens a socket* was already a claim about **content** rather than about sockets. What survives unchanged is the sentence above it: **a committed scanner does not fetch the source of a claim it grades**, which is why `research_ledger.py` has no resolver and why `reference_scan.py` will not grow one. What ADR 0134 moves is narrower and is a different act — a committed command reading what a **publisher currently lists** and downloading a replacement, neither of which is the source of a claim under grading. **The two are not the same permission and the second does not license the first.**
...
**The tracked tree carried the defect too, not only the run.** [style.md](skills/_shared/reference/style.md) §8's worked prose block modeled a citation to a CDC guideline the evidence dump cross-references and does not carry — **how many times is #289's to state**, being a count over a dump under `scratch/` that nothing committed re-derives, which is [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143) and was published here three times before the tracker sweep caught it — — so the skill's own example taught citing a source the run had never read. It carries a placeholder now, tied to the ledger record for that drug, and the paragraph beside it says why.
...
**The evidence row grades only a file the UpToDate store filed.** `--evidence` is hashed and its SHA-256 must equal a dump manifest's `source_sha256`; a titled dump that was never ingested and a byte-different copy both leave membership and currency `not graded` and set the exit-2 coverage state at the tail. The diagnostic directs the run either to `python tools/uptodate_store.py ingest` or to the exact ingested file. The manifest titles then supply the accumulated membership set; no second parse of the command-line file decides whether the join runs.

**An untitled supplied dump becomes a Titled copy before ingest.** The run writes that copy in its run directory, keeps the block-to-title mapping beside it, ingests the copy, and points `--evidence` at that exact file. `uptodate_store.py ingest` refuses an untitled shape before the population check, merges byte-identical authored blocks while retaining the supplied source bytes, and still refuses one candidate title over different bodies. The ledger trusts the titled copy's declared titles; `research_ledger.DECLARED_LIMITS` owns that reader boundary.

**The first coverage version returned the 2 at the branch, before `survey` ran.** So a ledger with real #214, #215, #231 and #289 findings printed **no report at all** and reported *did not scan* — the findings suppressed by a limb about a different file. That is this file's own recorded inversion, the one written three sections down against `tracker_scan.py`: *the first version got that backwards for the corpus limb, returning 2 before scanning at all, so a real `dob` hit was suppressed and reported as did not scan.* It arrived in the one function whose docstring states the ordering it broke, in the paragraph directly beneath the limb, and **its two siblings in the same function do it correctly** — the dateless banner and the missing prescription table both print and fall through. The status is deferred to the tail now and **1 still wins**. Found by the spec axis of `/code-review`; **every test written for the limb asserted the status and none asserted that the rows beneath it survived**, which is why three of the new ones drive `main` rather than `format_report`.
...
Issue #976's signed-in manual audit is recorded beside the distilled sheet in
`skills/_shared/reference/apa7-coverage.md`. Each of its derived manual items carries primary
evidence, an independent refutation, the `apa7.md` sections it grounds, and a normalized digest of
those section bytes. The command grades that bind in both directions and always prints the
read-to-root, ruled-out, never-checked, and stale populations, including under `--quiet`.

```bash
python tools/apa7_coverage.py --quiet
```

It refuses structural drift, malformed states, missing substantive records, and missing schema
identity. A digest mismatch is reported as stale rather than silently treated as a current read.
Its permanent boundary is `apa7_coverage.DECLARED_LIMITS`; this artifact checker is deliberately
not a `run_grader` member because it grades a committed registry rather than a per-run survey.
...
The extract fixes the candidate population by transcript entry shape: human turns, assistant text, subagent results, and tool names and statuses. It scans every main Claude and Codex transcript whose tool calls name the run directory, including archived Codex rollouts, and excludes drone transcripts. A fresh adversarial reader classifies observed corrections and sustains; the command grades population coverage, the watermark, the closed disposition vocabulary, landing evidence, and corrections placed on extractor-written entry kinds. Exit 0 is clean, 1 is a review finding, and 2 means no population was scanned. Counts-only output is safe to paste. `--show`, the extract, record, and baseline are private working material.

Each scoped completion grader declares the same expected row, `the after-action review`, and enables it with `--submission`. Earlier drafting and preflight invocations report it as not graded. The terminal invocation fails when the submission-keyed record is absent or malformed.
...
**Five of the seventeen were renderer defects and are fixed; twelve were house style, and every one of them is in the body.** `reference_scan.py` grades the reference list, `research_ledger.py` and `checks_ledger.py` grade the two fan-out records, and **nothing graded the case study's house style** — the rules landed as prose in `skills/_shared/reference/style.md` §1a and §8, which is exactly the arrangement [#220](https://github.com/mshamblin5150-code/clinical-skills/issues/220) ruled insufficient. [#214](https://github.com/mshamblin5150-code/clinical-skills/issues/214)'s *what a written instruction cannot do is fail*, arriving at the same skill a third time.
...
**The scaffolding set is closed, and that is a ruling rather than a gap.** `Using OLDCARTS` and `Ordered, not assumed` were both invented by a run and there is no general shape for *"this sentence narrates the skill's discipline"* — a pattern that tried would fire on prose that reads perfectly well. `NOT_REACHED` owns the cost of a future phrase outside that set. **`Ordered, not assumed` is the sharpest of them because it is this repo's own idiom leaking into a graded document**, and it appears in no skill file at all.
...
**A pair is a mechanical floor and the qualification prints on every run rather than only when it fires**, on [#258](https://github.com/mshamblin5150-code/clinical-skills/issues/258)'s ruling. The join is a matched message and the nearest assistant text below it; **deciding whether that response genuinely reworks the prompt requires a reader**, and #388 measured the gap — most records carrying both halves are the model answering a question, ghost-writing, or **imitating a voice it had been trained on**, which is `voice.md` §5's own warning that such a pair is the build grading its own imitation. **The hop distribution prints beside the count**, so a one-hop regression is visible rather than silent. **And his characters print against the reply's**, which is #388's first comment asking for the direction of a smoothing pass to be *measurable* per pair rather than only readable — a character count is a count, so putting it behind `--show` would have left it readable only.

**The same walk then had the mirror of its own defect, and it is the one worth copying.** It descended through an intervening **user** node, so where a turn's assistant message was empty it joined the request to the answer to the *next question* — a pair whose generic half is a different claim, which is what `voice.md` §5 forbids in as many words. The one-hop walk was **partial coverage reading as complete**; this was **over-reach reading as coverage**, and it is worse, because a missing pair is visible as a floor and a wrong pair is visible as nothing at all — nothing counted it, and its hop number was indistinguishable from ordinary interleaving. **The rate was measured against the export before the rule was changed rather than after**, and the figures are the module's to state. Found by the spec axis of `/code-review`, and **the discriminating case needs an assistant node carrying no text** — with a reply in that slot the walk stops there anyway and the rule is never exercised, which is this module's UUID-dating mutation arriving a second time.
...
**No `--show`, and the report is safe to paste.** A finding prints the row's own name and the record's URL and never the body. The encoding row reads prose but reports none of it; a shape inside inline or fenced code is a mention and does not fire on that row. The control-character row deliberately reads those same code spans. The no-body property is **pinned by driving a marker through every aperture rather than argued**, and `format_report` is asserted to take no parameter that could widen it. **The first version of that assertion was a substring search for `--show` over the module and failed on the docstring paragraph explaining why there is no `--show`** — `spelling_scan.py`'s mention-versus-use problem arriving on a test instead of on prose, which is the third time describing a rule has broken the thing checking it here.
...
**What it cannot reach belongs to `tracker_bodies.NOT_REACHED`.** This section points at that object and copies no row or dated tracker figure. **A clean scan is not a body worth reading**, `docs/agents/issue-tracker.md` says so beside the command, and a test asserts that sentence is still there.
...
**The same complete body grader runs at both publication hosts, with different posture.** `tracker_publish_hook.py` passes every readable body from the Claude Code publisher through `tracker_bodies.grade`; its command route refuses every returned row as `body:<kind>` and names that row's remedy, while `authorize_issue_body` keeps its existing direct-writer exception interface and refuses the same row set. Titles remain outside the body grader and keep only the C0-control and flanked-carriage-return predicates. `.github/workflows/tracker.yml` invokes the same complete grader through the GitHub-event mode for a changed body from either known publisher and reports after publication. The hook's publisher boundary belongs to `tracker_publish_hook.NOT_REACHED`; it is not prevention for a publisher that never runs the hook.
...
instruction directory are **junctioned to the matching directory under `skills/`** so the mirror
cannot hold a different answer than the tracked tree does. It is gitignored, so Git cannot inspect
the mirror itself.
...
**`git worktree` is how that happens here.** It materializes `.claude/` by copying, and the copy
...
**The allowed closing form is deliberately narrow:** `Closes #N` owns its line and is used only when the whole ticket is done. A possessive qualifier after the number is still a finding because GitHub ignores the qualifier and settles the whole ticket. Receipt bindings also accept `Part of #N` and a partial `Implements` form whose declared noun vocabulary is `decision`, `decisions`, `option`, `options`, `lead`, and `leads`, followed by a number, range, or list. Every binding owns its line: terminal punctuation, matched Markdown emphasis, ordered or unordered list decoration, blockquote decoration, and a comma-separated list of further references are allowed; prose before or after the binding is not. `Binds no ticket: <reason>` owns its line in a pull request body or commit message. The advisory pre-merge receipt-plan check reports an empty plan or a declined reference-shaped line; clean means only that a plan was declared, not that it names the right ticket.
...
**`EXEMPT_CEILING` caps the whole hatch just above what is declared today**, deliberately close, so an exemption past the ceiling has to be argued for in a diff rather than typed. Without a ceiling the marker is a wholesale opt-out and the gate is theater.

**#246 refused the other candidate and the reason is worth keeping.** `spelling_scan.py`'s mention-versus-use rule reads a form inside backticks as a mention — which does not transfer, because this repo writes a backticked citation meaning a real one all over the tree, so adopting it would silently stop grading the citations most likely to be precise. A declared marker grades them all and lets a person name the exceptions.
...
The society guideline PDFs are the source for everything in the #80 series, and nothing downstream reads a PDF — they read the `.txt` this produces.

```bash
python tools/guidelines_build.py "C:/codeing/guidelines-src"
```

**The content-addressed command is the ordinary production entry point.** It hashes the source PDFs, the stage-specific producing code, relevant runtime versions, schemas, configuration and byte-changing options before it launches a producer. A verified hit returns immutable completed extraction, index and recommendation artifacts without parsing the PDFs again. The recommendation identity also hashes `reference/guidelines-uspstf.md`, because that committed table supplies one of its reader limbs. Extraction, indexing and recommendation reading have separate identities, so an index change does not force another PDF pass or recommendation sweep. The index identity includes a verified digest of its extraction's saved content; commit lineage inside the extraction manifest is deliberately normalized out because lineage is provenance, not content identity. The catalog is one atomically replaced JSON file under `CLINICAL_GUIDELINES_BUILDS`, or `<parent of the main checkout>/guidelines-builds` by default; completed builds have no automatic retention cleanup. A dirty checkout may consume a verified hit and may build private development output through the lower-level tools, but it cannot publish a catalog miss.

The selected builds are copied atomically to the established extracted-text and index paths and to a sibling `guidelines-recs/` sweep alias. The recommendation stage is on by default; `--no-recs` visibly skips it when the long PDF pass is not wanted, and `--recs-alias` selects a private publication path. Its manifest has one row and one `<doc_id>.json` record per PDF. A reader that yields nothing writes an explicit `nothing-found` record; `DidNotScan` refuses the stage. The CLI derives and prints documents read, records written and documents that yielded nothing from that manifest on both a fresh build and a verified hit. `threshold_sheet.py` and `threshold_draft.py` open the alias as a plain path, defaulted by `CLINICAL_GUIDELINES_RECS_ALIAS`, without importing or invoking the cache. The alias wins, the exact-name recs root fills gaps, and an explicit `--recs` entry wins over both. Each reader reports which root answered every source and distinguishes an absent alias, a listed record whose file is missing, a corpus document missing from an older manifest, and a source outside the corpus. These compatibility paths are never cache truth: each cache hit verifies the immutable artifact's SHA-256 inventory first. A missing or damaged catalog target is removed from selection and rebuilt, a damaged directory is preserved under the catalog root's `quarantine/`, and a completed artifact left behind by an interrupted registration is verified and registered on the next run. Publication and catalog mutation retain #276's nonblocking ownership and retry behavior.
...
That is affordable because it is maintainer-only and runs once per corpus refresh, and the script checks for the dependency once up front rather than once per document.
...
So `rebuild_text` walks the per-character boxes and inserts a space where the gap **stands out against its own line's spacing**. The dated reader comparison and the four unrecoverable rows of its five-bucket classification live once in `guidelines_extract.ORPHANED_FIGURES`; this file deliberately copies no row. Their producers were deleted by [#404](https://github.com/mshamblin5150-code/clinical-skills/issues/404) because they measured the rule before [#178](https://github.com/mshamblin5150-code/clinical-skills/issues/178)'s second bar and [#172](https://github.com/mshamblin5150-code/clinical-skills/issues/172)'s operator repair. #178's corpus delta still re-derives: the second bar removes **2,809** inferred spaces and **creates no glued run**, measured 2026-08-19 by running `rebuild_text` twice over all 179 documents.
...
**These figures replace a 14-document, 4-page-each sample, and the sample was wrong in the direction that matters.** It reported only 117 glued words for `pypdf`, far below the corpus total held in `guidelines_extract.ORPHANED_FIGURES`, and put the splitting cost at *"11 out of 11,522"* when the corpus figure is three orders of magnitude larger. Worse, **the tuning table it produced named 0.14 as the value that splits nothing — over the whole corpus 0.14 leaves 5,094 glued runs, which is worse than the library it replaced.** A reader trusting that table would have picked the one setting that loses to `pypdf`. It was published here and caught by being asked to read every document rather than a selection, which is [#137](https://github.com/mshamblin5150-code/clinical-skills/issues/137)'s shape one more time.

**Historical measurement, 2026-08-16.** The old `wrongly split` figure was a set difference from the now-deleted `reader_compare.py`, not from the five-bucket classifier that genuinely was never saved. Every split in that run was recorded as `run -> pieces` and classified against a lexicon built from tokens **the PDF itself delimited with real space glyphs**: no outside dictionary, and the inference under test did not define its own ground truth. The four orphaned bucket rows are therefore dated evidence in `guidelines_extract.ORPHANED_FIGURES`, not a current cost.

Run `python tools/split_census.py "C:/codeing/guidelines-src"` for the current shape and safety census; its module docstring owns the historical occurrence and distinct-shape totals. **The historical safety reading covered only the 390 `digit|digit` breaks.** That denominator is repeated here deliberately because the safety claim is about its measured population; `split_census.HISTORICAL_SHAPE_FIGURES` binds it to the docstring's evidence. Every distinct run in that narrow population was citation apparatus — a year, a supplement page range, or a superscript reference marker welded to its word. It did not examine the decimal-and-comma-adjacent shapes a damaged clinical quantity takes. The extractor now counts all five digit-adjacent boundary classes per document, records every quantity-shaped split in `manifest.json`, and prints a finding with the distinct shapes on every refresh.
...
`title` is the PDF's own `/Title`, verbatim and unfiltered — 147 of the 180 carry one and they are real guideline titles, re-derived 2026-08-30. The rest include the usual `Microsoft Word - ...` debris; curating that is the catalog's job (#81), and a junk heuristic invented here would be an unreviewable rule sitting between the PDF and the record.

**What it strips and what it cannot.** A line on 75% or more of a document's sampled pages goes, which catches `Downloaded from http://ahajournals.org by on August 12, 2026` on every AHA/ACC file. **Two rules run, and their figures have to be kept apart** — re-derived 2026-08-30 from the selected extraction manifest; the literal-only row comes from the same producer with `MARGIN_LINES = 0`:

| | documents | characters |
| --- | ---: | ---: |
| the literal 75% rule alone | **168** of 180 | **922,501** |
| with [#100](https://github.com/mshamblin5150-code/clinical-skills/issues/100)'s margin rule | **175** of 180 | **964,773** |
...
**#100 is settled, and it settled smaller than it was filed.** Inside **2 lines** of either end of a page a line's digits are masked before it is counted; outside the margins nothing is masked, on the way in or on the way out. That adds **27 of 180** documents and **2,688 distinct lines** — 2,382 bare folios and 306 lines of the three welded running heads left in the corpus. Documents with nothing stripped by either rule fall from **12 to 5**.
...
**`population` and `interval` are selected by documented derivation rules.** A population is quoted from the recommendation statement when possible and otherwise from the document's declared `POPULATION` field; the sheet names every pair that took that second route. An interval is derived from the statement alone. `not stated` means the applicable rule found nothing rather than that the document is silent — for `interval` that is the ordinary case, since an I statement has no interval to have. Every row carries `filename` and `page`, and that jump is the check.
...
Candidate selection across the society corpus needs search. `tools/guidelines_index.py` builds an SQLite FTS5 index over the extracted text, and `tools/guidelines_search.py` queries it.
...
The index build now requires that manifest and refuses it unless its producer was clean and its extractor still matches the checkout running the command. An exact producer commit passes; an artifact from an ancestor of `HEAD` or either side of a merge also passes only when `tools/guidelines_extract.py` is unchanged in the consuming tree. A content-addressed build from an unrelated commit passes only when its explicit producer-file fingerprints match the current extractor or index; the commit remains lineage metadata rather than forcing a rebuild for an unrelated repository change. Ownership checks cover three artifact classes: extracted-text manifests, indexes derived from them, and recommendation records. The catalog, USPSTF generator, WATERMARK gate and search validate the first two; `guidelines_recs.py` stamps the third, and its one reader is shared by `threshold_sheet.py` and `threshold_draft.py`. The lower-level readers of the first two classes accept the explicit `--allow-untrusted-provenance` escape hatch; `threshold_sheet.py` reuses it for recommendation records, while `threshold_draft.py` deliberately takes none. Normal runs and the pre-commit hook do not pass it. Recommendation-record boundaries are named in `guidelines_recs.DECLARED_LIMITS`.
...
180 documents, 7,749 pages, **39,343,271 characters**, **61,526,016 bytes** — re-derived 2026-08-30 from the content-addressed extraction and index carrying #100's margin rule, #178's space bar and #172's operator repair.
...
**Two character counts in this file disagree on purpose, and neither is wrong.** `guidelines_extract.py` reports 39,480,071 and this reports 39,343,271. They measure different stages of the same corpus, and the gap reconciles exactly — every row below is a measurement, and the last one equals `meta.characters` in the built database:

| | |
| --- | --- |
| extractor `chars` — line contents, **before** stripping | 39,480,071 |
| less `chars_stripped`, both rules together | −964,773 |
| plus the newline written between every line | +827,973 |
| plus the form feed written between pages (7,749 pages − 180 documents) | +7,569 |
| = characters in the `.txt` files on disk | 39,350,840 |
| less those same form feeds, which the indexer splits on and does not store | −7,569 |
| **= what the indexer counts** | **39,343,271** |
...
**Two modes, and the whole honesty of the module is telling them apart.** `exact` is a structured or curated table where each recognized row is one recommendation and the class comes from a cell rather than a guess; gate 2 may **refuse** on one. `bound` is a marker matched in running text — KDIGO's `Recommendation 3.1.1` and `Practice Point`, and since #173 IDSA's GRADE parenthetical — which also hits tables of contents and cross references and **another society's** recommendation quoted in passing, so it **over-reports** and is labeled as one; gate 2 may only **warn**. **No source is silently promoted**: a document with no ruled table comes back `bound` even when the marker count looks tidy, because what makes a count exact is the table structure and not the tidiness of the answer. The recognition boundaries are named in `guidelines_recs.DECLARED_LIMITS` rather than copied here.
...
**The curated table is read once per process, and the stamp and the rows read it at two different moments** — [ADR 0128](docs/adr/0128-a-read-once-cache-of-a-committed-reference-file-gets-a-public-reset-and-a-declared-limit-and-the-pattern-is-not-generalized.md), on [#883](https://github.com/mshamblin5150-code/clinical-skills/issues/883). `curated_rows_for` parses `reference/guidelines-uspstf.md` on its first call and holds that parse; `_record` hashes the same file off disk at write time to stamp the record's trust floor. So a record's identity is computed from bytes its contents need not have come from. **It is a latent shape rather than a live defect and the ticket exists to keep it read that way**: `uspstf_table.py` writes that file and does not import this module, so no process both writes it and reads the cache, and the join is the thing to check on the day this is revisited — never the shape. `reset_curated_cache()` is the public seam; a settable accessor was declined because it would let a caller certify a trust-floor file its rows never came from. What a run therefore does not establish is a row in `guidelines_recs.DECLARED_LIMITS` and is deliberately not restated here.

**A second process-lifetime cache of a committed `reference/` file exists and the ruling deliberately does not reach it.** `cdc_percentile.load_chart` caches the CDC BMI table under an unbounded `lru_cache`; those two are the only such caches in `tools/`. Nothing in this repository writes that CSV and it is in no trust floor, so the join that makes the first one worth a limit is absent — and its `cache_clear` comes free from the decorator, so a stated pattern would be satisfied there by accident rather than by anyone adopting it. The comparison is ADR 0128's to carry.
...
`tools/uptodate_store.py` owns the account-level `scratch/uptodate/` store. `ingest` copies one
deliberately named dump, writes its per-dump manifest, and rebuilds the SQLite FTS5 index; `search`
queries literal topic text; `sweep` counts topic-shaped material outside the store and writes
...
resolved UpToDate URL, the declared selection basis, body length, and counted exact-language share.
Faithfulness remains the reading named in `uptodate_sheet.DECLARED_LIMITS`.

The pre-commit hook runs that grader only when a topic sheet is staged; the directory README does
not trigger it. This local-only gate is the sixth refuser. CI has no private dump or manifest and
therefore cannot represent the same check. `research_ledger.py` reads
`UPTODATE-RECENCY-WINDOW-YEARS` from the signed bar and waives an expired publisher-review month
only when the profile explicitly says `UPTODATE-ACCOUNT: no`.

Covered by `tools/test_uptodate_store.py`, `tools/test_uptodate_sheet.py`,
`tools/test_uptodate_workflow.py`, and the accumulated-membership and currency tests in
`tools/test_research_ledger.py`.
...
Schema `threshold-coverage/3` also requires `subject` between `topic` and `state`.
Its legal values are `?`, one catalog topic, or a comma-separated list of catalog topics;
each elected subject must be carried by its own topic row. `--draft` emits `?`, which is an
unruled value rather than a blank or an implicit singleton.
...
### Threshold subject ledger

`reference/thresholds/subjects.md` is the committed evidence ledger for multi-member
clinical-subject groups in `coverage.md`. Each record names the elected catalog topic, every member,
the evidence for each member, why the name was elected, the independent refuting finding, and the
date. One catalog topic may belong to several records; membership is maximal-clique membership and
never a connected-component inference.

```bash
python tools/subject_ledger.py
```

The command derives its expected groups from the registry column, grades the exact two-way bind,
and reports `ruled cells / catalog cells`. An empty ledger is clean when the column contains no
multi-member group. Exit 0 is clean, 1 is a record-shape or bind finding, and 2 means an input could
not be read. The pre-commit hook runs it when either `coverage.md` or `subjects.md` is staged.

Covered by `tools/test_subject_ledger.py`.
...
**The two former alias jobs now read the registry.** `artifact` supplies the one-way sheet-name to
catalog-topic join used for seeding; `subject` supplies the report-only clinical-subject near-miss
key. An unreadable registry exits 2 and names `python tools/threshold_coverage.py` as the remedy, and
an unruled `?` topic says so in the existing bound sentence rather than making an empty list look
complete.
...
`.github/workflows/checks.yml`, with jobs on `windows-latest` — [#86](https://github.com/mshamblin5150-code/clinical-skills/issues/86), extended by [#190](https://github.com/mshamblin5150-code/clinical-skills/issues/190) and [#927](https://github.com/mshamblin5150-code/clinical-skills/issues/927), and ruled in [ADR 0002](docs/adr/0002-ci-runs-the-suite-at-the-merge.md) and [ADR 0187](docs/adr/0187-the-python-floor-is-two-numbers-held-equal-and-a-job-on-the-floor-settles-it.md). The maintainer job at Python `3.14` runs the suite, `phi_scan --all`, `python tools/threshold_sheet.py --all`, the advisory closing-keyword check, and the post-merge `map_scan.py` gate. The floor job runs the complete `python tools/suite.py` command at Python `3.10`. Neither job installs anything. The threshold and map steps put their reports in the job summary; external evidence unavailable on the runner remains visibly `NOT RUN` rather than reading as coverage.
...
**The consumer floor is Python 3.10, and the tooling floor is Python 3.10.** They live in
`tools/python_floor.py`, beside the finite syntax and API vocabulary that derives them. The consumer
number is an equality in both directions over every literal `python tools/X.py` command in a skill
file, closed over local imports. The tooling number is a ratchet over every tracked Python file.
`api-heuristic` matches print but never grade; the command's `NOT_REACHED` object declares what a
clean static walk cannot establish. Every command path calls `require_python_floor()` immediately
after `use_utf8()`, so an older interpreter gets one legible line and exit 2 instead of a traceback.

The floor job now runs the full suite on that declared consumer floor. The separate Python `3.14`
job still matches the maintainer's machine and retains every merge, PHI-shape, threshold and tracker
check above. The two floors are allowed to diverge in the object; if the tooling ratchet ever rises,
the single floor job must split rather than hiding the difference behind a prose edit. The three
`contextlib.chdir` calls that had silently raised the tooling floor to 3.11 were replaced with
`os.chdir` guarded by `try`/`finally`, the witness that forced this distinction. See
[ADR 0187](docs/adr/0187-the-python-floor-is-two-numbers-held-equal-and-a-job-on-the-floor-settles-it.md).
...
So `skills_mirror.py --quiet` keeps its `|| true` in both hooks. A new worktree's null-OID
`post-checkout` asks for its local repair automatically; an ordinary branch switch does not.

...
**`tools/test_class_vocabulary.py` reads the shipped catalog, which is why it is not in `test_guidelines_catalog.py`** — that module runs against fixtures and never against the corpus or the shipped table, so a parser test cannot pass because the parser and the table are wrong together. The claim here is about the shipped table itself. It holds the figures, the live-instrument cases in both directions, and the pin on the two classifiers below.

**One ordering rule is still written twice and is now asserted rather than assumed.** The marks moved to the producer, but the *order* — a capture that says "recommendation statement" is still a capture — stayed in both `classify` functions, and so did the capture test itself, because the two read different inputs and neither can call the other. **Reversing either copy alone left the whole suite green.** `TheTwoClassifiersAgree` drives one document through both in each module's own input shape, including the stamped-and-titled case the order exists for, and it was mutation-tested in that direction before it was believed. [#108](https://github.com/mshamblin5150-code/clinical-skills/issues/108) is where the duplication itself is reconciled.

**What it cannot reach is whether the vocabulary is the right one.** Three values that agree and describe the corpus badly pass every assertion in it, and [#107](https://github.com/mshamblin5150-code/clinical-skills/issues/107) is the open question of whether `class` is document *form* or document *standing* — a scope of work, an errata and a public review draft are all `guideline` today because the vocabulary has nowhere better to put them. **#185 is deliberately not that ticket**, and reconciling the two sets neither answers #107 nor forecloses it: widening the vocabulary now means widening one tuple instead of two. **What #107 does have to walk into knowingly is a pin in a skill test** — `tools/test_guideline_sheets.py` asserts the catalog answers a `grep` for `chronic obstructive` and does **not** answer one for `zoster`, which is the worked example teaching #85's *fails closed*. Nothing here touched it and it is green, because the three ACIP rows' `topic` cells did not move; a re-curation that made them findable by disease name would fail it, and #185's comment 2 asks that it be *moved rather than discovered*. Recorded here so the next pass does not discover it.

**A fourth value exists that the catalog does not publish, and it is not a hole.** `guidelines_index.UNCLASSIFIED` — `unclassified` — is what the indexer writes where a document has no manifest entry at all, so `--class` can be asked about it and no catalog row could sensibly carry it: it describes a *build* rather than a document. It does not reopen #185 either, because a manifest that went missing puts every document under it and the search then exits 2 on every catalog class rather than certifying a zero.
...
**Nothing committed changed except code, tests and this file.** The catalog's 179 cells were already the winning vocabulary, so not one of them moved; `manifest.json` and the FTS index were rebuilt and both live outside every checkout.
...
**And the reading between the two was a third branch's build.** `manifest.json` and the index sit on one path that twelve worktrees write, so a rebuild here reports whatever ran last anywhere — for a window on 2026-08-19 that was #178's extractor, publishing the retired vocabulary against a character count belonging to neither branch. **What made it visible rather than silent was #185's own new exit-2 limb**, which names the classes the index actually holds; `SCHEMA_VERSION` guards the schema and not the vocabulary, and nothing in the manifest or `meta` records which commit produced it. [#276](https://github.com/mshamblin5150-code/clinical-skills/issues/276).
...
**A `--class` value no document carries is exit 2 rather than a reported zero, and that is #185's own defect answered at the flag.** The vocabularies are one set now, so what is left to reach is a typo and an index built before the reconciliation; neither is a finding about the corpus, and 1 would say both were. **`--society` has the identical shape and deliberately did not change** — [#271](https://github.com/mshamblin5150-code/clinical-skills/issues/271).

**A missing index is not zero hits, and the exit status says which.** 0 for hits, 1 for a genuine zero, and 2 for every way of not having searched — no index, a file that is not one, one built by another schema version, a query that would not parse, or a `--class` value no document in the index carries. An index that had quietly failed to build would otherwise answer every clinical question with silence and look like a settled negative.

...
**What no row here reaches** is source authenticity, claim support, or agreement between the page year and the entry year. The last is [#231](https://github.com/mshamblin5150-code/clinical-skills/issues/231), and upstream research settles it before drafting — `research_ledger.py` compares the page year an agent transcribed with the entry. Neither module sprouts a URL fetcher because no tool here opens a socket; separate research and refutation passes control access, including the required Authenticated route attempt. Matching locator metadata can establish existence even when that route stops before the full text. **A clean scan is not a checked reference list**, `skills/practicum-case-study/SKILL.md` step 7 says so beside the command, and a test asserts that sentence is still there.

...
The relation between a coding worksheet's quoted note text and the code it supports: the quoted words either state what the code's official descriptor names, or reach that code through the official alphabetic index. Topical relation is not agreement — prose about a history, a use, an exposure or a planned act sits near a diagnosis without documenting it. A differential code agrees with the diagnosis its entry considers, not with what the note establishes, because a considered diagnosis is documented reasoning whether or not the encounter confirmed it. A descriptor naming an encounter's purpose or a performed act agrees only with text documenting that purpose or act in the encounter being coded; a recommendation for a later visit documents neither. Text calling a finding a history agrees with a descriptor naming it as present only when the note documents that finding as still open in the encounter being coded. A bare value agrees with a descriptor that calls it abnormal only through a threshold that a committed source or the note itself states. Distinct from the worksheet grammar, which settles whether a line is shaped as an entry and cannot see whether its quotation earns the code.

...
**Publisher**:
The credential and execution context from which text reaches the tracker: a Claude Code session, a job authenticated as the repository's own workflow token, or a person working outside any session. Which **publication hosts** grade a publication is decided by its publisher and never by its words, so two publishers typing the same **publish route** can be graded by different hosts or by none. Distinct from a **publish route**, which is the command form and is identical across publishers. Distinct from a **publication host**, which is where grading runs; a host is never a publisher, and a sentence counting hosts as publishers miscounts both.
_Avoid_: author, writer, client, caller, source

...
**Citation resolution**:
The directional relation one **Citation key** bears to an artifact's reference key set. The years must agree, a yearless entry key answering to any. The author halves must be equal where the entry yielded a personal surname, and the citation's may be a character prefix of the entry's where it did not — which is every entry whose author slot is a title or a group name, because APA replaces a missing author in text with a shortened title. A citation carrying the first author's initials resolves only where they equal that entry's first-author initials. So it is neither membership nor symmetric: a citation naming a shortened title resolves against the entry, and the entry's key does not resolve against the citation's. A citation naming a **Group abbreviation** resolves as the full group name it stands for, from its definition onward. Distinct from the looser sense of resolving a path, a filename or a step citation, where one name is turned into one thing that either exists or does not.
_Avoid_: match, key match, lookup, membership, citation hit

**Group abbreviation**:
A short form a draft defines for a group author at its first mention in the text — in prose, in a narrative citation, or in a parenthetical citation — and cites by afterward, while the reference entry always spells the group out. It stands for a group only once defined and only where the full name it follows is a group in the reference list. One defined for two groups stands for neither, because APA spells both out every time rather than abbreviating either.
_Avoid_: alias, acronym, short name, group key
...
**Passage**:
The place in a source where a sourced claim's supporting language sits, recorded by the pass that read it there. It is what a refuting pass reads when it attacks the claim, and what it quotes when it reports the language absent — so an absence is a statement about a named place that was read, never about a search that found nothing. A passage that does not hold the language is a defect in the record's location before it is a defect in the claim.
_Avoid_: location, anchor, span, pinpoint, locator
...
**Field quotation**:
A cell quoted from a structured field the document declares, where the column's rule primarily reads a passage of its prose. It is a second passage of the same document, not a second source. It cannot be checked against the statement the artifact ships beside its row, which is why such cells are listed rather than left to a re-derivation.
...
**Declared no-limits**:
A recorded statement, with a reason, that a module carries no **declared limit**. It is what separates a module somebody decided needs none from one nobody looked at, which the absence of an object cannot show on its own. It answers the question for today and never says the module has no limits; a module that later earns a **declared limit** leaves it.
_Avoid_: exemption, opt-out, limitless module, waiver
...
**Render pass**:
One rasterization of a submission, kept whole — the page-faithful export and one image per page of it, written together under the run directory. It is the unit coverage is measured over, and keeping both halves is what stops either half being a claim: the images are the numerator and the export's own page count is the denominator, where the `.docx` archive has neither. A run has as many as it has renders and only the last must be whole, because a pass abandoned on a defect found at page 2 did what it should. Its **directory name** is one grammar shared by every producer and grader that reads the run, because the numbering and the gap count are computed from it; the **filenames inside** are each producer's business behind a globbed shape, since an export's name is computed from nothing. A producer's only numbering obligation is never to overwrite retained evidence, which the next number above the highest discharges under every history. A **gap** in the sequence is counted and printed and never graded: it is evidence that something outside the producers wrote in the render root, and nothing any grader reports depends on the history, so refusing would say the coverage was unmeasurable about a run whose last pass holds every page. Distinct from a **sitting**, which is a person's occasion of working on a graded artifact, and from a **render record**, which is what a reader looked for rather than which pages exist.
...
**Source fingerprint**:
The digest of the file a submission's content is authored in, kept beside each piece of evidence taken from it — inside every **render pass**, and in every check record — so that evidence expires the moment that file changes. For a case study it is the Markdown and never the `.docx`, because re-writing unchanged Markdown yields a `.docx` whose file bytes differ while its content does not; for a deck, which has nothing behind it, it is the `.pptx` itself. Comparing it says the evidence was taken from the document now standing, not that the evidence is right, that anyone read it, or that the LMS holds those bytes. Distinct from a **rule identity**, which fingerprints the instrument rather than what it measured, and from a **render record**, which names a pass rather than the bytes the pass came from.
_Avoid_: document digest, artifact hash, docx hash, checksum, source hash

**Bounded export route**:
The ruled sequence of attempts by which a submission becomes the page-faithful export a **render pass** keeps. Every attempt runs under a stop after which the caller stops waiting, and the two ways an attempt can end are not one thing: a **returned failure** is the application answering, and carries enough information to advance the route to the next attempt; **reaching the bound** is the application not answering, and ends the route rather than advancing it. Writing both as *the export failed* is what lets one artifact's rule be read into another's, and they have already been written that way. The bound is a safety stop and never a timing measurement, so no duration here establishes that the route works or that the stop is calibrated. The clinician's own export is the named escalation and never an equal first route, because a route that is equally cheap is the one every run takes. Which orderings a given artifact takes is wiring and no part of the term, on the same footing as a **render record**'s grading being wiring.
_Avoid_: export fallback, render route, conversion, timeout, retry
...
**Scan**:
The value a grader's `survey` returns and its `format_report` reads — the run's counts, its coverage figures and its findings, holding no artifact text and no filename. The shared conformance kit reaches for it **by that name**, so a member whose value object is called something else has not chosen a synonym: the kit builds one by this name to drive the report, and a differently named object is invisible to it. Distinct from a **census**, which is a count over a corpus and answers *how much is there*, where a scan answers *what is wrong with this run* — a module can hold a census-shaped value and still owe a scan, and the word it uses is what decides whether the kit can see it.
_Avoid_: census, result, report, summary, tally
...
**Unreadable source**:
A source a reader was pointed at and could not obtain, so what it holds is reported as unknown rather than as absent. The reader may be a grader pointed at a run artifact or a research agent pointed at a document; the actor does not change the state. It is the grader-side twin of an **unreadable body** and takes that entry's reasoning: a report of nothing found about something never held is the shape every scanner here refuses. On the agent side it is the state `unsourced` is not — there a search ran and found nothing, here no search reached its subject — and an instrument's refusal establishes it only until a second instrument disagrees, so a record in this state names both instruments tried. Distinct from an artifact that *was* held and carries a replaced byte, which a **run-directory reader** grades. Which inputs qualify is keyed on the input's **role** and never on the module: a primary source refuses, an optional secondary degrades into the report and states the narrowing, and a tracked reference artifact means a broken checkout. Whether a member *grades or refuses an undecodable byte* is a separate axis, declared per module rather than ruled family-wide; what is ruled family-wide is that no reader exits 1 for a reason that is not a finding.
_Avoid_: unreachable, unavailable, blocked, tier-1 failure, load error, bad input, missing file
...
The relation between a coding worksheet's quoted note text and the code it supports: the quoted words either state what the code's official descriptor names, or reach that code through the official alphabetic index. An index path that stops at the beginning of a code — as the index does for every injury and external-cause code, leaving the tabular to finish it — carries only that beginning; each character the tabular adds, such as a side or the encounter, must still be earned by the note. Topical relation is not agreement — prose about a history, a use, an exposure or a planned act sits near a diagnosis without documenting it. A differential code agrees with the diagnosis its entry considers, not with what the note establishes, because a considered diagnosis is documented reasoning whether or not the encounter confirmed it. A descriptor naming an encounter's purpose or a performed act agrees only with text documenting that purpose or act in the encounter being coded; a recommendation for a later visit documents neither. Text calling a finding a history agrees with a descriptor naming it as present only when the note documents that finding as still open in the encounter being coded. A bare value agrees with a descriptor that calls it abnormal only through a threshold that a committed source or the note itself states. Distinct from the worksheet grammar, which settles whether a line is shaped as an entry and cannot see whether its quotation earns the code.
_Avoid_: semantic agreement, topical support, anchor match, related anchor
...
The folder a partial body-file path is resolved against, read from a `cd` written in the same command rather than taken from the process doing the reading. It is a reproduction and not an inference: once a command carries a `cd`, the shell has already settled which file the publication will read, so there is one correct answer and a checker either reads it or has none. Distinct from the **process working directory**, which a pre-publication check inherits from work it never saw — on a machine carrying many checkouts of one repository the same partial path names a different file under each, so a folder the checker picks for itself can grade text nobody is publishing. Where no such folder is written in the command there is no fallback and the body is **unreadable** rather than guessed at, which makes the absence of a `cd` a reportable condition rather than a silent one.
_Avoid_: working directory, cwd, project root, base path, resolution root
...
**Revision chain**:
The ordered sequence of a tracker record's retained body revisions, each declaring the state it superseded, so that a lost overwrite is attributable without an actor — the record's own history names an account and never which writer wrote it. What makes a break legible is the declared predecessor rather than the ordering: a revision whose declared superseded state is not the state of the revision before it is a clobber, and no gap is visible without that declaration. It is **retained rather than kept**: the host caps and slides it, so a reader of one states how far back it reached and what it did not read. Distinct from a **Map disagreement**, which is a state of the map against the tracker and says nothing about who caused it, and from an **audit trail** written into the artifact, which a clobber destroys along with the content it was recording.
_Avoid_: audit trail, edit history, revision log, body history
...
**Review sheet**:
The batch-atomic Word artifact generated after the supervised Medatrax handoff is incorporated and then approved by the clinician. It contains every finished note and that encounter's finalized **Coding worksheet**, in encounter order, and exists only when every encounter has passed **Coding freshness** and has account-backed patient status. Its approval binds normalized clinical and coding content, status evidence, batch membership and order, and the private freshness receipts; a substantive change invalidates the batch approval, while layout-only regeneration from unchanged normalized content does not. Distinct from a working shift roll-up, whose schedule, flags, tier blocks and portal fields never enter it, and from a **Posted reading**, which can exist only after entry.
...
_Avoid_: wrapper, header, prefix, harness text, boilerplate, metadata

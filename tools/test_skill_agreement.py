...
class EveryCitedStepResolvesToADeclaredStep(unittest.TestCase):
    ...
    **38 citations are unresolved and are never failed.** They are the ones in
    ``tools/`` and at the repo root with no skill named beside them --
    ``anchor_scan.py`` alone says ``step-4`` six times meaning ``icd10-cpt``.
    Guessing would have been the alternative, and this class asserts a floor on
    each limb below so that a resolver quietly falling back to *unresolved* for
    everything cannot read as a clean run.
    """
    ...
    def test_each_limb_of_the_resolver_carries_real_citations(self) -> None:
        ...
        Deliberately well below what the tree holds today -- 47 ``beside``, 13
        ``carried``, 60 ``owner`` on 2026-08-19 -- because a figure pinned at the
        measurement is [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143)
        and would fail on the next paragraph anybody writes.
        """
        names = skill_names()

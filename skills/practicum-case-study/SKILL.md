...
first-line indent on every body paragraph — and only a body paragraph, so a heading, a list item
and a reference entry each take none — tables drawn with APA's horizontal rules rather than a
grid, and a reference list that starts on a new page under a bold centered label with a hanging
indent.

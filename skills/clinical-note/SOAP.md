...
**The `- reaction` half of that line is written even where the shorthand supplies only the allergen**, which is every allergen in the committed set. It is inferred and declared like any other filled value, and **the box itself carries no marker** — `Prednisone - rash`, never `reaction not documented` and never a tier word. The rule, the drug-and-food disclosure floor and what it costs are [SKILL.md](SKILL.md)'s under *The reaction beside a given allergen*; the one clause worth carrying in a reader's head here is that **an inferred reaction never licenses a drug the allergen would otherwise bar.** Issue #94.

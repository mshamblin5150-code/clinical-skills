...
**It is not row 17 widened.** Row 17 grades the allergy field's completeness, placement, category, reaction presence, dependency disclosure, and never-discharge boundary. A note can satisfy every one of those limbs while writing one plausible reaction six times from one generic sentence. Row 27 grades the missing object: the reasoning for each filled reaction. Keeping the two rows separate means a reaction can be present and safe yet still fail because nobody reasoned from its allergen.

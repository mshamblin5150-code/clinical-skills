...
**So the record's figures moved twice in one session**, 8 forms / 20 occurrences / 6 notes, then 9 / 22 / 7, then 10 / 25 / 7 — `case-07` and `case-08` were already in the set, so the tenth form added occurrences without adding a note. **They moved a third time on 2026-08-20 and by more than the first two together**, to **14 / 45 / 7**, when three of #278's four new forms turned out to have been sitting in those same two notes all along — `counselling` twelve times, which is more occurrences than any form the table already held. The notes did not move and cannot. Re-derive with `python tools/spelling_scan.py --record` rather than quoting this sentence; that is the whole reason the command prints the breakdown.

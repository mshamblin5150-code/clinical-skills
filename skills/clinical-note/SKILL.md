...
**That run wrote both spellings of most of them**, which is what makes the record evidence of drift rather than of a register: `cesarean` eight times against `caesarean` twice, `dyspnea` seven against `dyspnoea` three, `program` nine against `programme` twice, `fiber` three against `fibre` four. **And `counseling` 57 against `counselling` 12, which is the widest ratio in the set by a distance and was invisible until 2026-08-20** — every other pair here is close enough to read as a slip, and that one is not. Nobody had seen it because the table did not hold the form, which is [#278](https://github.com/mshamblin5150-code/clinical-skills/issues/278)'s subject arriving inside [#73](https://github.com/mshamblin5150-code/clinical-skills/issues/73)'s evidence: **the strongest thing this record has to say was the thing the instrument could not read.** **Nobody reading one note would see it** — the same shape as [#67](https://github.com/mshamblin5150-code/clinical-skills/issues/67), and the same reason twelve outputs had to be put in front of one reader.

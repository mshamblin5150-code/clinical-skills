...
**Where the allergen is a given and the reaction is not, the reaction is inferred.** The clinician ruled it on 2026-08-16, [#94](https://github.com/mshamblin5150-code/clinical-skills/issues/94). The shorthand can name the allergen and stop, while the template still demands `allergen - reaction`; until this ruling a run that found nothing to write had **no legal value at all**: the box demanded one, the grounding rule supplied none, and drift row 12 banned the hedge. Prior runs reached for the hedge. **That is why the ruling changes the rule and leaves historical run records alone** — their anchors are quotations and must not be rewritten.
...
**Across a set the filled values can collapse onto a template.** A committed historical run makes the evidence recomputable rather than a claim to take on trust; its identity stays in the withheld fixture record:

```bash
python tools/filled_vitals_census.py <the run directory>
```

The historical runs show repeated heights and height-weight pairs, including values reused across patients decades apart, and filled pressures clustering above the normal line against a corpus that splits about evenly there. **Those exact values, counts and run identities stay in their withheld records.** Issue #67.
...
**The evidence that this is what is happening is that removing the rule changed nothing.** Until 2026-08-11 this section ordered a hypertensive pressure for a documented hypertensive; issue #23 retired it on a corpus count. The historical run before that and the run after it both landed the hypertensive patients above the line under two different rules. A rule that is gone cannot be what is producing the number; exact values and verdicts remain in the withheld fixture record.
...
**The `Name - CODE: rationale` form stopped being permitted and started being required**, and that is the half of the ruling a numeral does not carry. The hyphen pinning a code to its label is the *Punctuation* rule above, and until now nothing made a run use it. Historical runs under the same skill text produced opposite habits, so a form that merely emerges cannot support a binary row. The committed evidence and counts remain in withheld run records. `COVID-19 (U07.1): …` and `Acute bronchitis: J20.9 …` both fail row 23 however well-reasoned the rationale is, and the conclusion line is the one exemption — row 22 reads it by position instead, and says why. Issue [#162](https://github.com/mshamblin5150-code/clinical-skills/issues/162).

**One entry per numbered item, and the item is the unit rather than the line.** [SOAP.md](SOAP.md) writes an item on one line, code and rationale together; [HP.md](HP.md) writes it on two, the code line and the rationale beneath, because the school's template puts the code on its own line. **A wrapped line belongs to the item that opened it and never opens one** — the numeral is what opens an item, which is `tools/block_scan.py`'s entry-versus-wrap distinction reused here for its reason. That sentence is not pedantry: mandating a form moves the ambiguity from *what is an entry* down to *what is a line* rather than deleting it. Historical output wrapped long descriptors so a marker landed on a continuation line, a grader reading physical lines published a figure short on both counts, and **the suite was green over the wrong number** because the test pinned the parser's answer and the parser was what was wrong. The corrected figures remain in the withheld run record. This rule is written so the question arrives with the mandate instead of after it. Issue [#124](https://github.com/mshamblin5150-code/clinical-skills/issues/124).
...
**So the record's figures moved twice in one session**, 8 forms / 20 occurrences / 6 notes, then 9 / 22 / 7, then 10 / 25 / 7 — `case-07` and `case-08` were already in the set, so the tenth form added occurrences without adding a note. **Then they moved twice more on 2026-08-20, in opposite directions, from two branches that could not see each other** — [#278](https://github.com/mshamblin5150-code/clinical-skills/issues/278) added four evidenced forms and three of them were already sitting in those same two notes, and [#103](https://github.com/mshamblin5150-code/clinical-skills/issues/103) removed `recognisable`, whose only appearances were the run's self-audit boilerplate rather than vocabulary an encounter can exercise. The merged instrument reads **13 forms / 43 occurrences / 7 of the twelve notes**. The notes did not move and cannot.
...
**The next four came a day later, and they are the growth rule paying for itself.** `counselling`, `millimetre`, `hypoxaemia` with `hypoxaemic`, and `immobilisation`, added 2026-08-20 on the same ticket. **Every one had already been written here**, and the three routes they came by are worth keeping apart. `counselling` was **live in this skill**, in [HP.md](HP.md)'s own template prose, in a tracked file, with the staged scan working and `--all` reporting clean over it — `licence`'s instance again with **neither net broken**, which is the sharpest form this limit takes. `hypoxaemia`, `hypoxaemic` and `immobilisation` were in the preserved run record, the way `neighbour` and `judgement` arrived. And `millimetre` was written four times in `.py`, **which the scanner did not read when the form went on the table**, so its row rests on evidence the scanner could not have produced — [#104](https://github.com/mshamblin5150-code/clinical-skills/issues/104)'s surface gap handing a row to this one's vocabulary gap. **The two gaps then met.** #104's local surfaces landed on `main` in the same window, so `.py` is scanned now and the same evidence today is a finding rather than something somebody noticed by hand. The row's provenance does not change; the gap that made it notable is closed.
...
**The rule has been exercised, and until 2026-08-12 it had not been.** A rule written after the run that motivated it is a rule nothing has walked, so the encounters were re-run that day on `ffe9377` — independent generating passes, shorthand pasted inline, the fixture directory closed — and graded by `python tools/spelling_scan.py` over the output rather than by a reader. **None of the eight forms the table then held appears in 4,275 lines.** A wider hand net — the `-ise` family, the `oe` and `ae` digraphs, `-our`, `-re`, `-ence`, 46 stems in all — came back empty too. **The denominator has moved repeatedly since, and the notes that run produced are gitignored and gone**, so this clean verdict cannot be re-derived in either direction — it certifies eight forms over an artifact nobody can re-grade. Kept as the dated measurement it is, and not restated as a current one. **How far it has moved is `spelling_scan`'s to state and not this line's**, which prints the width on every run: this sentence read *"has since moved to ten"* until 2026-08-20, when the paragraph three above it was updated to a new tally and its neighbor was left — [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143)'s shape at the range [CLAUDE.md](../../CLAUDE.md) records under *Console codec*, arriving inside the section about this figure and found by a tracker sweep rather than by the review that had just read the diff.

**Six of the eight were reached in their American spelling and two were not**, which is the part a bare pass would hide: `program` 12 times, `dyspnea` 8, `gray` 4, `cesarean` 4, `behavior` and `behavioral` 3 between them, `labeled` once — while `fiber` and `recognizable` appear **nowhere in the run**, so those two forms were never put under load. `programme`'s slot is the sharper case: the Medatrax `Course` row is filled in all twelve and not one of them phrases it the way the run that wrote `programme` did. **A form the run had no occasion to write has not been tested**, and a run reporting it as a pass is flattering itself — the [fixture policy](../../fixtures/README.md)'s point about a vacuous row, arriving here.

**Issue #103 ruled the two unloaded forms differently.** `fiber` stays because a later ordinary `clinical-note` run wrote it twice from constipation dietary advice: the encounter put that vocabulary under load without a fixture built to force a word. `recognisable` leaves the table because its only appearances were the run's own row-7 self-audit boilerplate, while ordinary notes repeatedly reached the `recognize` and `recognized` root without ever needing that derivative. **Standing rule 4 still forbids the spelling; the scanner simply does not claim to test a derivative no encounter can exercise.** The preserved record is not edited, so its two appearances remain evidence of why the question arose rather than entries the current `--record` vocabulary counts.

**Those figures are not recomputable from this repo**, and that is worth stating rather than leaving to be discovered: the run's notes are patient records and are gitignored, so a reader with only this checkout has this paragraph and nothing to check it against. The command is `python tools/spelling_scan.py <the run directory>`, and the standing answer is the one the [fixture policy](../../fixtures/README.md) gives — **run it again.** A separate committed historical record lets `--record` recompute its own figures; its identity stays outside required instructions.
...
**Where nobody named one and the count cannot be checked, write SOAP, say which you chose, and offer to redo it as an H&P.** Ruled 2026-08-16. The rule above needs the `Student Overview` count, and a run outside `batch-shift` usually has no Medatrax session to read it from — so what happens is not that the rule is applied, but that it is **guessed at from the same words that state it**. Historical evidence shows passes choosing the FNP H&P unprompted and being discarded. Nothing was wrong with their reasoning; the branch is simply not derivable without the portal. The count and run identity remain in the withheld fixture record.

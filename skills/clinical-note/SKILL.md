...
answer and no run declares anything, because a note has no `PROPOSED` block to declare it in and
the tier block audits generated content, unresolved note defects, genuine gaps and unknown tokens.
...
| 26 | **FLAG scope** | Every `FLAG` is true of the **finished note** it travels with: the named finding remains unaddressed in the **Assessment or Plan**, and the stated missing action does not appear there. A failure of the **source encounter** that the finished note corrects belongs under `FILLED·proposed` and is not a FLAG. Check every entry against the note body; encounter-scoped wording and a clarifier do not rescue a contradiction |

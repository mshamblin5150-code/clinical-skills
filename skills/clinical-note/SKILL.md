...
**That shape is required rather than merely observed.** A prior run produced it unprompted; what it lacked was anything requiring it. Drift row 17 now does.
...
**Counting it needs a pattern rather than a string, and that is the reusable part.** Runs paraphrase a missing-reaction hedge by changing capitalization, inserting words between `reaction` and `not documented`, or naming the allergen inside the clause. A literal count therefore misses some notes. **That is why drift row 17 forbids any hedge rather than enumerating observed strings.** Exact phrasings, counts and note identities remain in the withheld fixture record under [#147](https://github.com/mshamblin5150-code/clinical-skills/issues/147)'s rule.
...
**And the encounter #59 asked for was never found.** The ticket named a complaint that does not hurt in an encounter with **no pain source in it** — the patient #42's ruling created, which reasons to zero honestly — and no committed fixture holds that shape. The clinician ruled against the question rather than against a note. That is weaker than how the rules around it were settled and is stated rather than smoothed over; coverage is still owed in [#138](https://github.com/mshamblin5150-code/clinical-skills/issues/138). Issue #59.
...
**Its quiet failure is a note that grades itself as compliant.** A note can put nutrition and activity counseling into `FILLED·proposed`, omit every matching line from the Plan, and still report drift row 4 as a pass by pointing at an Assessment sentence. The claim then has a real string behind it at every level except the one that acts. Historical output showed the same run writing the Plan line correctly elsewhere, so this is not the skill being unable to write it; exact text and counts remain in the withheld record.

...
- Screenings appropriate to the patient's age — **checked against the shipped sheet rather than recalled**, and carrying the population the recommendation is keyed to. See *Guideline sheets*. This is the bullet a note followed to build a nine-item screening list on an age it had invented.
...
**The obligation fires on the sheet, not on the subject.** Before writing a Plan item that asserts an age band, a screening interval, a treatment threshold or a target number, open the sheet that covers it. A treatment item with no number in it — `return precautions given`, `follow up as needed` — is outside this rule entirely, and so is every part of the note that is not the Plan.
...
- **A threshold sheet is not**, and its own [README](../../reference/thresholds/README.md) says so: *an empty directory entry is not a negative finding about a guideline.* A missing row there means one of three things, and the note may never claim it means the first:
  ...
  2. the recommendation carries **no number at all** and was scoped out by name — 50 of hypertension's 103 are, reading `no number`, including `single-pill combination recommended` and `shared decision-making principle`,
...
| 24 | **Guideline backing** | Every Plan item asserting an age band, a screening interval, a treatment threshold or a target number was **checked against the shipped sheet that covers it**, and its `FILLED·proposed` line carries the verdict as a tail on that line: a citation naming the sheet, the source and strength, the **population**, and the value — or `no USPSTF row`, or `sheet does not settle it`, or `recalled, no shipped sheet`. **The three silences are not interchangeable**, and a threshold sheet's may never be written as `no guideline applies`. A population key that was **filled** says so; a population condition the encounter never established takes `needs:`. **No citation appears in the note body.** A Plan item carrying no number never fails this row |

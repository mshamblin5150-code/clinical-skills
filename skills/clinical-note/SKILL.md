...
**A second narrowing landed on 2026-08-17 and it is the mirror of the one below.** The zero's carve-out removes the *no anchor* exit; the height's leaves it and adds to it — a filled height's line **names the age and the sex** as well whenever those anchors are available. So `no habitus or percentile datum in the source to move it` still passes and no longer passes *alone*. **Ruled by the clinician**, with the pressure bar beside it, and both are settled by `python tools/filled_vitals_census.py`. The argument is in *Two of these are graded now* above. Issue #97.

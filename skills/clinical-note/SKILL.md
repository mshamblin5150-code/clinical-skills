...
| 36 | **Entry copy and Plan labels** | The finished note keeps every welded `NOT CODED:` clause for row 22. Its Plan uses each of `Non-pharmacologic:`, `Pharmacologic:`, `Health Promotion/Patient Education:`, and `Referral/Follow-up:` exactly once and no other label. Oxygen is pharmacologic; monitoring, positioning, and laboratory and imaging orders are non-pharmacologic, with orders also on Objective's `Labs/Tests today` line; health promotion and referrals use their named labels. Run `python tools/entry_copy.py <finished note>` before entry; a nonzero exit blocks posting. The derived Entry copy under `entry-copies/` has no `NOT CODED` mark and is the note entered and read back in Medatrax. The finished note, never the Entry copy, supplies `SUBMISSION-SHA256` |

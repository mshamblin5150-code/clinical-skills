...
**The pain score's two directions are split between row 4 and row 13, and they do not overlap.** Row 4 owns the additive one: a filled score above 0/10 is answered in the Plan, and 0/10 is the single value that asks for nothing. **A filled 0/10 is therefore not a discharge** — no obligation arose for it to discharge, and *filled content is unremarkable* is already barred from putting it there by default. What row 13 forbids is the score being spent on something else: `abdominal pain 2/10, no imaging` cites a generated number as the reason a documented finding went uninvestigated. That is the same act as deferring a film on a filled saturation, and the score being low is not what makes it different.

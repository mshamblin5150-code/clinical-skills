...
**So the record's figures moved twice in one session**, 8 forms / 20 occurrences / 6 notes, then 9 / 22 / 7, then 10 / 25 / 7 — `case-07` and `case-08` were already in the set, so the tenth form added occurrences without adding a note. **Issue #103 then removed `recognisable` and moved the current instrument to 9 / 23 / 7 without changing the record.** Re-derive with `python tools/spelling_scan.py --record` rather than treating any of those dated figures as the record's permanent property; that is the whole reason the command prints the breakdown.

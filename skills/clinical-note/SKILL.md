...
**The `Name - CODE: rationale` form stopped being permitted and started being required**, and that is the half of the ruling a numeral does not carry. The hyphen pinning a code to its label is the *Punctuation* rule above, and a form that merely emerges from habit cannot support a binary row. `COVID-19 (U07.1): …` and `Acute bronchitis: J20.9 …` both fail row 23 however well-reasoned the rationale is, and the conclusion line is the one exemption — row 22 reads it by position instead, and says why. Issue [#162](https://github.com/mshamblin5150-code/clinical-skills/issues/162); evidence and counts remain in withheld run records.
...
**Every FNP H&P carries `Medical Decision Making` after `Final diagnosis`.** It is a numbered list with one entry per differential diagnosis, in the same order and with the same ordinal. Each entry repeats the differential's diagnosis-and-code pair, states the patient-specific discriminator that places it, explains why it is favored or less likely, and explains why that evidence leads to the workup, treatment, disposition, or follow-up decision. It does not repeat a textbook description and it does not substitute for the shorter rationale already required in the differential.

**The three clinical sections correspond.** The H&P differential names the possibilities, Medical Decision Making explains the case-specific decisions, and the Plan carries those decisions forward. An MDM item with no matching differential entry, a differential entry with no matching MDM item, or a reordered pair fails correspondence.
...
**One entry per numbered item, and the item is the unit rather than the line.** [SOAP.md](SOAP.md) writes an item on one line, code and rationale together; [HP.md](HP.md) writes it on two, the code line and the rationale beneath, because the school's template puts the code on its own line. **A wrapped line belongs to the item that opened it and never opens one** — the numeral is what opens an item, which is `tools/block_scan.py`'s entry-versus-wrap distinction reused here for its reason. That sentence is not pedantry: mandating a form moves the ambiguity from *what is an entry* down to *what is a line* rather than deleting it. A long descriptor can wrap and push a marker onto a continuation line; counting physical lines then returns the wrong unit. This rule is written so the question arrives with the mandate instead of after it. Issue [#124](https://github.com/mshamblin5150-code/clinical-skills/issues/124); corrected figures remain in the withheld run record.
...
| 29 | **H&P MDM** | Every FNP H&P contains `Medical Decision Making` after `Final diagnosis`, numbered one-for-one and in the same order as its differential. Each item repeats its differential diagnosis-and-code pair, states a patient-specific discriminator, gives its favored or less-likely position, and explains why that evidence leads to the decision that follows. Every item corresponds to the Plan where a decision is carried forward |
| 30 | **Medication history** | No finished medication history defers with `unavailable`, `reconcile`, or a missing-sig placeholder. A documented PMH that ordinarily receives maintenance pharmacotherapy has a compatible plausible regimen; a named medication with an incomplete sig has the missing components filled. Every medication carries the drug name in the form the shorthand uses, dose, route, frequency, ongoing status or finite duration, and reason, and every generated component is declared in `FILLED·asserted` |

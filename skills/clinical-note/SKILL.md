...
**A filled vital or measurement that lands normal against a documented condition owes an account.** This is the mirror of the two paragraphs above, and it closes the opposite cheat. Where the value **is** that condition's own diagnostic measure, a normal number is a claim about the disease's state whether the note intends it as one or not. Filling 124/78 for a documented hypertensive and saying nothing about it is indistinguishable from picking a bland number out of the air, and *indistinguishable* is the whole problem: the two runs produce the same document.

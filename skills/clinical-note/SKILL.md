...
**A filled vital may raise an obligation and may never discharge one.** Everything above is the additive half of the license — a generated value that lands abnormal creates work the note must do. This is the subtractive half, and it is the one that carries the danger. **Where a plan decision withholds, defers or narrows a workup, it is reasoned from the givens alone.** A filled vital, a filled body measurement and a filled pain score are not there for that purpose. They may trigger an evaluation; they may never satisfy, reassure or defer one.
...
| 13 | **Filled reassurance** | Every reassurance in the note traces to a given. No decision to withhold, defer or narrow a workup rests on a filled vital, body measurement or pain score, and any cause a filled abnormal is attributed to is a **given finding** |
...
**Row 13 is row 1 read in a mirror, and it is checked by asking what the note declined to do.** Row 1 exempts filled vitals from tracing to a given, because they have to be allowed to exist. Row 13 refuses them that exemption, because they must not be allowed to reassure. Every other row reads what the note contains; this one reads what it decided against. Take each plan decision that withholds, defers or narrows — no imaging, no culture, no referral, watchful waiting, no analgesia — and name what it rests on. Anything resting on a generated number fails.

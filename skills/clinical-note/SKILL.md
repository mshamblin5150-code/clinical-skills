...
After the clinical note and before or after the Medatrax field block, emit a visibly separated heading, `Proposed coding worksheet — verify before entry`. It is outside the clinical note body and outside the Medatrax field block; no proposed procedure or E/M code is silently presented as part of the patient's record or copied into Add Visit Data.

Run [icd10-cpt](../icd10-cpt/SKILL.md) over the complete note plus its tier block. The worksheet includes the E/M supporting elements and every supported CPT and HCPCS procedure or supply code. Each procedure entry carries its official descriptor, a verbatim note anchor, source provenance where the anchor was filled, substantive specificity, and confidence from `reference/procedure-codes-2026.sqlite` on the service date. Do not pad the worksheet with a code for an action the encounter did not document.

The procedure database does not select an E/M level. State the problems addressed, data reviewed or analyzed, and management risk; the clinician assigns the final E/M level after reading the applicable CPT Professional 2026 instructions. A candidate code may be shown only as proposed and never as database-verified merely because its number and descriptor exist. When the live book was not read, say that the E/M phrasing is recalled and requires clinician verification.
...
| 34 | **Coding worksheet** | Every finished encounter is followed by a separated `Proposed coding worksheet — verify before entry`, outside the clinical note body and Medatrax fields. It supplies E/M supporting elements for clinician selection and every supported CPT and HCPCS procedure or supply code. Each procedure code has its official descriptor, encounter anchor, specificity account, provenance when filled, and service-date verification against the committed procedure-code database; no undocumented service is coded and no differential code is offered for entry |
...
**`python tools/differential_scan.py <a run directory>` supplies row 24's mechanical floor.** It reads every physical `FILLED·proposed` opener, fails a guideline tail pushed onto a continuation line, checks a directly joined USPSTF grade and year or a threshold sheet's source, class and value, and disproves a `no row`, `sheet does not settle it`, or `recalled, no shipped sheet` verdict where the shipped artifacts make the contradiction mechanical. Screening, counseling, immunization, vaccination, target, cutoff and threshold subjects with no tail are definite findings. For every other missing tail, the command refuses to guess whether the item depends on a population or threshold: **missing tails on undecidable items are candidates**, printed outside the exit status. A `recalled, no shipped sheet` verdict whose subject joins no registry topic is likewise a candidate. A run with no qualifying item is a clean zero on this floor, not an unscanned run.

...
A synthetic example: the plan line prescribes a cough-and-cold combination to a four-year-old, although its label contraindicates use under six. The correct note keeps the drug in the Plan with its sig and puts the contraindication beside it as a recommendation to withhold for the preceptor to rule on. It does not report that the drug was not given, because the shorthand says it was prescribed.

**Dropping one is worse than the defects it gets mistaken for, because the note then documents the opposite of what happened.** A drifted finding is something the note failed to say; a dropped order is something the note says that is **false about the encounter**, on a record signed under the clinician's name. A note that carries six of seven ordered tests, omits the monospot and then writes `No monospot sent on those findings` has converted its own erasure into a clinical claim. That is why row 18 exists rather than being left to row 15's reading. Issue #66.
...
**The allergy row counts *no drug allergen* and not *says nothing*, and that distinction is the whole of issue #78.** `NKDA` is *no known **drug** allergy*, so a patient with hay fever is NKDA and a note naming a seasonal allergy is no evidence against filling it. Counted undivided the corpus says only 111 of 284 written statuses say nothing — 39%, a minority, and the reverse of the fixture floor. That was #78's own trigger for reopening the ruling, and it fired on a column measuring the wrong thing: four of the eight fixture cases in it name nothing but a seasonal allergy. **Split on the kind `NKDA` is a claim about, the two slots land as far apart as they ever did — 69% against 13%** — and the clinician confirmed the reading on 2026-08-16: *silence about drug allergies takes `NKDA`; seasonal allergies fall under environmental, which is a separate category, as is food.*
...
**Each branch's prose asserted a current tally, and the merged tree contradicted both.** #278 wrote `14 / 45 / 7` and #103 wrote `9 / 23 / 7`; each was true of its own tree and neither is true here. Git **conflicted** on this sentence — the safe direction, and only because both branches happened to edit the same line. The scanner's own table auto-merged silently one file over, gaining four rows and losing one with nothing to look at. That is [#86](https://github.com/mshamblin5150-code/clinical-skills/issues/86)'s *the merge is the unguarded moment* arriving on the figure this paragraph exists to keep honest, and it is the case the binding below was written for a day earlier: **the tally is now checked against what the scanner computes, so a merge that picks either branch's number fails a test instead of shipping.**

**That last figure is bound and the three before it are not, which is the difference between history and a claim about the tree.** `8 / 20 / 6`, `9 / 22 / 7` and `10 / 25 / 7` are what this paragraph *used to read* — past values by construction, and no reader can act on them. The fourth is what the record holds now, so `tools/test_spelling_scan.py` parses this sentence and fails when it disagrees with what the scanner computes. **It was unbound for the length of one review**, and a reviewer mutated it to a figure the record has never held with every spelling test still green — [#143](https://github.com/mshamblin5150-code/clinical-skills/issues/143) inside the change that cites #143. Re-derive with `python tools/spelling_scan.py --record` rather than quoting this sentence; that is the whole reason the command prints the breakdown.

**That run wrote both spellings of most of them**, which is what makes the record evidence of drift rather than of a register: `cesarean` eight times against `caesarean` twice, `dyspnea` seven against `dyspnoea` three, `program` nine against `programme` twice, `fiber` three against `fibre` four. **And `counseling` 57 against `counselling` 12, the largest pair in the set and invisible until 2026-08-20.** Its split is **perfectly partitioned by note**: the two notes that wrote the British form never wrote the American one, and the ten that wrote the American form never wrote the British one. **That is the paragraph's own claim at its sharpest, and it is the opposite of a slip** — a reader of any single note sees perfect internal consistency, and the drift exists only across the twelve, which is exactly why twelve outputs had to be put in front of one reader. **Only `caesarean` and `fibre` put both spellings inside one note at all.** Nobody had seen any of it because the table did not hold the form, which is [#278](https://github.com/mshamblin5150-code/clinical-skills/issues/278)'s subject arriving inside [#73](https://github.com/mshamblin5150-code/clinical-skills/issues/73)'s evidence: **the strongest thing this record has to say was the thing the instrument could not read.**

**This sentence was published with the mechanism backwards and is worth keeping as a correction rather than a clean edit.** It first read *"the widest ratio in the set, in the same two notes — every other pair here is close enough to read as a slip"*. Both halves are false: the two notes carry **no** occurrence of the American form, so it is a between-note partition rather than a within-note ratio, and *slip* describes the two forms that genuinely do sit inside one note, which are the two named above. **The count was right and the reading of it was invented**, which is this repo's recurring shape landing on the paragraph about a record that exists to stop exactly that. Found by a tracker-sweep subagent, re-derived note by note before it was believed, and both properties are pinned now. **Nobody reading one note would see it** — the same shape as [#67](https://github.com/mshamblin5150-code/clinical-skills/issues/67), and the same reason twelve outputs had to be put in front of one reader.
...
**`hypoxaemic` is the third stem change**, beside `labelling` and `catheterisation`: the suffix rule appends and `-ia` to `-ic` replaces, so the adjective is out of reach of its own noun. It is on the table because the run wrote both, which is the same test `manoeuvring` fails — that one is *not* carried, because only `manoeuvres` was ever written. **Evidence decides which inflections, not English.**
...
**Nothing in this repo fixtures that limb**, and it is stated here rather than left to be found. A legitimate filled zero needs a complaint that does not hurt in an encounter with no pain source, and no committed fixture holds that shape. The [fixture policy](../../fixtures/README.md) forbids authoring one merely to make the row pass. [#138](https://github.com/mshamblin5150-code/clinical-skills/issues/138).

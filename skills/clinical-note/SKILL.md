...
**A family disease bundle is distributed plausibly across the required relatives.** Preserve every disease the shorthand names, assign it among mother, father, maternal grandparents, paternal grandparents, and siblings in a clinically reasonable three-generation pattern, and allow the same disease to appear in more than one relative. Do not repeat the whole bundle under both parents and do not write `distribution not specified` or `exact relative not specified` in the finished note. Every generated relative-to-disease assignment is declared in `FILLED·asserted`.

**Every multi-disease line in Family History is conjunctive.** Join its final disease with `and`, never `or`: `Diabetes, hypertension, and coronary artery disease.` The word `or` would say the history is undecided between diseases rather than reporting all of them.
...
**Every FNP H&P carries `Medical Decision Making` after `Final diagnosis`.** It is a numbered list with one entry per differential diagnosis, in the same order and with the same ordinal. Each entry repeats the differential's diagnosis-and-code pair, states the patient-specific discriminator that places it, explains why it is favored or less likely, and names the workup, treatment, disposition, or follow-up decision that follows. It does not repeat a textbook description and it does not substitute for the shorter rationale already required in the differential.
...
| 29 | **H&P MDM** | Every FNP H&P contains `Medical Decision Making` after `Final diagnosis`, numbered one-for-one and in the same order as its differential. Each item repeats its differential diagnosis-and-code pair, states a patient-specific discriminator, gives its favored or less-likely position, and names the decision that follows. Every item corresponds to the Plan where a decision is carried forward |

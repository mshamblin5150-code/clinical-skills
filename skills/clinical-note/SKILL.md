...
#### An impossible given stays given

**A number can be faithfully transcribed and still be unusable.** *Impossible* means the token cannot represent the thing its label says it represents: no patient could have the recorded measurement, or the stated product strength does not exist. *Implausible* is not the same class. An unusual but possible height, weight, pressure or dose stays available to the note and is questioned on the ordinary clinical reading; this rule fires only when the value cannot be true as labeled.

**Preserve the exact token in the note and stop it there. Never replace it with a plausible candidate.** The note names that the recorded value is unusable pending source verification. A likely correction may be named only in `UNKNOWN`, marked as a guess, and is never substituted into the body. Two candidate readings that both fit are two guesses, not a license to choose the closer one.

**`UNKNOWN` and `FLAG` do different jobs, and an impossible given takes both.** `UNKNOWN` quotes the token and records any candidate readings, each marked as a guess and explicitly not applied. `FLAG` asks the clinician to verify the source and names what cannot safely proceed until that happens. The first preserves uncertainty; the second makes the unresolved value actionable. Neither is a `FILLED` line, because no replacement value was generated.

**Nothing downstream may spend the impossible number.** It is excluded from arithmetic, diagnosis, code selection, dose calculation or administration, and clinical reasoning. A derived value that needs it is not derived; the needed usable input goes under `GAPS`. A given order carrying the number is still preserved on *A given order is a given*'s rule, with the verification objection beside it and no instruction to administer the unusable dose. Deleting the order would erase what happened; normalizing its number would invent what happened.

**A structured Medatrax field never receives the impossible token or a guessed replacement.** Emit the field as an explicit `GAPS` entry: the source token is quoted, the usable value is unrecoverable, and source verification is required. That is a genuine gap even though characters were supplied — the encounter supplied no value the field can use. Free-text note content still carries the exact given, so the record preserves the defect without entering it as patient data.

**No numeric scanner defines this class.** A deliberately wide, label-anchored range can surface an order-of-magnitude vital for a reader, but it cannot recognize an unavailable drug strength, and a bare `number/number` pattern also matches valid medication strengths. The rule is the reader's clinical determination that the labeled value cannot be true; a tool may over-report candidates and never silently repair one. Issue [#132](https://github.com/mshamblin5150-code/clinical-skills/issues/132).
...
A synthetic example: the plan orders a contrast-enhanced MRI for a patient whose history documents an anaphylactic reaction to gadolinium. The correct note keeps the ordered study in the Plan and puts a recommendation beside it to change the protocol or choose another study for the preceptor to rule on. It does not report that the MRI was cancelled, because the shorthand says it was ordered.

**Dropping one is worse than the defects it gets mistaken for, because the note then documents the opposite of what happened.** A drifted finding is something the note failed to say; a dropped order is something the note says that is **false about the encounter**, on a record signed under the clinician's name. Omitting a given order and then writing that it was not placed converts an erasure into a clinical claim. That is why row 18 exists rather than being left to row 15's reading. Issue #66.
...
**A genuine conflict is written as a span containing both.** `Duration within the past 1 to 2 days`, never one endpoint chosen over the other. Both stated values survive as the span's endpoints, so nothing is rounded, replaced or corrected, and *Never "correct" a number* stands untouched. The same span form is ordinary in shorthand ranges such as `8-9 years ago` or `worsening in the past 5-6 days`; the synthetic values teach the form without exposing a fixture input.

**The fixture cases and their control are deliberately not described here.** [fixtures/README](../../fixtures/README.md) requires a set's inputs and identity to be withheld from a generating pass and the input pasted rather than pointed at; this file is one the pass always reads.
...
**And it is never a FLAG.** FLAG holds a documented finding the note failed to act on; two durations that can be preserved honestly as one span are not an unresolved defect. Reporting the two endpoints as a contradiction would merely say the note could not read the timeline it was required to reconcile. Issue #33.
...
#### Medication history is completed, not deferred

**A documented PMH and an absent medication list produce a plausible medication history.** The finished note never reads `medication list unavailable`, `dose unavailable`, `sig unavailable`, `reconcile medications`, or an equivalent deferral. For each condition that ordinarily receives maintenance pharmacotherapy, infer a conservative, common regimen compatible with the patient's documented conditions, allergies, age, renal or hepatic information, and given medications. A condition that ordinarily needs no drug can remain without one; the note does not add a drug merely to make every diagnosis have a pair.

**Medication completeness is a coverage decision, not a pill count.** Before finishing the note, map every PMH condition in the private `FILLED·asserted` accounting to a listed medication, a listed medication that legitimately covers more than one condition, or no routine maintenance pharmacotherapy on the available facts. A multimorbid adult with three medications passes only when that mapping accounts for every remaining PMH condition; a longer list fails when a pharmacologically managed condition is still uncovered. The mapping stays out of the finished note, and no medication is added merely to make the list look long.

**Every historical medication is complete enough to administer as written.** It carries the drug name in the form the shorthand uses, dose, route, frequency, ongoing status or finite duration, and reason for taking. When the shorthand names the drug but omits part of its sig, preserve the named drug and infer the omitted dose, route, frequency, and ongoing status or duration. When the shorthand supplies a complete part, keep it unchanged. Each inferred drug and each inferred sig component is declared in `FILLED·asserted`, carrying the value selected and the PMH or given drug that grounds it.

**The compatibility check precedes the inference.** A generated regimen never hides or resolves a conflict between givens, and it never supplies a medication contraindicated by the documented record. If no ordinary compatible regimen can be selected, retain the given history, name the conflict in the Assessment or Plan, and fill only what remains clinically supportable. This is medication-history completion, not authority to generate a laboratory value, imaging result, symptom, or examination finding.

#### Family history distributes the reported diseases

**A family disease bundle is distributed plausibly across the required relatives.** Preserve every disease the shorthand names, assign it among mother, father, maternal grandparents, paternal grandparents, and siblings in a clinically reasonable three-generation pattern, and allow the same disease to appear in more than one relative. The diseases remain givens; only the relative-to-disease relationships are filled, and no disease may be added. Do not repeat the whole bundle under both parents and do not write `distribution not specified` or `exact relative not specified` in the finished note. Every generated relative-to-disease assignment is declared in `FILLED·asserted`; it never supports a patient diagnosis, finding, or result.

**Each relative's multi-disease list in Family History is conjunctive.** Join its final disease with `and`, never `or`: `Diabetes, hypertension, and coronary artery disease.` The word `or` would say the history is undecided between diseases rather than reporting all of them.
...
**So a positive tobacco status is never filled.** Not `vapes some days`, not `quit smoking years ago`, not `smokes 0.75 PPD`, however plausible the patient makes it. This is the same argument the corpus settled for blood pressure at issue #23 — *a documented condition is an anchor to reason from, never a verdict to produce* — arriving at a slot instead of a value. A tobacco history is an abnormal finding, an abnormal finding in the note must be a given, and there is no exception for a slot that happens to be enumerated.
...
**The one field always enumerates all three categories, one line each and in this order: Drug, Food, Environmental.** Ruled by the clinician 2026-08-20, issue [#168](https://github.com/mshamblin5150-code/clinical-skills/issues/168). **When the shorthand names no allergy, treat that silence as NKDA; do not write that no allergy history was taken.** Ruled by the clinician 2026-08-20, issue [#203](https://github.com/mshamblin5150-code/clinical-skills/issues/203). Silence fills `Drug - NKDA`, `Food - none reported`, and `Environmental - none reported`. A stated allergen replaces only its category's filled negative; the other two lines remain. Every filled category value is declared in `FILLED·asserted` carrying its value.
...
**A food intolerance is carried, and the word `intolerance` on the line is what makes it safe to carry.** `sorbitol intolerance` is not an immune reaction, so it is not an allergy at all — and it belongs in the box because an excipient intolerance can constrain prescribing. Naming it without the word would assert a food allergy the patient does not have; omitting it loses a real constraint. This is a synthetic example ruled with the paragraph above.
...
**The box is also where the coding step reads.** A note that writes a bare `NKDA` and leaves an environmental allergen in `PMH` hands `icd10-cpt` a different sentence to code from. **So the placement was never presentational**, which is the strongest single reason this needed a ruling rather than a convention.
##### The reaction beside a given allergen

**Where the allergen is a given and the reaction is not, the reaction is inferred.** The clinician ruled it on 2026-08-16, [#94](https://github.com/mshamblin5150-code/clinical-skills/issues/94). The shorthand can name the allergen and stop, while the template still demands `allergen - reaction`; before this ruling there was **no legal value at all**: the box demanded one, the grounding rule supplied none, and drift row 12 banned the hedge. Historical run records remain unchanged because their anchors are quotations and must not be rewritten.

**Checking it needs a rule rather than a literal string.** A missing-reaction hedge can change capitalization, insert words, or name the allergen inside the clause while preserving the same forbidden meaning. **That is why drift row 17 forbids any hedge rather than enumerating strings.** Historical phrasings, counts and note identities remain in the withheld fixture record under [#147](https://github.com/mshamblin5150-code/clinical-skills/issues/147)'s rule.
...
**That reasoning is per allergen, and the declaration is the enforceable bar.** After obvious misspellings are corrected, each distinct allergen whose reaction was filled gets its own `FILLED·asserted` entry. The entry names the allergen, carries the reaction, and names what the reaction was **reasoned from for that allergen** — its drug, food, or environmental class and any encounter fact that moved the choice. One rationale for the list does not satisfy several entries, even when the same reaction is genuinely plausible for several allergens. Repetition by itself is not a failure: forcing different reactions would invent variation the encounter did not supply.

**Distinct means the clinical allergen after transcription correction, not the number of tokens.** Two spellings corrected to one drug produce one Allergies entry and one reaction declaration, not two patient allergies. This is *Correct obvious misspellings of clinical terms as you read* applied before the reactions are counted; transcription noise is not content. Drift row 27 walks the declaration one allergen at a time. Issue [#205](https://github.com/mshamblin5150-code/clinical-skills/issues/205).
...
**The worked example is synthetic and names no committed fixture.** That is required by [fixtures/README](../../fixtures/README.md)'s blind-instruction rule: this file is one every generating pass reads, so a fixture-derived example would hand the pass part of its own answer. Issue [#147](https://github.com/mshamblin5150-code/clinical-skills/issues/147).
...
**The fixture counts belong in the withheld assertion records, not here.** [#78](https://github.com/mshamblin5150-code/clinical-skills/issues/78)'s corpus-wide kind split makes food allergy a corpus concern rather than a fixture-only one. A food intolerance is still not a food allergen and does not take an allergy code merely because it appears in this box; whether an intolerance reaches the box is [#96](https://github.com/mshamblin5150-code/clinical-skills/issues/96)'s ruling above.
...
**What *addressed* means was ruled 2026-08-15, and it narrows the word to two exits.** The old Assessment-**or**-Plan wording allowed a note to mention a generated abnormal without doing anything about it. The rule as ruled:
...
**This repo reached the same rule independently for blood pressure against documented hypertension and BMI against documented obesity.** A filled BMI below 30.0 needs the Assessment to say the obesity is resolved, improved or post-surgical, or to name the weight-loss intervention. That is the same rule with the nouns changed, including its exclusion: an `E66` sitting in a problem list is no more an account than an `I10` is. Two independent derivations landing on one shape is the argument for stating it generally.
...
**The rejected alternative is kept here because it is what the ruling costs.** A filled `0/10` could have been a discharge like any other, owing the Plan an account the way row 14 makes a filled normal pressure owe one — a run that writes zero into a silent box has chosen a number that makes analgesia unnecessary, and under this ruling that run passes row 15. **So the Plan catches nothing here, and every guard on a bland zero is upstream of the Plan.** The choosing rules forbid zero where the complaint unarguably hurts or the exam holds a pain source, and *A reasoned 0/10 is not the default* below bars *filled content is unremarkable* from putting it there by itself. **The disclosure guard** is *a filled 0/10 may not take drift row 19's no anchor exit* below, which makes the line name the search instead. That is the shape of this decision — guards on which number gets chosen, one on how it is declared, and nothing at all after it reaches the note — and it is worth knowing before trusting a zero.

**The ruling still needs legitimate coverage of its narrowest shape:** a complaint that does not hurt in an encounter with no pain source, which can reason honestly to zero. Do not manufacture an encounter merely to exercise it; coverage follows the fixture policy and remains tracked in [#138](https://github.com/mshamblin5150-code/clinical-skills/issues/138). Issue #59.

**A pending test refuses only what its result would establish.** This is the descriptor rule from [icd10-cpt](../icd10-cpt/SKILL.md) carried into the differential. A culture can establish bacteria in blood or name an organism, so it can gate `R78.81 Bacteremia` and an organism-specific code. It does not gate `A41.9 Sepsis, unspecified organism`, `J18.9 Pneumonia, unspecified organism`, `L03.-` cellulitis or `N39.0 Urinary tract infection, site not specified` when the encounter clinically established the diagnosis, because those descriptors name no organism for the culture to supply. Code the diagnosis the encounter established and route only the organism-specific code into a welded `NOT CODED:` clause.

**A pending test is not a finding.** A differential may reject a diagnosis on documented findings or carry it when the findings establish it; *culture pending*, *no culture drawn* and *no result yet* do neither. The sepsis worked shape below may rank sepsis down on the documented bedside findings while the CBC and lactate are ordered anyway. That reasoning changes no action and does not spend a filled value under drift row 15; a missing culture would be the wrong reason regardless of whether the vitals were given or filled.

**Imaging is different because its result may establish the disease itself.** A chest film can establish the infiltrate rather than merely name its cause, so an unresulted film may leave pneumonia unsupported where the encounter documents only a suspicion. It is still not evidence against pneumonia: name the documented clear lungs or other rejecting findings when they are the reason, and do not let a pending film erase a diagnosis the encounter already established clinically. A resulted negative film is a finding and may reject the disease. This separate limb is why the culture rule does not silently decide pneumonia examples. Issue [#149](https://github.com/mshamblin5150-code/clinical-skills/issues/149).
...
**Which encounter, and what she said, are deliberately not written here.** They remain in the withheld fixture record under [fixtures/README](../../fixtures/README.md)'s separation rule. A run that could read the answer off this file would recall it instead of reasoning to it, and no grader could tell the difference — which is the whole defect that separation exists to prevent, arriving by way of a rule's own provenance.
...
**Across a set the filled values can collapse onto a template.** Check the run as a population rather than trusting individually plausible numbers:

```bash
python tools/filled_vitals_census.py <the run directory>
```

The question is whether individually defensible values repeat as a template or cluster implausibly against the corpus distribution. **The evidence, exact values and counts stay in the withheld fixture records.** Issue #67.
...
**What a strongly one-sided pattern shows is anchors going unused, and the pressures are where that is unarguable.** A height is often unanchored. A blood pressure in an encounter with a documented condition, a given pulse, a given temperature, a documented pain score and an exam describing distress is not — the anchors are sitting in the note, and a run that repeatedly ignores their direction has stopped reading them.

**Do not move the value to create the disclosure either.** *Do not move the value to avoid making the disclosure* is stated below for the BMI thresholds; this is its mirror. **A filled value is not chosen to give the note an abnormal to work up.** The incentive is real and it is this file's own doing: far more is written here about what an abnormal filled value obliges than about what a normal one does, so a run demonstrating its compliance can manufacture something to be compliant about.
...
**This is not the bland-normal rule coming back**, and the difference is the whole of the section above. A value with an anchor follows the anchor wherever it goes — a 373 lb patient's pressure, a COPD patient's saturation, a distressed toddler's pulse are all abnormal and all earned. What is forbidden is an abnormal with nothing behind it. **So the two halves of the row work together** — the anchor makes the abnormal legitimate, and naming it is what a reader checks.

**The former rule is not allowed to survive as habit.** Until 2026-08-11 this section ordered a hypertensive pressure for a documented hypertensive; issue #23 retired it on a corpus count. A documented condition is now only an anchor to reason from, never a verdict to reproduce. Evidence and historical verdicts remain in the withheld fixture record.
...
**A filled height's clause names the age and the sex** — the span from its own label to the next declared value, which is what the scanner reads and is **not** the physical line: several declared values share one. [#127](https://github.com/mshamblin5150-code/clinical-skills/issues/127) is the same undefined-unit question in the neighboring block, and this is the first place the unit is pinned rather than assumed. **When age is absent the height leans on an inferred age**, which is a real cost and is named here rather than left to be found. `Plausible adult male height` names a sex and no age and fails; `Mid-range for a 67-year-old man` and `plausible for a 43-year-old female` pass. **What is graded is the naming, not the claim** — `Approximately the 60th percentile for a 16-year-old male` satisfies this rule's naming shape, but no committed tool computes stature-for-age and the BMI-only `tools/cdc_percentile.py` must never be used to justify it. Do not generate a height percentile without a stature-for-age source. **Spell the sex** — a bare `M` or `F` does not count, because `T 98.4 F filled` sits in the same block and its Fahrenheit mark would otherwise satisfy the rule for whatever height is beside it. **The `no habitus or percentile datum` exit is untouched**: it still passes, and the age and sex are named alongside it rather than instead of it.
...
**Filled pressures may not land not-normal far more often than a fair split explains.** The corpus grounds the fair split at 130/80; `tools/filled_vitals_census.py` applies the committed false-alarm threshold to the finished run. The threshold is deliberately conservative so an ordinary coin-flip outcome passes and only a strongly one-sided run fails. Exact fixture-set sizes, outcomes and verdicts stay in the withheld record.
...
**A clean run is not a walked section, and the gap is named rather than left implicit.** Three vital classes are graded. **Temperature, heart rate, respiratory rate, oxygen saturation and pain score are counted and not graded** — the corpus offers no even split for any of them to ground a bar the way 130/80 grounds the pressure one. Whether a filled temperature was reasoned is still a question for a reader; exact fixture counts remain in the withheld record.
...
#### HPI is history, not a replay of the visit

**The HPI contains what the patient or caregiver experienced, reported, tried, and observed before or during the history-taking conversation.** Its narrative advances the symptom chronology, associated symptoms, context, prior evaluation, attempted treatment, and response. A prior study may appear when the patient reports its result and that history changes the current reasoning.
...
It is a `FILLED·proposed` line like any other order, generated from the exposure and never from noticing that the encounter omitted it. **The encounter not having swabbed is not by itself a FLAG** — see *FLAG is the block that matters* in step 6. A clinician who documents a sick contact and treats empirically has made a call, and a note that flags him for it flags him on every encounter where he made the same one. Whether testing would have changed the encounter's management decides how strongly the note should act; once the Assessment or Plan carries that action, the finding is not abandoned and there is no FLAG.
...
**Where neither appears — about one note in fourteen — infer the age, and preserve that provenance in the private tier record.** The old rule stopped, because age sets `Patient Time` and a wrong band misallocates clinical hours. But a note that stops cannot be entered at all, and the clinician infers and enters it himself: three confirmed cases, one carrying nothing but `1 ppd x 41 yrs`.
...
```
FILLED·asserted   AGE 60 y filled from 41 pack-years begun in adulthood;
                  sets Patient Time = Adult (18 – 60).
```

That line is the private record of the fill. **The clinical note body and the Medatrax field block carry the age and its consequences as ordinary values, with no `filled`, `inferred`, `guess`, or confirmation label.** The provenance is retained for review and is never submitted as part of the note or copied into Medatrax. Issue [#158](https://github.com/mshamblin5150-code/clinical-skills/issues/158).
...
| [`reference/thresholds/`](../../reference/thresholds/) | per topic, the numeric decision points — target, cutoff, dose, interval — each with a verbatim snippet, source, page and class | A run joins a threshold sheet on the `artifact` column, whatever the row's state, using [`reference/thresholds/coverage.md`](../../reference/thresholds/coverage.md). An artifact under `unread` is a real sheet whose full-document read is pending, and the sheet's own `## Scope` names what it did not read. The registry derives its full topic population from the 180-document catalog. |

**Guideline routing begins with the diagnosis, not with the screening line.** Before writing diagnosis-directed testing, treatment, medication continuation or hold, monitoring, referral, or disposition, search the catalog for the diagnosis or clinical subject and follow its `artifact` through `reference/thresholds/coverage.md`. Open every joined sheet that can govern the decision. A multimorbidity plan is routed once for each active diagnosis; finding a screening row does not discharge the treatment decisions elsewhere in the Plan.

**The population-or-threshold obligation remains absolute.** Before writing a Plan item whose appropriateness rests on a **population or a threshold** — every screening, counseling and immunization item, and every treatment item resting on a target or cutoff — the applicable row is consulted rather than recalled. The obligation fires on the item's subject, never on whether the item states a number. Gating this duty on a number in the proposed item was the live alternative and it was rejected: it would let the item escape by being vague. An item resting on neither is outside this rule entirely.
...
A sheet whose `## Scope` reports nothing unread has eliminated meaning 3; it has not
turned either remaining meaning into a note-body verdict.

So a threshold sheet that holds no row for what the note asserts earns `sheet does not settle it` and never `no guideline applies`. Write `sheet does not settle it` whole, with no subject and no source in the tail. The two `recalled` verdict wordings are `recalled, no shipped sheet` (optionally followed by `; catalog lists <document>`) and `recalled, source page unread`; no other recalled silence is a verdict.
...
FILLED·proposed   Review the source recommendation with the clinician [recalled, source page unread]
```
...
**The tail stays on the item's line however long the line gets, and the lines above are not shortened to make that look comfortable.** The rule is about **the tail**, and nothing else in the block inherits it — a `DERIVED` line's arithmetic wraps the way the inferred-age line does, because arithmetic is not a marker and loses nothing by being read on two lines. A citation pushed onto a continuation line is the defect the same-line rule exists to prevent, and it is worse here than for `NOT FOR ENTRY` because a citation is three to five times longer and so likelier to wrap by accident. Historical output already put end-of-line markers on continuation lines, which is [#70](https://github.com/mshamblin5150-code/clinical-skills/issues/70) — **so a marker on the item's own line is countable, not reliably countable**, and that limit is named here rather than left for a counter to discover.
...
**The population is taken from the sheet's own cell. It may be shortened; it may never be softened.** Every condition in the cell survives into the tail — the wording may compress, and nothing may drop out. `adults aged 40 to 75 years who have 1 or more CVD risk factors and an estimated 10-year CVD risk of 10% or greater` becomes `adults 40 to 75 with 1+ risk factor and 10-year CVD risk 10%+`, which is shorter and asserts the same three things. Dropping the risk threshold would be softening, and it is the one edit that makes an unmet population read as met. **The test is whether a reader could be surprised by the cell**, and the two examples above are the calibration: nothing in either sheet's row would surprise someone who had read only the tail. Two things about that cell are worth knowing before it is trusted. In the USPSTF sheet the column is **quoted from the statement when possible and otherwise from the document's declared `POPULATION` field** — the sheet points to the named pairs at its top and states the full rule in its field-quotation section — so it remains a reading, and a row whose population decides the patient's care is a row to check against the page it names. **Every committed population cell is present**, and the count pin that keeps them present does not establish that their content is correct.

**A pediatric BMI population is evaluated with the same committed CDC tool as its code.** `Interventions for High Body Mass Index in Children and Adolescents` is keyed to *a high BMI (≥95th percentile for age and sex)*. Run `tools/cdc_percentile.py` before writing the item; the computed percentile decides whether the population is met. If the tool filled a midpoint month from a whole-year age, the citation tail says `age month filled` on the general rule below. The month remains a filled population key, but the percentile is no longer recalled.

```
FILLED·proposed   Weight management counseling discussed [uspstf: grade B, children and adolescents 6 years or older with a high BMI (≥95th percentile for age and sex), 2024] age month filled
```

**The population is the field that is not optional.** A note can fill the wrong age and then build an age-keyed screening list whose recommendations are individually correct but owed to a different patient population. A citation without its population makes that note look better without making it truer. Issue [#158](https://github.com/mshamblin5150-code/clinical-skills/issues/158).
...
**Whether the recommendation applies to this patient is not machine-checkable, and no wording here makes it so.** The population is quoted so a reader can compare it in one jump; that is the whole of what this buys. Two further limits are the sheets' own, written down in [reference/thresholds/README.md](../../reference/thresholds/README.md) rather than discovered: a sheet whose numbers are all real and all filed under the wrong heading passes every *automatic* gate in that directory — since [#174](https://github.com/mshamblin5150-code/clinical-skills/issues/174) a second independent read sets each row's heading beside another reader's description of the number, and grades neither, so that hole closes only where somebody runs one and reads the pairs — and some documents cannot be omission-gated at all while others can only be warned about — so *the sheet was consulted* and *the sheet is complete* are separate claims and only the first one is cheap.
...
answer and no run declares anything, because a note has no `PROPOSED` block to declare it in. The
tier block is not one kind of claim: its separate lanes hold arithmetic, generated content,
...
#### H&P medical decision making

**The H&P Assessment uses the case-study sequence.** `Differential diagnoses` is a clean numbered and likelihood-ranked list of diagnosis-and-code pairs, with no rationale paragraphs beneath it. `Final diagnosis` follows and attaches the decisive patient-specific discriminator to each selected diagnosis; when more than one diagnosis is final, use a short numbered list rather than welding several reasoned conclusions into one sentence.

**Every FNP H&P then carries `Medical Decision Making`.** It is a numbered list with one entry per differential diagnosis, in the same order and with the same ordinal. Render each entry as a diagnosis-and-code header carrying its verdict — `favored`, `less likely`, or `must exclude` — followed by the patient-specific evidence that puts it in or out and why that evidence leads to the workup, treatment, disposition, or follow-up decision. This is where the differential rationale lives; it does not repeat a textbook description.

**The four clinical sections correspond.** The H&P differential names the possibilities, Final diagnosis identifies the selected entries and their discriminators, Medical Decision Making explains every case-specific placement and decision, and the Plan carries those decisions forward. An MDM item with no matching differential entry, a differential entry with no matching MDM item, a reordered pair, or a final diagnosis absent from the differential fails correspondence.

#### Review of systems layout

**Both branches write one ROS system per line and end with the exact global closer `All other systems reviewed and are negative.`** Individual system lines remain short fragments and never use `reviewed and negative`; that phrase is reserved for the single closer after the last named system.
...
**Where the differential ends is not where the heading does.** A second, separately headed block after the conclusion — `Additional problems addressed today`, `Also addressed this visit`, `Reasoning carried forward` — remains inside the count when its items are diagnosis-shaped, whether coded or uncoded:
...
**A diagnosis-shaped line anywhere in the Assessment is an entry and carries a code**, whatever heading a run writes above it. The narrow reading — count only the block the note itself headed `Differential:` — was the live alternative, and it is escapable by moving an uncoded diagnosis one heading down. That is the same defect in a new place, so it lost.
...
**What only a reader can finish, and that is a consequence of the ruling rather than a gap in the tooling.** Deciding that `Body mass index 28.6, in the overweight range` is a diagnosis and `Drug and condition conflict: …` is not takes a reader, and so does finding a diagnosis-shaped line under a heading a run invented. `python tools/differential_scan.py <a run directory>` supplies [#164](https://github.com/mshamblin5150-code/clinical-skills/issues/164)'s declared floor: it counts the labeled Differential blocks, the numbered items inside them, and the items carrying the required `Name - CODE` slot; a numbered item without one fails row 13. It also supplies row 23's mechanical floor: a labeled block written as prose, a sequence that does not start at `1.` and remain contiguous, an item with no required opener, or one numeral opening more than one pinned entry fails. **Every report says the count covers only those labeled blocks, that the wide Assessment count still needs a reader, and that clinical likelihood order still needs a reader.** A confirmed finding exits 1 before a not-run path can hide it. The command does not find a diagnosis moved under another heading and cannot decide whether the ranking is clinically correct, so **a clean scan is not a walked row 13 or 23**.
...
**That leaves a symptom on the left-hand side, and it is the ruling rather than a side effect.** Put to the clinician on 2026-08-15 against a whole differential rendered this way — symptom labels standing where a grader reading the school's *"3 differential diagnoses"* expects diagnoses — and answered: the disease belongs in the rationale, which is where the differential is graded. Nothing is lost, because every disease considered is still named there with the specific finding that rejects it.

**What the convention buys is that a label cannot assert.** Without one required shape, a note can name a refused disease on the left while keeping its code out of the code slot, or hide the refusal in a footnote, and still look compliant. Pin the label to the code and there is no slot left to hide an unestablished disease in. Issue #68.
...
```
NOT CODED: I24.9 Acute ischemic heart disease, unspecified, serial troponins are pending; NOT CODED: I26.99 Other pulmonary embolism without acute cor pulmonale, CT angiography has no result.
```

The semicolon is what bounds a refusal and its reason, so the second mark is not optional shorthand — `NOT CODED: I24.9 …; I26.99 Other pulmonary embolism without acute cor pulmonale, CT angiography has no result` reads as one refusal with a long reason, and `I26.99` is then a code the note names and never accounts for.
...
```
3. Chest pain, unspecified - R07.9: pleuritic chest pain after recent travel, ECG without ischemic change, serial troponins and CT angiography pending. NOT CODED: I24.9 Acute ischemic heart disease, unspecified, serial troponins are pending; NOT CODED: I26.99 Other pulmonary embolism without acute cor pulmonale, CT angiography has no result. Less likely.
```

**Those two refusals do not share a reason, and one reason covering both would be wrong.** Acute ischemic heart disease waits on the serial cardiac evaluation; pulmonary embolism waits on pulmonary vascular imaging. The synthetic example gives each refused code its own unresolved evidence rather than letting one generic `workup pending` clause cover both. The clinician's ruling, 2026-08-15; [#149](https://github.com/mshamblin5150-code/clinical-skills/issues/149) carries the general form.
...
| `labelled`, `programme`, `licence` | `labeled`, `program`, `license` |
| `neighbour`, `judgement`, `manoeuvre` | `neighbor`, `judgment`, `maneuver` |
| `counselling`, `millimetre`, `hypoxaemia`, `immobilisation` | `counseling`, `millimeter`, `hypoxemia`, `immobilization` |
...
**About the output, not the input.** A British spelling arriving in the shorthand is normalized on the way out like any other spelling variant — the same treatment [GLOSSARY.md](GLOSSARY.md)'s *Misspellings* rule gives a misspelled drug name. It is not a hedge and it is not a number, so nothing in *Given* protects it.

**Forms above were found in this repo**, and that is why the rule is here rather than assumed. They appeared across expansion tables, ordinary prose, assertion records and preserved generated output. Expansion-table errors are especially costly because the skill copies an expansion into a note by design; preserved output remains byte for byte apart from recorded redactions because correcting it would falsify the evidence. Exact emitted vocabulary, figures and record identity stay in the withheld record. Issue #73.
...
**Adding a spelling can change a preserved record's computed figures without the record moving**, and the two facts have to be kept apart. **The artifact produces exactly what it always produced; the instrument gets better.** The withheld record and `tools/test_spelling_scan.py` pin the current figures so each change announces itself rather than passing quietly.

**Another form is `judgement`, added the same day, and it was named in this repo's own documentation the whole time.** [docs/agents/issue-tracker.md](../../docs/agents/issue-tracker.md) lists it beside `neighbouring` as a British form the table does not hold — written to warn about **ticket text**, which nothing scans. **Naming a form in prose is not adding it to the table**, and a form documented as invisible is still invisible. Fixture provenance, exact occurrences and note identities stay in the withheld record.

**A scanner's vocabulary can move while the artifact it measures does not.** Add a form the artifact already contains and the count changes; remove a derivative that ordinary work cannot exercise and it changes again. [#278](https://github.com/mshamblin5150-code/clinical-skills/issues/278) and [#103](https://github.com/mshamblin5150-code/clinical-skills/issues/103) record the concrete changes; exact fixture content, figures and note identities stay in the withheld run record.

**Parallel branches can each assert a correct current tally and still merge into a tree that contradicts both.** The scanner's vocabulary can auto-merge while the prose that reports its figures takes only one branch's value. That is [#86](https://github.com/mshamblin5150-code/clinical-skills/issues/86)'s *the merge is the unguarded moment*: **the preserved tally is checked against what the scanner computes, so a merge that keeps a stale value fails a test instead of shipping.** Exact historical and current figures stay in the withheld run record and are re-derived with `python tools/spelling_scan.py --record`.

**The preserved run also proves that consistent spelling inside one note does not establish consistency across a batch.** Some forms divide by note while others coexist within one note; only the batch-level read exposes the distinction. Exact forms, partitions, occurrences and note identities stay in the withheld run record. [#278](https://github.com/mshamblin5150-code/clinical-skills/issues/278), [#73](https://github.com/mshamblin5150-code/clinical-skills/issues/73).

**The sixteenth is `manoeuvre`, added 2026-08-19, and it is the clearest instance this table has of its own limit.** It was written into a skill file in the **same commit** as `licence`, minutes apart, and `spelling_scan.py --all` reported one and not the other — `licence` is a row above, `manoeuvre` was on no row, and a clean scan said nothing about the difference. It was found by looking by hand afterwards. [#278](https://github.com/mshamblin5150-code/clinical-skills/issues/278); fixture coverage stays in the withheld record.

**The next four came a day later, and they are the growth rule paying for itself.** `counselling`, `millimetre`, `hypoxaemia` with `hypoxaemic`, and `immobilisation`, added 2026-08-20 on the same ticket. **Every one had already been written by this repo**, and the routes matter: one was live in this skill with the staged scan working, another was written in `.py` before the scanner read Python, and the remaining fixture provenance stays in the withheld record. [#104](https://github.com/mshamblin5150-code/clinical-skills/issues/104)'s surface expansion closed the Python gap; the vocabulary gap is why the table still grows.

**`hypoxaemic` is the third stem change**, beside `labelling` and `catheterisation`: the suffix rule appends and `-ia` to `-ic` replaces, so the adjective is out of reach of its own noun. Each carried form has its own repository evidence; a merely possible English inflection is not added without evidence. **Evidence decides which inflections, not English.** Exact preserved-output provenance stays in the withheld record.

**The ticket asked for spelling families and the clinician declined them**, which is what fixes the growth rule rather than leaving it implied. Productive suffix rules fire on correct words too: a scanner that refuses `seizure`, `figure` and `metre` is worse than one that says what it holds. **So the rule stays evidence: a form goes on this table when somebody writes it here.** What generalizes instead is that the command now prints which forms it checked, beside the files it read — a clean run says *no form on this table*, where it used to say something a reader heard as *American English*.
...
It reads Markdown and Python contents plus filenames, and the local `commit-msg` hook reads commit messages. Ticket and PR text remain manual. In Python, a counted `# spelling-scan: mentions N` immediately above one AST statement is the exact-statement equivalent of a code span; changing the count of listed forms without changing the declaration fails the scan. It still holds this table rather than the language: a clean scan means no *listed* form was used — **which it now prints**, beside the set of files it walked, so neither limit has to be remembered off this page. [#278](https://github.com/mshamblin5150-code/clinical-skills/issues/278). `tools/test_spelling_scan.py` parses the table above and asserts the two hold the same rows **in both directions**, so neither can drift — the reverse half was added on [#278](https://github.com/mshamblin5150-code/clinical-skills/issues/278), which is what exercised it: before that, a form could go into the scanner with this table never mentioning it. **The rule is complete without the command** — this table is the instruction, and skipping the scan costs a check rather than an answer.

**A clean spelling scan proves only that no listed form appeared on the surfaces scanned.** It does not prove that an output exercised every listed form, so no run may report an unloaded form as a pass — the [fixture policy](../../fixtures/README.md)'s point about a vacuous row, arriving here. Exact coverage, counts, verdicts and run identities stay in withheld records.

**Issue #103 ruled unloaded forms by whether ordinary note work could exercise them.** A form reached naturally in dietary advice stays; a derivative appearing only in self-audit boilerplate leaves the table when ordinary notes repeatedly reach its root without needing that derivative. **Standing rule 4 still forbids the spelling; the scanner simply does not claim to test a derivative no encounter can exercise.** The preserved record is not edited, so its appearances remain evidence of why the question arose rather than entries the current `--record` vocabulary counts.

**The gitignored run's figures are not recomputable from this repo**, and that is worth stating rather than leaving to be discovered. The command is `python tools/spelling_scan.py <the run directory>`, and the standing answer is the one the [fixture policy](../../fixtures/README.md) gives — **run it again.** A separate committed historical record lets `--record` recompute its own figures; its identity and counts stay outside required instructions.
...
**Fixture generation is the exception.** Follow [fixtures/README](../../fixtures/README.md)'s blind-instruction rule: do not read the clinician's real `scratch/shorthand.md`; run with it absent or with an explicitly synthetic glossary. Ordinary note and day-file work still reads the clinician's file first.
...
**Where nobody named one and the count cannot be checked, write SOAP, say which you chose, and offer to redo it as an H&P.** Ruled 2026-08-16. The rule above needs the `Student Overview` count, and a run outside `batch-shift` usually has no Medatrax session to read it from — so what happens is not that the rule is applied, but that it is **guessed at from the same words that state it**. The branch is not derivable without the portal; evidence and counts remain in the withheld fixture record.
...
Everything else the encounter does not supply goes under GAPS rather than being invented — with three exceptions, each generated by design and declared rather than reported missing: start and end times, which the Times convention estimates; **vitals and body measurements**, which are filled to the value the patient most plausibly had; and **age**, which is inferred and recorded at the top of `FILLED·asserted`.

**Age + unit and Patient Time are emitted as plain field values.** The inference remains only in `FILLED·asserted`, which is the private provenance record: no tier word, guess label, or confirmation warning is ever submitted. It is never copied into the Medatrax block. The final note body follows the same boundary under *The tier language stays out of the note*. The age still drives the ordinary note content and the `Patient Time` band; hiding the provenance from submitted surfaces does not withhold either value. Issue [#158](https://github.com/mshamblin5150-code/clinical-skills/issues/158).
...
FLAG              <a documented finding the finished note still fails to act on>
...
**An item is one numbered `FILLED·proposed` entry, and every entry holds one separately acceptable action.** Repeat the label and write decimal numbers from `1.`; the label and numeral together open the item. A wrapped line belongs to the numbered item that opened it and never opens one. **Separately acceptable is the boundary:** if the preceptor could accept or drop one part without accepting or dropping the other, they are two items. One drug with its complete sig is one action; a second drug is another. Each test, referral, education point, follow-up interval and independently selectable nonpharmacologic intervention gets its own number. `Nutrition and activity counseling` is two items, as are an offloading shoe, non-weight-bearing instructions and elevation. Numbering a bundle does not make it one action.

**The landing is the body content itself, not the section name.** A Plan order or treatment, an Education point, a Follow up interval, a results line, an Assessment entry, or **a self-contained Assessment recommendation for the preceptor to rule on** can each be a landing. An Assessment sentence merely promising a Plan item that is absent is not one. Match each numbered item independently; landing one part of a bundle never lands the rest.
...
**A guideline citation rides on the `FILLED·proposed` item and does not move it.** There is no fourth tier and no `FILLED·cited` line. The three tiers answer *what did this encounter supply*, and for a screening item the answer is still **nothing — this skill wrote it**; a citation answers a different question, which is what stands behind the content. Two axes, so the citation is a tail on the line rather than a new heading. **The load-bearing part is that a cited item stays under FILLED**, where the clinician's confirmation pass reads it: moving cited items out would remove a generated screening list from the block the preceptor must confirm. Format and obligation are under *Guideline sheets*.
...
**FLAG is the block that matters.** A flag is a finding that was documented and remains abandoned in the finished note — an abnormal that reached the Objective and stopped there, a vital nobody addressed, a second problem the Assessment never names. It is neither a gap (nothing is missing from the source) nor a filled line (nothing was generated). It is the note failing to act on what it was told, which is the defect this skill exists to catch.

**A FLAG is always a claim about the finished note, never about the source encounter.** Read each entry literally against the Assessment and the Plan it travels with. If the finished note supplies the action the FLAG says is absent, there is no FLAG — even where the encounter did nothing and this skill supplied the entire response. Merely naming the finding does not clear a true missing-action FLAG. An entry saying *no antibiotic* beside a Plan that prescribes one is false, not encounter-scoped shorthand, and no clarifier sentence makes it acceptable.

One FLAG per finding. Name the finding and name what was not done with it — `BP 151/93 undiscussed`, not `vitals not addressed`.

**A source-encounter omission this skill corrects is not an abandonment, and does not go here.** Testing from a documented exposure is the case — the encounter not having swabbed is not by itself a flag. The generated response belongs under `FILLED·proposed`, not `FLAG`, and the FLAG block says nothing once that response lands in the note. The two are told apart by what remains wrong in the finished artifact: if the note carries the needed order, treatment, education or follow-up, it acted; if the specific response still remains absent, it is flagged. Flagging the corrected omission turns a routine standing order into a recurring accusation, and a block full of those is a block nobody reads — which costs the abandoned finding sitting next to them.

**An order written because a filled value was refused the reassurance behaves identically.** Where the givens alone require a workup the encounter did not do — a chest film for documented diminished breath sounds — the order goes under `FILLED·proposed` and this block stays silent. A clinician who treated empirically made a call, exactly as the one who documented an exposure and did not swab did, and flagging him for it flags him on every encounter where he made the same call. If the finished note orders the film, a FLAG claiming no film is false; if the film still remains absent, that claim is true. See *A filled vital may raise an obligation and may never discharge one*.
...
- **Age.** Inferred by design where the shorthand and the entry both lack it, and recorded at the top of `FILLED·asserted`; the note body and Medatrax fields carry the value without a provenance label.
...
| 21 | **Proposals** | Every `FILLED·proposed` item appears in the note body. **An item is one numbered `FILLED·proposed` entry holding one separately acceptable action.** The repeated label and decimal numeral open it; **a wrapped line belongs to the numbered item that opened it and never opens one**. If a preceptor could accept or drop two parts independently, they are two numbered items. **Checked by counting, the way rows 2, 16 and 18 are:** count the numbered entries, then name where each one landed — the Plan order or treatment, the Education point, the Follow up interval, the results line, an Assessment entry, or **a self-contained Assessment recommendation for the preceptor to rule on**. **An item that landed nowhere was dropped**, and landing one part of a bundled item never lands the rest. Fixed by writing the item or by striking it from the block, **never by a verdict saying it is there**. An `UNKNOWN`, a `FLAG`, a `GAPS` line, or an Assessment promise of an absent Plan item is not a landing |
...
| 23 | **Ranking** | The Assessment's differential is a **numbered list ordered most likely first** — `1.` is the favored entry — and **one entry per numbered item**. No diagnosis is argued down inside a paragraph: a differential written as prose fails this row however good the reasoning in it. The item is the unit, not the line, and a wrapped line belongs to the item that opened it. The diagnosis pins its code with a hyphen on both branches; SOAP continues with `: rationale`, while H&P keeps the Differential as `Name - CODE` and carries the rationale in the matching MDM item. A code in parentheses or pinned with a colon fails this row. The conclusion is exempt from the form and row 22 says why |
| 24 | **Guideline backing** | Every active diagnosis is routed through the catalog and `coverage.md` before its diagnosis-directed testing, treatment, medication continuation or hold, monitoring, referral, and disposition are written. A joined sheet is read even when the decision is qualitative. Every Plan item whose appropriateness rests on a **population** or threshold carries its bracketed tail on that same physical `FILLED·proposed` line; the obligation fires on its subject, never whether it states a number, so an unnumbered screening line owes this row. An item resting on neither a population nor a threshold never fails this row. A qualitative recommendation scoped out only because it holds no number is opened at its named source page and its tail records guideline, status, recommendation or practice-point identifier, population, and page; a draft is labeled `draft`. The verdicts are `no USPSTF row`, `sheet does not settle it`, `recalled, no shipped sheet`, and `recalled, source page unread`, applied under the distinctions above. No citation appears in the note body. Whether the recommendation applies remains a reader's decision, and the mechanical scanner grades only the documented floor it declares below |
| 25 | **Impossible givens** | Every numeric given that cannot be true as labeled is preserved exactly in the note and declared unusable pending verification; `UNKNOWN` quotes it and marks every candidate as a guess not applied; `FLAG` requests source verification and names what is blocked. The value supports no arithmetic, diagnosis, code, dose or clinical reasoning. A structured Medatrax field that would take it carries an explicit `GAPS` entry saying the source value is unusable and the usable value is unrecoverable, never the impossible token or a guessed replacement. An unusual but possible value is outside this row |
| 26 | **FLAG scope** | Every `FLAG` is true of the **finished note** it travels with: the action it says is absent remains absent from the **Assessment or Plan**. A failure of the **source encounter** that the finished note corrects belongs under `FILLED·proposed` and is not a FLAG. Merely naming the finding does not clear a true missing-action FLAG. Check every entry against the note body; encounter-scoped wording and a clarifier do not rescue a contradiction |
| 27 | **Reaction reasoning** | For each distinct allergen whose reaction was filled, after obvious misspellings are corrected, the tier block carries its own `FILLED·asserted` entry naming the allergen, the reaction, and what it was **reasoned from** for that allergen. One rationale for an allergen list fails every entry it does not support separately. Repeated reactions do not fail by themselves, and two spellings of one allergen do not create two entries. A reaction the shorthand supplied is given and outside this row |
| 28 | **Diagnostic closure** | Every diagnosis-and-code pair asserted under `Final diagnosis` appears as a numbered differential entry with the same diagnosis and code. The differential may be wider than the conclusion; the conclusion is never wider than the differential. Three entries are a floor and never a ceiling |
| 29 | **H&P Assessment and MDM** | `Differential diagnoses` is a clean numbered likelihood-ranked list of diagnosis-and-code pairs. `Final diagnosis` follows, selects only entries from that list, and attaches the decisive patient-specific discriminator to each. `Medical Decision Making` follows, numbered one-for-one and in the same order as the differential; each item carries its favored, less-likely, or must-exclude verdict, the case-specific evidence, and why that evidence leads to the decision carried into the Plan |
| 30 | **Medication history** | No finished medication history defers with `unavailable`, `reconcile`, or a missing-sig placeholder. A documented PMH that ordinarily receives maintenance pharmacotherapy has a compatible plausible regimen; a named medication with an incomplete sig has the missing components filled. Every medication carries the drug name in the form the shorthand uses, dose, route, frequency, ongoing status or finite duration, and reason. The private `FILLED·asserted` accounting maps every PMH condition to its medication, a shared medication, or no routine maintenance pharmacotherapy on the available facts; completeness is judged by that coverage rather than a minimum count |
...
| 36 | **Entry copy, form sections, and Plan labels** | The finished note keeps every welded `NOT CODED:` clause for row 22. Its Plan uses each of `Non-pharmacologic:`, `Pharmacologic:`, `Health Promotion/Patient Education:`, and `Referral/Follow-up:` exactly once and no other label. Oxygen is pharmacologic; monitoring, positioning, and laboratory and imaging orders are non-pharmacologic, with orders also on Objective's `Labs/Tests today` line; health promotion and referrals use their named labels. Run `python tools/entry_copy.py <finished note>` and `python tools/form_sections.py <entry-copies/note-N.md>` before entry; either refusal blocks posting. The Entry copy removes `NOT CODED` clauses and substitutes measured portal characters; unmeasured non-ASCII characters in pasted sections refuse. The form command accounts for every line and preserves an existing record, reporting divergence without blocking or overwriting it. Enter and read back the Entry copy's four derived form sections. The finished note, never the Entry copy, supplies `SUBMISSION-SHA256` |
...
**It applies to two measures today and is written for more.** Blood pressure against hypertension and BMI against obesity are both covered by withheld fixture rows. A third would qualify on the same test the vitals section states: the value has to *be* what the condition is diagnosed by. Most vitals are not, for most conditions, and this row is not an invitation to explain every normal number in the note.
...
**Row 15 once opened with a sentence broader than the rest of the row, and it was struck on 2026-08-16.** It read *Every reassurance in the note traces to a given*, while the operative sentence reaches only a decision to withhold, defer or narrow. **A clause can reassure without changing anything the note does.** The clinician ruled for the narrower row: *a reassuring clause that changes no action is not a discharge*. Concrete runs, encounters and clauses remain in the withheld fixture record under [#147](https://github.com/mshamblin5150-code/clinical-skills/issues/147)'s rule. Issue [#69](https://github.com/mshamblin5150-code/clinical-skills/issues/69).

**What the ruling costs is that the sentence a reader sees is not the sentence this row grades.** A clinical sentence may use a measurement in differential reasoning while provenance remains visible only in the FILLED block. Widening the row was rejected because reasoning from the vitals in front of the clinician is not the same act as using a generated value to withhold work. This row forbids the second act, not the first.

**A withheld fixture row states this rule a second time, and a test holds the two together.** The test reads the *Passes when* cell out of each table and asserts neither still states the retired sentence and both state the ruling. It reads the **cell** rather than the file, because both files argue about the retired sentence in prose below their tables and must go on being able to — deleting the record fails as loudly as leaving the rule broad.
...
**Row 17 makes existing prohibitions walkable at the social and allergy slots.** Row 12 bans `Allergies (reaction): Not documented this visit.` because it defends the note rather than reporting on the patient. Row 1 bans an invented positive tobacco history because an abnormal finding must trace to a given. Neither rule by itself directs the final matrix walk to inspect these slots, so row 17 does.
...
**Row 19 is the first row a single note cannot fail loudly.** Rows 1 through 18 each resolve inside the document they are walked against: a finding is in the Assessment or it is not, an order landed somewhere or it did not. Row 19's subject is a value that is individually defensible and collectively a template — `5'9"` for a 35-year-old man is an ordinary patient, and four of them are one patient written four times. **So the row does not ask whether the number is right.** It asks whether the line says how the number was arrived at, which is a question one note can answer, and it leaves the cross-note half to [batch-shift](../batch-shift/SKILL.md) step 6 and to a withheld set-level fixture row.

**Its two prohibitions are one prohibition seen from both sides.** *Do not move the value to avoid making the disclosure* was already written under *A filled BMI near a threshold says how near*, and it was written about one direction because that was the direction someone had thought of. A run can fail in the other direction too, by systematically choosing abnormal values in a file whose abnormal-value machinery is much longer than its normal-value machinery. **A run demonstrating compliance produces something to be compliant about**; it does not justify moving values toward either side.
...
**A second narrowing landed on 2026-08-17 and it is the mirror of the one below.** The zero's carve-out removes the *no anchor* exit; the height's leaves it and adds to it — a filled height's line **names the age and the sex** as well whenever those anchors are available. Write the age in digits followed by a year, month, week, or day unit however abbreviated, or write the digits after `age`; punctuation between those parts does not matter, and a number spelled as a word does not name the age. So `no habitus or percentile datum in the source to move it` still passes and no longer passes *alone*. **Ruled by the clinician**, with the pressure bar beside it, and both are settled by `python tools/filled_vitals_census.py`. The argument is in *Two of these are graded now* above. Issue #97.
...
**Do not author a fixture merely to make that limb pass.** A legitimate filled zero needs a complaint that does not hurt in an encounter with no pain source; the [fixture policy](../../fixtures/README.md) requires a real coverage reason rather than a manufactured example. [#138](https://github.com/mshamblin5150-code/clinical-skills/issues/138).
...
**It is the first row whose failures were all compliant when they happened**, which is a different thing from rows 17 and 18's *the prohibition was already in the file*. Here there was no prohibition and no permission, so runs could refuse the `E66`/`Z68` family, remain silent, or write it as though filled measurements were measured without contradicting the instructions. **A run cannot be graded against silence**, so the row exists to end the silence rather than to catch a defect the old text forbade.

**Its two limbs fail in opposite directions and a note can only fail one.** Withholding the family solely because its inputs were filled fails one limb; writing it without disclosing those inputs fails the other. The pass is the narrow thing between them — write the code, verify it, and say what it rests on.
...
**Its remaining carve-out is narrow and not a general escape.** A code withheld on a **coding-guidelines** ground is outside the row because nothing here encodes those guidelines: whether a `Z68` should be assigned at a *normal* BMI, with no reportable condition under it, remains an open question in a withheld fixture record. **It does not reach provenance.** *This value was filled* is never a reason to withhold, and a run citing it to hold back an `E66.3` has used a guidelines exemption to do a provenance refusal. Issue #46. Pediatric BMI bands have no verification carve-out now: the CDC table and calculator settle them. Issue #123.
...
**It is not row 18 widened, and the two rows have opposite subjects.** Row 18's subject is a **given** order — something the clinician did, which the note must carry. Row 21's is a **generated** proposal — something this skill contributed, which the note must actually contain rather than merely list. A note can carry every given order and still promise itself a counseling line it never wrote, and it reads as a pass on row 18 because nothing the shorthand ordered went missing. Widening 18 would also redate a recorded `GIVEN` score, which is the cost *append, never insert* exists to avoid.

**It is checked by counting, and the block states its own denominator.** Count the numbered `FILLED·proposed` entries and name where each separately acceptable action landed in the finished note. The numeral, not a physical line, is the boundary; a wrap belongs to the item above it. An item that landed nowhere was dropped, and a block that bundles independently selectable actions fails before its landings are counted. That keeps the row a comparison of two integers, which is what rows 2, 16 and 18 rely on and what makes a false self-report impossible to argue about. Issue [#127](https://github.com/mshamblin5150-code/clinical-skills/issues/127).

**Its quiet failure is a note that grades itself as compliant.** A note can put a counseling action into `FILLED·proposed`, omit every matching line from the Plan, and still report a pass by pointing at an Assessment sentence. The claim then has a real string behind it at every level except the one that acts. The landing count exposes that omission without relying on the note's self-report.
...
**It is not row 13 widened, and the two rows fail on different objects.** Row 13's subject is the **code**: does a code whose descriptor names a confirmed organism or disease sit on a diagnosis the encounter did not establish. Row 22's is the **label**, and a note can pass row 13 perfectly while failing this one — `Nondisplaced fracture of head of left radius - M25.522 Pain in left elbow` assigns no over-claiming code anywhere and still puts a disease nobody established at the head of an entry. That is why a second row was needed rather than a wider first one. Issue #68.
...
**Row 24 is appended for the reason row 23 was, and it is not row 21 widened.** Row 21 asks whether every `FILLED·proposed` item landed somewhere in the body — two counts, and a note that wrote all of them passes. Row 24 asks a question about the same items that counting cannot reach: **was the number in it looked up or recalled.** A complete screening list can pass row 21 while every item appears in both the Plan and block without any of them having been checked against a sheet.

**The one-action boundary is load-bearing here too.** A bundle of screenings or counseling points may require different populations, grades or silence verdicts, so one tail cannot answer honestly for all of them. Each separately acceptable action is its own numbered item and carries its own verdict; the row is walked only after row 21's item shape is satisfied.
...
**`python tools/differential_scan.py <a run directory>` supplies row 24's mechanical floor.** It reads every physical `FILLED·proposed` opener, fails a guideline tail pushed onto a continuation line or a bracket opening with a tail keyword that matches no documented form, checks a directly joined USPSTF grade and year or a threshold sheet's source, class and value, and disproves a `no row`, `sheet does not settle it`, or `recalled, no shipped sheet` verdict where the shipped artifacts make the contradiction mechanical. Screening, counseling, immunization, vaccination, target, cutoff and threshold subjects with no tail are definite findings. For every other missing tail, the command refuses to guess whether the item depends on a population or threshold: **missing tails on undecidable items are candidates**, printed outside the exit status. A `recalled, no shipped sheet` verdict whose subject joins no registry topic and a `recalled, source page unread` verdict are likewise candidates. A run with no qualifying item is a clean zero on this floor, not an unscanned run.

**The scanner never reaches the limb that matters.** Whether a correctly extracted recommendation applies to the patient in front of the clinician is a clinical judgment, and the citation exists to put the population where a reader can rule on it — never to rule on it. Every report says so, and **a clean scan is not a walked row 24**. A row that scored applicability would be asserting exactly the thing this whole section is arranged to avoid asserting. Issue [#85](https://github.com/mshamblin5150-code/clinical-skills/issues/85), scanner floor [#192](https://github.com/mshamblin5150-code/clinical-skills/issues/192).

**Row 25 is appended for the reason rows 14 through 24 were. Append, never insert.** Its natural neighbors are row 9, which owns arithmetic, and row 10, which owns the Medatrax entry; it sits at the bottom so every existing numbered citation keeps its subject.

**It is not row 10 widened.** Row 10 asks whether a field has a provenance category, and an impossible number is still a given — which is exactly why that row passed while a field could carry a value no patient had. Row 25 asks whether the value is usable at all and makes the `GAPS` exit explicit. A note can pass row 10 on provenance and fail this one on validity.

**Check it from the shorthand forward, one impossible token at a time.** Name where the exact token remains visible, read its `UNKNOWN` and `FLAG` entries, then inspect every arithmetic line, diagnosis, code, dose and reasoning clause for downstream use. Where the token maps to a structured Medatrax field, read that field for the explicit `GAPS` entry. A range scan may supply candidates for this walk and cannot settle it: the drug-strength case needs a reference this repo does not ship, while a bare pressure-shaped pattern produces a valid medication-strength false positive. A clean scan is therefore never a walked row. Issue [#132](https://github.com/mshamblin5150-code/clinical-skills/issues/132).

**Row 26 is appended for the reason rows 14 through 25 were. Append, never insert.** Its natural neighbor is row 2, which decides whether the finished note addressed a documented finding, and it sits at the bottom so every existing numbered citation keeps its subject.

**It is not row 2 widened and not the block's naming rule repeated.** Row 2 walks the shorthand forward and asks where each abnormal landed. Row 26 walks every FLAG backward against the finished note and asks whether the block's claim is true. A finding can land in the Plan, pass row 2 and still be described by a FLAG as having no treatment; a naming-only check can then pass because the entry names both the finding and the claimed omission. The new row is what catches the contradiction.

**Check it one FLAG at a time, literally.** Name the finding, name the action the entry says is absent, and read the Assessment and Plan for both. If the action is present, remove the FLAG; if the finding still has no response, keep it and name that remaining failure. Never rescue a false entry by saying it describes the encounter — the block travels with the finished note and reports on that artifact. Issue [#159](https://github.com/mshamblin5150-code/clinical-skills/issues/159).

**Row 27 is appended for the reason rows 14 through 26 were. Append, never insert.** Its natural neighbor is row 17, which establishes that an inferred reaction exists, and it sits at the bottom so every existing numbered citation keeps its subject.

**It is not row 17 widened.** Row 17 grades the allergy field's completeness, placement, category, reaction presence, dependency disclosure, and never-discharge boundary. A note can satisfy every one of those limbs while writing one plausible reaction several times from one generic sentence. Row 27 grades the missing object: the reasoning for each filled reaction. Keeping the two rows separate means a reaction can be present and safe yet still fail because nobody reasoned from its allergen.

**No rate and no cap are hidden in the row.** Several distinct allergens can honestly produce the same reaction, so requiring variation would make compliance depend on inventing clinical history. Walk the distinct allergens after spelling correction, then match each to one declaration and read that declaration for its own class or encounter anchor. A shared rationale is missing reasoning, while a repeated value with separate reasoning passes. Issue [#205](https://github.com/mshamblin5150-code/clinical-skills/issues/205).
...
**Then invoke `/AAR`.** Use the encounter's run key plus the submission date as the submission key; where `batch-shift` owns the run, use the shift key that skill supplies rather than creating a second review per nested note. For a standalone note, after `/AAR` exits clean run:

```bash
python tools/differential_scan.py <the run directory> --submission <encounter-key>-<date>
```

The completion output must include both `the after-action review: clean` and `the Medatrax posted reading: clean`. The scanner's clinical floors remain unchanged; a missing review or a missing or stale `SUBMISSION-SHA256` fails completion.

...
**Measured rather than assumed, and re-derivable rather than quoted.** [#56](https://github.com/mshamblin5150-code/clinical-skills/issues/56) audited a committed historical run's diagnosis lists against `reference/icd10cm-2026.sqlite`. Every code resolved, and a substantial subset carried `unspecified` or `not specified` in its official descriptor. Exact counts and record identity remain in the withheld fixture files, where `tools/test_specificity_scan.py` pins them against the committed notes.
...
**And the skill does not necessarily choose that inch freshly each time.** `tools/filled_vitals_census.py` over a historical run found repeated filled heights across a small set of distinct values. So a `Z68` off a filled body can be substantially a readout of the skill's modal height. Exact counts remain in the withheld run record. That is [#67](https://github.com/mshamblin5150-code/clinical-skills/issues/67) rather than this skill's defect to fix — but it is why the mark is not decoration.
